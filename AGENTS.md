...
- `bin/remote-ui-smoke`: start a throwaway Remote UI server with temp config/log roots, create a fixture agent, and run a Chrome/Playwright smoke check for composer refresh preservation and mobile dock layout. Set `TYCHO_REMOTE_UI_SUMMARY_ATTACHMENTS_ONLY=1` to isolate Summary attachment overflow, input, refresh, and ancestor-scroll behavior from the broad smoke suite.
