...
`generation` and `agent_key`, plus pending old-generation proposals as
`expired_actions`. Each expired action is read-only, carries its prior
`historical_state`, and omits preflight and precondition authority. When the
archived agent is available, `archived_conversation` supplies read-only agent
and conversation paths. These records are factual continuity only; they cannot
be confirmed or executed in the new generation.
