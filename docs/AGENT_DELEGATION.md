...
The one narrow environment-based exception is delayed self-continuation: `tycho agent send "$TYCHO_AGENT_KEY" "Continue" --delay 60` records the current agent as the author without declaring or inferring a parent. It preserves the current edge owner and ownership generation. Immediate self-send and sends to any other key retain the normal user/parent rules.
...
Each relationship also records an owner and ownership generation. A direct prompt without `--parent-agent` changes the owner to `user` before the prompt is stored, advances the generation, removes undelivered reports, and cancels queued parent resumes. A later prompt declared with the recorded parent key restores `parent` ownership and advances the generation. Repeated prompts from the current owner keep the same generation. Parent prompts cancel any unresolved child inquiry before adding the new prompt; they are not treated as answers to that inquiry. When the child is already running, Tycho atomically stores the prompt, author, source, and resulting ownership stamp in its FIFO queue. Once the receiver claims queued work, it owns the complete pending batch: Tycho preserves FIFO content order, combines every pending entry into one native input, and uses the newest entry's owner and generation as the single run stamp. Per-entry stamps remain acceptance history; they do not split receiver-side batches across Takeover or reclaim boundaries.
...
Tycho gives each report a deterministic queue ID and accepts it into the active parent's durable prompt queue. The queue starts one consolidated batch after each terminal run, so simultaneous child replies and user prompts produce one native input rather than concurrent or serial model calls. It delays dispatch while another agent in the same workspace is running. A running parent exposes later replies as pending work and processes the complete pending queue after its current run. The newest entry supplies the batch run's ownership stamp. An archived parent receives the report in archived history but is never unarchived or resumed. Missing parents remain recorded in the delegation ledger.

Queue entries may carry `not_before`. Batch selection scans the durable acceptance order, moves every entry due at claim time into one batch in that same relative order, and leaves future entries in place. This avoids head-of-line starvation without reordering eligible work. The exclusive store lock persists a claim before launch, so restart and concurrent pollers cannot duplicate a delayed dispatch.

The sleep circuit breaker consumes normalized pre-execution tool invocations from Codex, Claude, and Pi. It ignores prose and completed-only events, deduplicates call IDs, and stops the complete harness process group on the third typed blocking wait. OpenCode remains capability-gated because a live 1.18.4 capture emitted only a completed `tool_use` record. A breaker incident records `sleep_circuit_breaker`, renders **Stopped due to overusing sleep-like commands**, and schedules one generation-bound delayed recovery. Manual send/run/takeover cancels that recovery under the same store lock; a recovery-triggered incident never schedules another recovery.

`POST /agents/:key/messages` returns HTTP `202` with `accepted: true`, `queued: true`, `started: false`, queue position, safe source/authority metadata, and the current agent when the target is running. `tycho agent send` prints the same queued-versus-started distinction and no longer rejects this state. When the active process exits or Stop finishes terminating it, Tycho automatically claims all due entries as one canonical batch and starts the eligible agent. Archived and blocked agents, active or restorable inquiries, paused or stopped schedule-owned agents, prior dispatch failures, and workspaces with another running agent remain non-dispatchable. Archive refuses ordinary queued prompts and mixed queues. A queue containing only protected `delegation_callback` entries may be archived without running obsolete callbacks: Tycho moves each unrun callback into read-only conversation history, clears the executable queue, and reports `archived_delegation_callback_count`. Explicit Retry queue resumes a failed dispatch without duplicating its prepared memory event.

Targeted agent CLI results also expose a non-destructive `queue_notice` when the current managed agent (`TYCHO_AGENT_KEY`) has pending or open work. `tycho queue AGENT_KEY` or its Remote equivalent atomically moves every pending entry into one durable open QueueWork batch and records one user Conversation block labeled **Read queue**. Repeated reads return that same batch and never create another block. New arrivals remain queued for the next batch. The response and Conversation block share the canonical FIFO entries and attachment arrays, while a required-actions projection presents user instructions first and retains delegated callbacks as structured contextual reports. Attachment normalization removes only exact duplicate normalized records; repeated file or URL targets remain separate when retained metadata differs. A write failure leaves the queue intact.

Queue delivery never means completion. Native input starts with the `tycho.queue_work/v1` contract: batch ID, stable FIFO entry IDs, user-instruction/report counts, the required instruction checklist, and the completion command. `tycho queue-work complete` records `completed`, `needs_input`, or `declined_with_reason` for user instructions and `incorporated` or `superseded_with_reason` for delegated callbacks. Reason outcomes require a reason. Partial valid progress persists, but missing, duplicate, unknown, conflicting, or invalid entries keep the batch open and return unresolved IDs. Identical completion is idempotent. If a run reports success before all entries have outcomes, Tycho records a gated partial attempt and resumes the same native session once; another unresolved finish remains operator-visible without an uncontrolled loop. `needs_input` leaves the completed disposition set visibly blocked.
...
Archiving either side does not delete the ledger. Callback-only parent archives mark the corresponding reports `parent_archived`; their complete queued messages remain in `memory.jsonl` with `archived_without_run: true`. Archived agent detail and conversation routes are read-only, so links keep working regardless of archive order.
