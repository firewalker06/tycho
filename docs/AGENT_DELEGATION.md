...
When `tycho agent create` runs inside a managed Tycho agent, it inherits `TYCHO_AGENT_KEY` as the parent automatically. This fail-safe keeps delegation durable even when an agent has an outdated skill or omits `--parent-agent`:
