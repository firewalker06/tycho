...
Tycho gives each report a deterministic queue ID and accepts it into the active parent's durable prompt queue. The queue starts at most one ordinary item after each terminal run and continues in FIFO order, so simultaneous child replies never create concurrent parent runs. It delays dispatch while another agent in the same workspace is running. A running parent exposes later replies as pending work and processes them after its current run; each callback keeps the parent's authority stamp from acceptance. An archived parent receives the report in archived history but is never unarchived or resumed. Missing parents remain recorded in the delegation ledger.
