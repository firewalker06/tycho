...
Targeted agent CLI results also expose a non-destructive `queue_notice` when the current managed agent (`TYCHO_AGENT_KEY`) has pending work. `tycho queue AGENT_KEY` or its Remote equivalent reads every entry present under the durable store lock, records one user Conversation block labeled **Read queue**, then consumes exactly that batch. The read response and Conversation block share one canonical attachment array in queue order. Attachment normalization removes only exact duplicate normalized records; repeated file or URL targets remain separate when any retained metadata differs. A write failure leaves the queue intact. Entries accepted after the locked read remain queued for the next batch.
