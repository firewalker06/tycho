...
| Settings in-page sections | Settings | selection and panel rhythm coupled to sticky shell; Session loop fields bypassed shared input colors; peer actions and empty catalog editors consumed excess horizontal/vertical space | `ds-section-nav`, `ds-section-panel`, `ds-input`, native catalog disclosure and contextual menu | Shared selection/panel and form styling migrated; peer actions use one More menu and collapsed catalogs retain value summaries |
