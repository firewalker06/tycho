...
| Settings in-page sections | Settings | selection and panel rhythm coupled to sticky shell; Session loop fields bypassed shared input colors | `ds-section-nav`, `ds-section-panel`, `ds-input` | Shared selection/panel and Session loop input styling migrated; sticky offsets remain product-specific |
