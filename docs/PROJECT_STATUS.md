...
| Cross-harness response style | Append the current `~/.tycho/config/response_style.md` policy to every cold and resumed run, with project/template override or opt-out and a per-run SHA-256 fingerprint | Keep operator-facing prose consistent without coupling the policy to a harness-native role API or persisting it into conversation memory; explicit task formats remain higher priority |
