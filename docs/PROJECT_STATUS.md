...
| Managed-agent store recovery | Validate every save, retain a private session/archival recovery ledger, and create checksum-verified atomic daily snapshots with configurable 30-day default retention | Lost session IDs self-heal from run history or last-known state; archived-key resurrection is rejected; restore validates both source and a pre-restore safety snapshot before atomically replacing the active store |
