...
| Running-agent prompt queue | Persist accepted queue entries and one durable claim in each managed-agent record; claim and dispatch under the existing agent-store lock, retain prepared claims on start failure, and let later accepted entries form the next batch | Server acceptance order survives reloads, multiple clients cannot double-claim work, retry cannot duplicate the prepared user message, and unresolved inquiries remain authoritative blockers |
...
synchronized, editable server-side queue. Terminal runs claim pending entries
as one ordered follow-up prompt, while inquiries and retryable start failures
retain the queue without silently advancing the conversation.
