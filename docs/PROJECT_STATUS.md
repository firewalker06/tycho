...
| Running-agent prompt queue | Persist direct, parent-declared, and delegated-callback entries plus one durable claim in each managed-agent record; capture message authorship and the exact delegation ownership generation at acceptance, then claim one ordinary entry per run under the agent-store lock. FRED retains its existing same-session acceptance batching. Remote UI keeps the composer enabled, labels queue source and captured authority, and places Stop in the shared top action row. | FIFO order and authority survive reloads, ownership changes cannot rewrite delayed work, multiple clients cannot double-claim, callback IDs stay deduplicated, start failures and Stop retain explicit retryable work, unresolved inquiries remain authoritative blockers, and archive or scheduled-session refresh refuses pending prompts instead of discarding them. |
