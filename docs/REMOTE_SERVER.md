...
The Conversation composer has a full-screen editor for longer prompts. It opens as an accessible modal with a constrained writing canvas, focus containment, visual-viewport sizing for mobile keyboards, and only an X close control—no editor header or explanatory copy. Draft text, focus, selection, attachments, and editor mode survive same-route polling refreshes. Drafts save locally while typing and survive reloads; route navigation, sending, the X control, or one Escape press exits full screen without discarding the draft.
