...
Structured inquiry forms use the same full-screen interaction and always end with an optional **Leave feedback** textarea. Declared inquiry fields remain the complete structured JSON answer. Feedback is submitted separately and recorded as a following user message with `inquiry_feedback` metadata, so free-form context never changes the parsed inquiry response.
