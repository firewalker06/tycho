...
async function ensureAttachmentImage(id, force = false) {
  ...
  try {
    const blob = await apiBlob(attachmentBlobPath(id));
...
function render() {
  ...
  els.nav.classList.toggle("hidden", subpage);
  els.view.classList.toggle("no-nav", subpage);
  els.view.classList.toggle("detail-page", subpage);
  els.view.classList.toggle("agent-detail", route.type === "agent" || route.type === "agentAttachment");
  els.header.classList.toggle("detail-header", subpage);
  if (route.type !== "agent" && route.type !== "agentAttachment") setAgentSettings(null);
  syncDetailHeaderLayout();
  if (subpage) {
    setNavHidden(false);
    updateDetailHeaderVisibility({ route, forceShow: window.scrollY < 16 });
...
function renderNow() {
  ...
  const scheduleAttention = attentionSchedules();
  ...
  const summary = waiting.length > 0
    ...
    `
    : "";
  const scheduleSummary = scheduleAttention.length > 0 && waiting.length === 0
    ? `
      <section class="summary-card attention">
        <div class="big-number"><strong>${scheduleAttention.length}</strong><span>schedule item${scheduleAttention.length === 1 ? "" : "s"} need attention</span></div>
        <p class="summary-text">Check scheduler freshness, paused schedules, failures, and interactive skips.</p>
      </section>
    `
    : "";
  ...
  return `
    <section class="detail-card">
      <div class="detail-card-body" style="padding-top: 12px;">
        <div class="section-label schedule-section-label">
          <div class="schedule-section-title">
            <strong>Schedules</strong>
            ${scheduleDaemonHeaderPills(daemon)}
          </div>
          <span class="pill ${daemonClass}">${escapeHtml(daemonStatus)}</span>
        </div>
...
function renderScheduleRow(schedule) {
  ...
  return `
    <div class="detail-row">
      <span class="status-mark ${className}" aria-hidden="true">${iconSvg("activity")}</span>
    ...
    <section class="detail-card">
      <div class="detail-card-body" style="padding-top: 12px;">
        ...
        <div class="kv-grid">
          ...
          ${kv("Archived", `${setup.counts?.archived_projects || 0} hidden`)}
...
function renderAgentAttachmentView(agent, attachmentId) {
  ...
  if (attachment.agent_key && attachment.agent_key !== agent.key) {
    return `
      <section class="agent-attachment-shell">
...
function renderAttachment(attachment) {
  ...
  if (href) {
    return `<a class="attachment-item" href="${escapeAttr(href)}" ${external ? 'target="_blank" rel="noreferrer"' : ""}>${content}</a>`;
  }

  return `<div class="attachment-item">${content}</div>`;
}
...
function attachmentBlobPath(id) {
  return `/attachments/${encodeURIComponent(id)}/blob`;
}
...
function setHeader(title, subtitle) {
  ...
  els.mark.innerHTML = brandLogoHtml();
  syncDetailHeaderLayout();
}
...
function brandLogoHtml() {
  ...
  return `<img class="brand-logo" src="/remote-logo.png${suffix}" alt="">`;
}
...
function setHeaderHidden(hidden) {
  ...
  els.header.classList.toggle("header-hidden", hidden);
}
...
function detailFooterFocused() {
  return !!document.activeElement?.closest?.("[data-agent-dock]");
}

function updateDetailHeaderVisibility({ route = parseRoute(), delta = 0, currentY = window.scrollY, forceShow = false } = {}) {
  if (route.type === "tab") {
    setHeaderHidden(false);
    return;
  }

  if (detailFooterFocused()) {
    setHeaderHidden(true);
  } else if (forceShow || currentY < 16 || delta < -8) {
    setHeaderHidden(false);
...
function handleScrollDirection() {
  ...
  if (route.type !== "tab") {
    updateDetailHeaderVisibility({ route, delta, currentY });
...
els.nav.addEventListener("click", (event) => {
  ...
  if (!button) return;
  navigate({ type: "tab", tab: button.dataset.tab });
});
...
window.addEventListener("focusout", () => {
  window.setTimeout(() => updateDetailHeaderVisibility(), 0);
});
