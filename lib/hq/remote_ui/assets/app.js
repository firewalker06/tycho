    ...
    <section class="workspace-browser" aria-label="Project workspace file browser">
      ...
      <div class="workspace-browser-grid ${route.file ? "has-preview" : ""}">
        <section class="workspace-list-pane" aria-labelledby="workspace-list-title">
          ...
          <div class="workspace-search"><label class="sr-only" for="workspace-search-input">Search workspace files</label><input id="workspace-search-input" type="search" data-workspace-search data-project-key="${escapeAttr(route.key)}" value="${escapeAttr(searchTerm)}" placeholder="Search files" autocomplete="off"><span class="workspace-search-status" aria-live="polite">${searchTerm.trim() ? (search?.loading ? "Searching…" : `${search?.total || 0} matches`) : ""}</span></div>
