...
els.view.addEventListener("submit", (event) => {
  ...
  if (["personal-assistant-setup-form", "personal-assistant-settings-form"].includes(event.target.id)) {
    ...
    mutate(async () => {
      const configured = await apiPost("/personal-assistant/setup", { model: String(values.get("model") || ""), reasoning_effort: String(values.get("reasoning_effort") || ""), timezone: String(values.get("timezone") || ""), external_events_prompt: String(values.get("external_events_prompt") || ""), confirmed: values.get("confirmed") === "on" });
