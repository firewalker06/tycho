...
async function saveRemoteServerToken(key, value) {
  ...
  const data = await brokerGetWithHeaders(`/servers/${encodeURIComponent(serverKey)}/health`, {
    "X-Tycho-Remote-Server-Token": tokenValue,
  });
  if (data.server && data.server.healthy === false) {
    throw new Error(data.server.error || "Remote server rejected broker credentials");
  }
...
function settingsMoreMenuHtml(setup) {
  ...
  const serverButtons = state.servers.length > 1
    ? [
        moreMenuSeparator(),
...
function renderAttachment(attachment) {
  ...
  const actions = `
    <span class="attachment-actions">
      ${download && id ? `<a class="icon-button" href="${escapeAttr(download)}" data-download-attachment="${escapeAttr(id)}" data-download-filename="${escapeAttr(title || "attachment")}" download="${escapeAttr(title || "attachment")}" aria-label="Download attachment" title="Download attachment">${iconSvg("download")}</a>` : ""}
...
function attachmentViewerHtml(id, options = {}) {
  ...
  const download = attachmentDownloadHref(attachment);
  const downloadButton = download && id ? `<a class="button primary button-link" href="${escapeAttr(download)}" data-download-attachment="${escapeAttr(id)}" data-download-filename="${escapeAttr(title || "attachment")}" download="${escapeAttr(title || "attachment")}">Download file</a>` : "";
...
function attachmentViewerActions(id, attachment) {
  ...
  const downloadAction = download
    ? `<a class="inline-icon-button" href="${escapeAttr(download)}" data-download-attachment="${escapeAttr(id)}" data-download-filename="${escapeAttr(String(attachment?.title || attachmentTarget(attachment) || "attachment"))}" download="${escapeAttr(String(attachment?.title || attachmentTarget(attachment) || "attachment"))}">${iconSvg("download")}<span>${downloadLabel}</span></a>`
    : "";
...
function quickSwitchLogoLabel(unreadCount = unreadAgents().length) {
  ...
  return `${unreadLabel}Open agent switcher`;
}
...
els.view.addEventListener("submit", (event) => {
  ...
  if (event.target.id === "server-connection-form") {
    ...
    connectLoopbackServer(formData.get("url"), formData.get("name"), formData.get("token")).catch((error) => {
      state.failureCount += 1;
      setConnection(error.message);
      render();
    });
  ...
  if (tokenForm) {
    const form = event.target;
    const formData = new FormData(form);
    saveRemoteServerToken(form.dataset.serverTokenForm, formData.get("token")).catch((error) => {
      state.failureCount += 1;
      setConnection(error.message);
      render();
    });
