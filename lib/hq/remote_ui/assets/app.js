...
function renderPersonalAssistant() {
  ...
  const thread = agent
    ...
    : `<div class="pa-conversation-scroll" data-preserve-scroll data-state-key="personal-assistant-thread">${item.configured ? `${continuity}${tychoLoadingState("Loading FRED", { className: "pa-loading-state", body: "Fetching today’s session." })}` : renderPersonalAssistantSetup(item)}${item.error ? feedbackMessage("Sending is paused", item.error, { intent: "danger" }) : ""}</div>`;
