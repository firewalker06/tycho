...
function renderPullRequestDiffDetail(agent, item, diff, options = {}) {
  ...
  return `
    ...
    <section class="diff-viewer" aria-label="Pull request diff"
      ...
      data-state-key="pr-diff-viewer:${escapeAttr(agent.key)}:${escapeAttr(item.id)}"
      data-poll-content-version="${escapeAttr(`${diff.snapshot_id || "none"}:${expandAll ? "expanded" : "collapsed"}`)}">
...
function syncPullRequestDiffSelection(agentKey, pullRequestId, options = {}) {
  ...
  window.requestAnimationFrame(() => {
    ...
    // Reassert the state model once without rebuilding the diff.
    syncPullRequestDiffLineControls(controls, selectedKeys, selectionLocked);
    if (options.focusComment && selection.lines.length) {
