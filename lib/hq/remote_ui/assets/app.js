...
// FRED (Personal Assistant) reconciliation logic needs to see the whole
// active session's history to match pending submissions/terminal runs, and a
// daily PA session is bounded in practice, so request the server's maximum
// window instead of the tighter default used for ordinary agent views.
const PERSONAL_ASSISTANT_CONVERSATION_LIMIT = 2000;
...
async function ensureConversation(agent, force = false) {
  ...
  if (personalAssistantAgent(agent)) {
    ...
    return personalAssistantRequest(requests, requestKey, async () => {
      try {
        const suffix = loadSnapshot
          ? `/conversation?limit=${PERSONAL_ASSISTANT_CONVERSATION_LIMIT}`
...
async function loadOlderConversationMessages(agentKey) {
  ...
  try {
    ...
    if (target) {
      target.blocks = olderBlocks.concat(target.blocks);
