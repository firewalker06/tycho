...
const PROMPT_ATTACHMENT_LIMITS = {
  ...
  accept: "image/*,video/*,audio/*,application/pdf,application/json,application/rtf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/*,.txt,.md,.markdown,.pdf,.rtf,.doc,.docx,.json,.jsonl,.csv,.tsv,.log",
};
...
function renderAgent(key, options = {}) {
  ...
  if (options.splitWorkspace && focusedMode) {
    ...
    replaceView(renderAgentWorkspace(agent, blocks, {
      ...
      detailKind,
      focusedMode: true,
      fullView: pullRequestMode && state.prDiffFullViewMode[agent.key] === true,
    }));
    return;
  }
...
function renderAgentConversationView(agent, blocks, options = {}) {
  return `
    <div class="message-list">
      ${blocks.length ? renderConversationBlocks(blocks, { agent }) : emptyState("No conversation yet", "Send a prompt to start or continue the agent.")}
    </div>
...
function renderAgentPullRequestDiffView(agent, pullRequestId, options = {}) {
  ...
  const body = `
    <section class="agent-pr-diff-shell">
      ...
      ${data.loading && !refs.length ? `<div class="empty-state pr-loading-state">${iconSvg("loaderPinwheel")}<span>Loading pull requests…</span></div>` : ""}
...
function renderPullRequestDiffToolbar(agent, data) {
  ...
  const refreshAllBtn = fetching
    ...
    : `<button class="inline-icon-button" type="button" data-refresh-all-pr-diffs="${escapeAttr(agent.key)}" ${disableButtons ? "disabled" : ""}>${iconSvg("rotateCcw")}<span>Refresh all diffs</span></button>`;
  return `
    <section class="diff-toolbar pr-diff-toolbar" aria-label="Pull request diff actions">
      <button class="inline-icon-button" type="button" data-refresh-agent-prs="${escapeAttr(agent.key)}" ${disableButtons ? "disabled" : ""}>${iconSvg("rotateCcw")}<span>Refresh list</span></button>
      ${refreshAllBtn}
...
function renderPullRequestDiffDetail(agent, item, diff, options = {}) {
    ...
    <section class="summary-card diff-summary-card pr-diff-summary">
      <div class="card-title pr-diff-title-row">
        ...
        <div class="pr-diff-title-actions">
          <button class="inline-icon-button" type="button" data-refresh-pr-diff="${escapeAttr(item.id)}" data-agent-key="${escapeAttr(agent.key)}" ${loading ? "disabled" : ""}>${iconSvg("rotateCcw")}<span>${snapshot ? "Refresh" : "Fetch"}</span></button>
  ...
  if (!snapshot) return `${header}${loading ? "" : emptyState("Diff not fetched", "Fetch this pull request diff to inspect it in Tycho.")}`;
  if (!diff || diff.loading || loading) return header;
...
function renderProjectDiffContent(key, scope = "worktree", options = {}) {
  ...
  return `
    ...
    <section class="diff-viewer" aria-label="Git diff">
        ${files.length ? files.map((file, index) => renderProjectDiffFile(file, index)).join("") : emptyState("No changes", "This diff scope has no file changes.")}
      </section>
    `;
}
...
function renderProjectDiffFile(file, index, options = {}) {
  ...
  return `
    <details class="diff-file detail-card" ${index === 0 || openAll ? "open" : ""} data-state-key="diff-file:${escapeAttr(path)}">
      <summary>
  ...
  return `
    <article class="${escapeAttr(messageClassName(block))}">
      <div class="message-header">
        ...
        ${renderBlockMenuAnchor(block, menuId)}
      </div>
...
function renderMessageContent(block, options = {}) {
  const content = String(block.content || "");
  if (block?.kind === "run_summary") return renderRunSummaryMessageContent(content, options.agent);
...
function renderRunSummaryMessageContent(content, agent) {
  const preview = truncate(content.replace(/\s+/g, " ").trim(), 360);
  const agentKey = agent?.key || "";
  const openButton = agentKey
    ? `<button class="inline-icon-button summary-message-open" type="button" data-open-agent-summary="${escapeAttr(agentKey)}">${iconSvg("maximize2")}<span>Open full summary</span></button>`
    : "";
  return `
...
function renderAttachment(attachment) {
  ...
  const actions = `
    <span class="attachment-actions">
      ${download ? `<a class="icon-button" href="${escapeAttr(download)}" target="_blank" rel="noreferrer" download="${escapeAttr(title || "attachment")}" aria-label="Download attachment" title="Download attachment">${iconSvg("download")}</a>` : ""}
...
function attachmentViewerHtml(id, options = {}) {
  ...
  const download = attachmentDownloadHref(attachment);
  const downloadButton = download ? `<a class="button primary button-link" href="${escapeAttr(download)}" target="_blank" rel="noreferrer" download="${escapeAttr(title || "attachment")}">Download file</a>` : "";
  const contentHtml = attachmentContentLoading(id, attachment, format)
    ...
    : format === "binary"
      ? `<p class="field-hint">Preview unavailable for this file.</p>${downloadButton || (attachment.blob_path ? `<a class="button primary button-link" href="${escapeAttr(attachment.blob_path)}" target="_blank" rel="noreferrer">Open file</a>` : "")}`
    : format === "markdown"
      ? renderMarkdown(content)
...
function attachmentViewerActions(id, attachment) {
  ...
  const downloadAction = download
    ? `<a class="inline-icon-button" href="${escapeAttr(download)}" target="_blank" rel="noreferrer" download="${escapeAttr(String(attachment?.title || attachmentTarget(attachment) || "attachment"))}">${iconSvg("download")}<span>${downloadLabel}</span></a>`
    : "";
  ...
  const remove = id
    ...
    : "";
  if (!copy && !downloadAction && !refresh && !remove) return "";

  return `<div class="attachment-viewer-actions">${copy}${downloadAction}${refresh}${remove}</div>`;
}
