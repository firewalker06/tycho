    ...
    <div class="detail-row schedule-row">
      ...
      <div class="schedule-row-end">
        <div class="compact-actions schedule-row-actions">
          <button class="inline-icon-button schedule-run-button" type="button" data-schedule-action="run" data-schedule-key="${escapeAttr(schedule.key)}" aria-label="Run schedule now" title="Run schedule now">${iconSvg("sportShoe")}<span>Run now</span></button>
...
function renderSessionLoopSettings(configured) {
  ...
  return `
    <form id="session-loop-settings-form" class="session-loop-settings-form">
      ...
      <button class="inline-icon-button" type="button" data-add-session-loop-template>${iconSvg("plus")}<span>Add prompt template</span></button>
      <div class="response-style-form-footer session-loop-settings-footer">
        ...
        <div class="response-style-form-actions ds-form-actions session-loop-settings-actions">
          <button class="primary inline-icon-button" type="submit">${iconSvg("check")}<span>Save loop settings</span></button>
        </div>
      </div>
    </form>
  `;
  ...
  return `
    <section class="session-loop-template-row" data-session-loop-template-row>
      ...
      <div class="session-loop-template-heading">
        ...
        <button type="button" class="icon-button" data-remove-session-loop-template aria-label="Remove prompt template" title="Remove prompt template">${iconSvg("trash2")}</button>
      </div>
...
function renderAgentComposer(agent, skills, options = {}) {
  ...
  const composer = `
    <form id="composer" class="composer${fullScreen ? " composer-full-screen" : ""}" data-agent-key="${escapeAttr(agent.key)}" data-agent-running="${agentIsRunning(agent) ? "true" : "false"}"${modalAttributes}>
      ${fullScreen ? `<button class="icon-button composer-close-button" type="button" data-toggle-composer-full-screen="${escapeAttr(agent.key)}" aria-label="Close full-screen editor" title="Close full-screen editor">${iconSvg("x")}</button>` : ""}
...
function renderAgentLoopForm(key) {
  ...
  replaceView(`
    <form id="agent-loop-form" class="agent-form ds-form-layout agent-loop-form" data-agent-key="${escapeAttr(agent.key)}">
      ...
      <section class="notice">
        <strong>${escapeHtml(agent.name || agent.key)}</strong>
        <p>The first check starts immediately in this session. Later checks reuse it as a normal Tycho schedule and stop at the selected end time.</p>
      </section>
      ...
      <div class="button-row form-actions ds-form-actions">
        ...
        <button class="primary inline-icon-button" type="submit">${iconSvg("rotateCcw")}<span>Start loop now</span></button>
      </div>
    </form>
  `);
}
...
function renderUnreadAgentsPanel(agents = quickSwitchAgents(), unreadCount = unreadAgents().length) {
  ...
  els.unreadPanel.innerHTML = `
    ...
    <form class="unread-panel-search" role="search" data-unread-agent-search-form>
      ...
      <button class="icon-button ${state.unreadPanelQuery ? "" : "hidden"}" type="button" data-clear-unread-agent-search aria-label="Clear agent search" title="Clear agent search">${iconSvg("x")}</button>
    </form>
