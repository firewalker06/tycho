...
function renderPromptQueueEntry(agent, entry, index) {
  ...
  const stateLabel = entry.state === "failed" ? "Dispatch failed" : entry.state === "dispatching" ? "Dispatching" : "Queued";
...
function agentComposerAction(agent, sending = false) {
  if (agentIsRunning(agent)) {
    return `
      <button class="secondary ui-button" type="button" data-agent-action="stop" data-agent-key="${escapeAttr(agent.key)}">Stop</button>
      <button class="primary ui-button" data-variant="brand" type="submit" data-agent-key="${escapeAttr(agent.key)}" ${sending ? "disabled" : ""}>Queue</button>
    `;
  }
...
function agentComposerActionState(agent, sending = false) {
  if (agentIsRunning(agent)) return sending ? "queueing" : "running";
...
els.view.addEventListener("submit", (event) => {
  ...
  if (event.target.id === "composer") {
    ...
    const queueing = agentIsRunning(findAgent(key));
