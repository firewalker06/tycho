...
function renderAgentReference(reference) {
  ...
  const content = `<span class="status-mark ${escapeAttr(statusClass({ status: reference.status || reference.state }))}" aria-hidden="true">${iconSvg("botMessageSquare")}</span><span class="agent-reference-copy"><strong>${escapeHtml(label)}</strong><span>${escapeHtml(status)} · ${escapeHtml(serverDisplayName(resourceServer(reference.server_key)))}</span></span>`;
  const unavailable = ["missing", "hidden"].includes(reference.state) || serverDisconnected(resourceServer(reference.server_key));
  if (unavailable) return `<span class="agent-reference unavailable" aria-label="${escapeAttr(`${label}, ${stateLabel}`)}">${content}</span>`;
  const href = routeHash({ type: "agent", key: reference.key });
  return `<a class="agent-reference" href="${escapeAttr(href)}" data-open-agent="${escapeAttr(reference.key)}" aria-label="Open ${escapeAttr(label)} on ${escapeAttr(serverDisplayName(resourceServer(reference.server_key)))}">${content}</a>`;
}
