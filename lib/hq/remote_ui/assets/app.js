...
function renderPersonalAssistant() {
  ...
  const settings = `<details id="personal-assistant-settings-menu" class="pa-settings" data-overlay-menu data-overlay-key="personal-assistant-settings"><summary aria-label="Settings actions" title="Settings actions" aria-haspopup="menu">${iconSvg("ellipsis")}<span class="sr-only">Settings actions</span></summary><div class="pa-settings-menu ui-overlay-surface" data-overlay-kind="menu"><div class="more-menu" role="menu" aria-label="Settings actions">${moreMenuButton({ label: "Settings", sublabel: "Open app settings", icon: "settings", attrs: "data-open-settings" })}</div><div class="pa-settings-details"><span>${escapeHtml(item.config?.timezone || "IANA timezone")}</span><span>${escapeHtml(item.config?.model || "Codex model")}</span><span>${escapeHtml(item.config?.reasoning_effort || "Reasoning effort")}</span><p>Tycho keeps this daily conversation and its continuity automatically.</p></div></div></details>`;
