...
function renderPullRequestLineSelection(agent, item, selection) {
  ...
  return `
    <section class="pr-context-selection-bar" aria-live="polite" data-pr-context-selection>
      ...
      ${pending ? `<button class="inline-icon-button ui-button" type="button" data-open-pr-context-composer="${escapeAttr(agent.key)}">${iconSvg("botMessageSquare")}<span>Review combined comment</span></button>` : ""}
    </section>
  `;
}
...
function renderDiffHunk(hunk, index, options = {}) {
  ...
  const finalSelectedLineIndex = selectedLines.length
    ...
    : -1;
  const renderLines = (items, offset = 0) => items
    .map((line, lineIndex) => renderDiffLine(line, lineIndex + offset, { ...options, hunkIndex: index }))
    .join("");
  const beforeSelection = finalSelectedLineIndex >= 0 ? lines.slice(0, finalSelectedLineIndex + 1) : lines;
  const afterSelection = finalSelectedLineIndex >= 0 ? lines.slice(finalSelectedLineIndex + 1) : [];
  return `
    <section class="diff-hunk" aria-label="Hunk ${index + 1}">
      <div class="diff-hunk-header"><code>${escapeHtml(hunk.header || "@@")}</code></div>
      <div class="diff-lines">${renderLines(beforeSelection)}</div>
      ${finalSelectedLineIndex >= 0 ? renderPullRequestInlineComment(options.pullRequestContext, options.path, index) : ""}
      ${afterSelection.length ? `<div class="diff-lines">${renderLines(afterSelection, finalSelectedLineIndex + 1)}</div>` : ""}
    </section>
  `;
}
...
function selectPullRequestDiffLine(target, extendRange = false) {
  ...
  if (selected.lines.length) {
    window.requestAnimationFrame(() => document.querySelector("[data-pr-context-comment]")?.focus({ preventScroll: true }));
  }
}
