...
async function ensureRouteData(options = {}) {
  ...
  if (route.type === "personalAssistant" && state.personalAssistant?.active_key) {
    await ensureAgentDetail(state.personalAssistant.active_key, options.forceAgent || options.force);
    if (state.agentDetails[state.personalAssistant.active_key]) upsertAgent(state.agentDetails[state.personalAssistant.active_key]);
    const agent = findAgent(state.personalAssistant.active_key);
...
function renderPersonalAssistant() {
  ...
  const agent = item.active_key ? findAgent(item.active_key) : null;
