...
function renderPersonalAssistant() {
  ...
  setHeader("Personal Assistant", "Daily Tycho conversation", "A");
  const phaseCopy = { closing: "Closing the daily conversation. New prompts are paused.", summarizing: "Creating the continuity handoff.", archiving: "Archiving the completed daily conversation." }[item.state] || "";
  const continuity = item.handoff_path ? `<code>${escapeHtml(item.handoff_path)}</code>` : "A bounded continuity handoff appears after the daily summary.";
  const proposalCards = proposals.map((proposal) => `<article class="detail-card pa-proposal" data-proposal-id="${escapeAttr(proposal.id)}"><div class="detail-card-body"><strong>${escapeHtml(proposal.type)}</strong><p>${escapeHtml(proposal.description)}</p><pre>${escapeHtml(JSON.stringify({ type: proposal.type, arguments: proposal.arguments }, null, 2))}</pre><p>${escapeHtml(proposal.state)}</p>${proposal.state === "awaiting_confirmation" ? `<button class="ui-button" data-variant="brand" type="button" data-confirm-pa-proposal="${escapeAttr(proposal.id)}">Confirm exact action</button><button class="ui-button" type="button" data-reject-pa-proposal="${escapeAttr(proposal.id)}">Reject</button>` : ""}</div></article>`).join("");
  const agent = item.active_key ? findAgent(item.active_key) : null;
  const blocks = agent ? (state.conversations[agent.key]?.blocks || []) : [];
  const protectedConversation = agent && item.state === "active" ? `<section class="personal-assistant-conversation"><h2>Today’s conversation</h2>${renderAgentConversationView(agent, blocks, { floatingActions: false, loading: conversationLoading(agent.key) })}<section class="agent-dock" data-agent-dock>${renderAgentComposerPlacement(agent, [], {}) .dock}</section></section>` : "";
  const body = !item.configured ? `<form id="personal-assistant-setup-form" class="detail-card"><div class="detail-card-body"><p class="eyebrow">Daily conversation</p><h2>Set up Personal Assistant</h2><p>${escapeHtml(item.introduction)}</p><label>Codex model<input name="model" required value="gpt-5.6-sol"></label><label>Reasoning effort<select name="reasoning_effort"><option>low</option><option selected>medium</option><option>high</option></select></label><label>IANA timezone<input name="timezone" required value="Asia/Jakarta" placeholder="Asia/Jakarta"></label><label><input type="checkbox" name="confirmed" required> I confirm Tycho may save this Personal Assistant setup.</label><button class="ui-button" data-variant="brand" type="submit">Save exact setup</button></div></form>` : `<section class="detail-card"><div class="detail-card-body"><p class="eyebrow">${escapeHtml(item.state)}</p><h2>${item.state === "active" ? "Protected conversation is active" : "Open today's conversation"}</h2><p>${escapeHtml(item.introduction)}</p><div class="kv-grid">${copyableKv("Model", item.config?.model)}${copyableKv("Reasoning", item.config?.reasoning_effort)}${copyableKv("Timezone", item.config?.timezone)}${copyableKv("Continuity", item.active_date || "Dormant")}</div>${item.error ? feedbackMessage("Personal Assistant needs attention", item.error, { intent: "danger" }) : ""}${phaseCopy ? `<p>${escapeHtml(phaseCopy)}</p>` : ""}${item.state === "active" ? `<p>Tycho controls closing, summarizing, and archiving. Manual and bulk archive controls are unavailable.</p>` : item.state === "error" ? `<button class="ui-button" type="button" data-open-personal-assistant>Retry opening today’s conversation</button>` : `<button class="ui-button" data-variant="brand" type="button" data-open-personal-assistant>Open today's conversation</button>`}<p>${continuity}</p></div></section>${protectedConversation}`;
  replaceView(`<section class="personal-assistant-page" data-personal-assistant-state="${escapeAttr(item.state)}">${body}<section aria-label="Action proposals"><h2>Action proposals</h2>${proposalCards || "<p>No proposed actions.</p>"}</section></section>`);
}
...
function handleViewClick(event) {
  ...
  if (confirmPa) {
    mutate(async () => { await apiPost(`/personal-assistant/actions/${encodeURIComponent(confirmPa.dataset.confirmPaProposal)}/confirm`, { confirmed: true }); state.personalAssistantProposals = (await apiGet("/personal-assistant/actions")).proposals || []; showGrowl("Action confirmed", "done"); });
    return;
  }
  const rejectPa = event.target.closest("[data-reject-pa-proposal]");
  if (rejectPa) {
    mutate(async () => { await apiPost(`/personal-assistant/actions/${encodeURIComponent(rejectPa.dataset.rejectPaProposal)}/reject`, {}); state.personalAssistantProposals = (await apiGet("/personal-assistant/actions")).proposals || []; showGrowl("Action rejected", "done"); });
    return;
  }
