...
function renderScheduleForm(route) {
  ...
  replaceView(`
    <form id="schedule-form" class="agent-form schedule-form" data-mode="${editing ? "edit" : "create"}" data-schedule-key="${escapeAttr(schedule?.key || "")}">
      ...
      <section class="field-card">
        <label class="field-label" for="schedule-ends-at">Ends at</label>
        <input id="schedule-ends-at" name="ends_at" type="datetime-local" value="${escapeAttr(values.endsAt)}">
        <span class="field-hint">Optional. Loop schedules stop automatically after this time.</span>
      </section>
    ...
    return `
      <div class="response-style-summary" data-testid="response-style-summary">
        ...
        <div class="response-style-summary-actions">
          ...
          ${responseStyle.exists ? `<button class="danger icon-button" type="button" data-delete-response-style data-testid="response-style-delete" aria-label="Remove response style" title="Remove response style">${iconSvg("trash2")}</button>` : ""}
        </div>
      </div>
    `;
  }
...
function renderAgentDetailViewMenu(agent, kind) {
  ...
  return `
    <div class="focused-view-toolbar">
      <details class="focused-view-menu" data-focused-view-menu data-active-view-mode="${escapeAttr(activeMode)}" data-state-key="focused-view-menu:${escapeAttr(key)}">
        <summary aria-label="View layout: ${escapeAttr(active.label)}" title="View layout: ${escapeAttr(active.label)}" data-view-icon="${escapeAttr(active.icon)}">
          ${iconSvg(active.icon)}
          <span class="sr-only">${escapeHtml(active.label)}</span>
        </summary>
        <div class="focused-view-menu-popover">
...
function renderInquiryForm(agent, options = {}) {
  ...
  const modalAttributes = fullScreen
    ...
    : "";
  return `
    <form id="inquiry-form" class="inquiry-form${fullScreen ? " inquiry-form-full-screen" : ""}" data-agent-key="${escapeAttr(agent.key)}" data-inquiry-id="${escapeAttr(inquiryId)}" novalidate${modalAttributes}>
      ${fullScreen
        ? `<button class="icon-button inquiry-close-button" type="button" data-toggle-inquiry-full-screen="${escapeAttr(agent.key)}" aria-label="Close full-screen inquiry" title="Close full-screen inquiry">${iconSvg("x")}</button>`
        : `<button class="inquiry-full-screen-button" type="button" data-toggle-inquiry-full-screen="${escapeAttr(agent.key)}" aria-label="Open full-screen inquiry" title="Full-screen inquiry">${iconSvg("maximize2")}</button>`}
...
function renderInquiryFeedbackField() {
  return `
    <section class="field-card inquiry-field inquiry-feedback-field" data-inquiry-feedback-field>
      <label for="inquiry-feedback"><span class="inquiry-field-label">Leave feedback</span> <span class="inquiry-requirement">Optional</span></label>
      <textarea id="inquiry-feedback" name="feedback" rows="3" data-inquiry-feedback placeholder="Add anything else you want the agent to know."></textarea>
    </section>
  `;
}
...
function renderInquiryField(field, index) {
  ...
  return `
    <section class="field-card inquiry-field" data-inquiry-field="${escapeAttr(key)}">
      <label for="${escapeAttr(id)}"><span class="inquiry-field-label">${escapeHtml(field.label || key)}</span> <span class="inquiry-requirement${field.required ? " required" : ""}">${escapeHtml(requirement)}</span></label>
      ${field.description ? `<span class="field-hint">${escapeHtml(field.description)}</span>` : ""}
      ${input}
      <span class="inquiry-field-error" data-inquiry-field-error role="status"></span>
    </section>
  `;
}
...
function renderAgentForm(route) {
  ...
  replaceView(`
    <form id="agent-form" class="agent-form" data-mode="${editing ? "edit" : cloning ? "clone" : "create"}" data-project-key="${escapeAttr(project.key)}" data-agent-key="${escapeAttr(agent?.key || "")}" data-project-name="${escapeAttr(project.name || project.key)}">
      ...
      <section class="field-card">
        <label class="field-label" for="agent-template">Prompt Template</label>
      ...
      <section class="field-card">
        <label class="field-label" for="agent-response-style">Response style</label>
        <select id="agent-response-style" name="response_style_mode" data-agent-response-style-select>
          ${agentResponseStyleChoiceOptions(templateResponseStyleSource(selectedTemplate), values.responseStyleMode)}
        </select>
        <span class="field-hint">Independent from Prompt Template. Global uses the active policy from Settings.</span>
      </section>
      <section class="field-card">
...
function renderAgentRow(agent) {
  ...
  return `
    <button class="agent-row" type="button" data-open-agent="${escapeAttr(agent.key)}">
      <span class="status-mark ${scheduledAgentIconStatusClass(agent)}" aria-hidden="true">${iconSvg(agentIconName(agent))}</span>
      <div class="row-title">
        <strong>${escapeHtml(agent.name || agent.key)}</strong>
...
function attachmentViewerActions(id, attachment) {
  ...
  return `
    <div class="attachment-viewer-actions">
      <details class="attachment-viewer-menu" data-attachment-viewer-menu data-state-key="attachment-viewer-menu:${escapeAttr(id || target)}">
        <summary aria-label="Attachment actions" title="Attachment actions">${iconSvg("ellipsis")}</summary>
        <div class="attachment-viewer-menu-popover">
          ${moreMenuHtml(items)}
        </div>
      </details>
    </div>
...
function renderAgentScheduleMenu(agent, schedule) {
  ...
  return `
    <details class="header-schedule-control ${scheduledAgentIconStatusClass(agent, schedule)}" data-header-schedule-menu data-agent-key="${escapeAttr(agent.key)}">
      <summary aria-label="Schedule actions" title="Schedule actions">
        ${iconSvg("calendarCheck2")}
      </summary>
      <div class="header-schedule-popover" role="menu" aria-label="Schedule actions for ${escapeAttr(scheduleName)}">
        ${moreMenuHtml(items)}
      </div>
    </details>
  `;
...
function closeHeaderScheduleMenu() {
  els.headerSchedule.querySelector("[data-header-schedule-menu]")?.removeAttribute("open");
}
