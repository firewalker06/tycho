...
function personalAssistantPreviewValue(value) {
  ...
  if (typeof value === "object") {
    return Object.entries(value)
      ...
      .map(([key, item]) => `${titleFromKey(key)}: ${personalAssistantPreviewValue(item)}`)
      .join(" · ");
  }
  return String(value);
}
...
function personalAssistantPreviewRows(proposal, preflight) {
  ...
  if (Object.hasOwn(details, "before") || Object.hasOwn(details, "after")) {
    rows.push(["Project before", personalAssistantPreviewValue(details.before)]);
    rows.push(["Project after", personalAssistantPreviewValue(details.after)]);
  }
...
function renderPersonalAssistantProposal(proposal) {
  ...
  const previewErrorHtml = preflightError
    ? `<p class="pa-control-error" role="alert">Could not load the exact server preview: ${escapeHtml(preflightError)} Confirmation is unavailable until the preview can be verified.</p>`
    : "";
...
function personalAssistantExpiredProposals(item, proposals = []) {
  ...
  const candidates = [
    ...(Array.isArray(item?.expired_proposals) ? item.expired_proposals : []),
    ...(Array.isArray(item?.expired_actions) ? item.expired_actions : []),
    ...(Array.isArray(item?.continuity?.expired_proposals) ? item.continuity.expired_proposals : []),
    ...(Array.isArray(item?.continuity?.expired_actions) ? item.continuity.expired_actions : []),
    ...historyEntries.flatMap((entry) => Array.isArray(entry?.expired_proposals) ? entry.expired_proposals : []),
    ...historyEntries.flatMap((entry) => Array.isArray(entry?.expired_actions) ? entry.expired_actions : []),
    ...(Array.isArray(state.personalAssistantHistoryEntry?.expired_proposals) ? state.personalAssistantHistoryEntry.expired_proposals : []),
    ...(Array.isArray(state.personalAssistantHistoryEntry?.expired_actions) ? state.personalAssistantHistoryEntry.expired_actions : []),
  ];
  const activeExpired = (Array.isArray(proposals) ? proposals : []).filter((proposal) => personalAssistantProposalExpired(proposal, item));
  const seen = new Set();
  return [...activeExpired, ...candidates]
    .map((proposal) => proposal?.proposal && typeof proposal.proposal === "object" ? proposal.proposal : proposal)
    ...
    .map((proposal) => ({ ...proposal, state: "expired", expired: true }));
}
...
function renderPersonalAssistantContinuity(item, proposals) {
  ...
  const historyNotice = state.personalAssistantHistoryLoading
    ? `<p class="pa-history-notice" role="status">Loading the saved summary…</p>`
    : state.personalAssistantHistoryError
  ...
  const expiredHtml = expired.length ? `<div class="pa-handoff-section pa-expired-proposals" data-pa-expired-proposals><strong>Expired proposals</strong><p>These pending proposals belong to an earlier conversation and cannot be approved here.</p><ul>${expired.map((proposal) => `<li data-pa-expired-proposal-id="${escapeAttr(proposal.id)}">${escapeHtml(personalAssistantActionCopy(proposal).label)} · ${escapeHtml(String(personalAssistantActionCopy(proposal).target))} — ${escapeHtml(personalAssistantProposalInstruction(proposal))}</li>`).join("")}</ul></div>` : "";
...
async function runPersonalAssistantAction(id, operation) {
  ...
  try {
    ...
    if (!currentRequest()) return;
    state.personalAssistantActionErrors[id] = personalAssistantActionMutationError(error);
    const code = String(error?.code || error?.details?.code || error?.data?.details?.code || "").toLowerCase();
...
function handleViewClick(event) {
  ...
  if (historyButton) {
    const id = historyButton.dataset.openPaHistory;
    const routeHashAtStart = location.hash;
    const context = personalAssistantSessionContext();
    ...
    apiGet(`/personal-assistant/history/${encodeURIComponent(id)}`).then((response) => {
      if (location.hash !== routeHashAtStart || parseRoute().type !== "personalAssistant" || !personalAssistantSessionMatches(context)) return;
      state.personalAssistantHistoryEntry = response.history || null;
    }).catch((error) => {
      if (location.hash === routeHashAtStart && parseRoute().type === "personalAssistant" && personalAssistantSessionMatches(context)) state.personalAssistantHistoryError = error.message;
    }).finally(() => {
      if (location.hash !== routeHashAtStart || parseRoute().type !== "personalAssistant" || !personalAssistantSessionMatches(context)) return;
      state.personalAssistantHistoryLoading = false;
      state.renderedViewHtml = "";
