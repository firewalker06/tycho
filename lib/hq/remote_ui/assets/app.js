...
function renderPersonalAssistantSettings(assistant) {
  ...
  return `<section class="detail-card" id="settings-personal-assistant" data-personal-assistant-state="${escapeAttr(item.state)}">
    <div class="detail-card-body" style="padding-top: 12px;">
      ...
      <p class="field-hint">${escapeHtml(item.introduction || "The assistant starts with Tycho's introduction and examples before any model run.")}</p>
      <div class="kv-grid">
      ...
      ${item.configured ? `<button class="ui-button" type="button" data-open-personal-assistant ${active ? "disabled" : ""}>${active ? "Conversation active" : "Open FRED"}</button>` : `<p class="field-hint">Setup is available through the Personal Assistant API and requires an exact confirmation before Tycho saves it.</p>`}
      ${item.configured ? `<button class="ui-button" data-variant="danger" type="button" data-reset-personal-assistant>Reset FRED</button><p class="field-hint">Stops and archives any active FRED session, then clears FRED setup, continuity, and pending actions.</p>` : ""}
      ${active ? `<p class="field-hint">Archive controls are intentionally unavailable. Tycho closes and archives this session after the daily handoff.</p>` : ""}
    </div>
  </section>`;
}
...
function personalAssistantActionCopy(proposal) {
  ...
  const target = args.agent_key || args.project_key || args.path || args.query || args.name || "Tycho";
  const label = {
    install_or_update_tycho_skill: "Install or update skill", create_agent: "Create agent", message_agent: "Message agent",
    start_agent: "Start agent", stop_agent: "Stop agent",
  ...
  const effect = {
    ...
    stop_agent: "Stops the running agent.",
...
function renderPersonalAssistantProposal(proposal) {
  ...
  const receipt = proposal.state === "executed" ? "Action completed." : proposal.state === "rejected" ? "Action rejected." : proposal.state === "failed" ? `Action could not run: ${proposal.error || "Unknown error"}` : "";
  return `<article class="pa-inline-action ${pending ? "pending" : "receipt"}" data-proposal-id="${escapeAttr(proposal.id)}" data-proposal-run="${escapeAttr(proposal.source_run_id || "")}" data-proposal-state="${escapeAttr(proposal.state)}">
    <span class="pa-action-label">${pending ? "Action ready" : "Action receipt"}</span>
    <dl><div><dt>Action</dt><dd>${escapeHtml(copy.label)}</dd></div><div><dt>Target</dt><dd>${escapeHtml(String(copy.target))}</dd></div><div><dt>Effect</dt><dd>${escapeHtml(copy.effect)}</dd></div></dl>
    ${pending ? `<div class="pa-action-controls"><button class="inline-icon-button ui-button" data-variant="brand" type="button" data-confirm-pa-proposal="${escapeAttr(proposal.id)}" aria-label="Accept proposed action" ${busy ? "disabled" : ""}>${iconSvg("thumbsUp")}<span>${busy ? "Accepting…" : "Accept"}</span></button><button class="inline-icon-button ui-button" type="button" data-reject-pa-proposal="${escapeAttr(proposal.id)}" aria-label="Reject proposed action" ${busy ? "disabled" : ""}>${iconSvg("thumbsDown")}<span>Reject</span></button></div>` : `<p class="pa-action-receipt" role="status">${escapeHtml(receipt)}</p>`}
...
function renderPersonalAssistantWelcome() {
  return `<article class="pa-welcome"><strong>What would you like to move forward?</strong><div class="pa-suggestions"><button type="button" data-pa-suggestion="Review priorities">Review priorities</button><button type="button" data-pa-suggestion="Check agents">Check agents</button><button type="button" data-pa-suggestion="Plan this project">Plan this project</button></div></article>`;
}
...
function renderPersonalAssistantSetup(item) {
  ...
  return `<section class="pa-setup ui-surface" aria-labelledby="pa-setup-title"><span class="section-label">FRED setup</span><h2 id="pa-setup-title">Choose how FRED should work</h2><p id="pa-setup-guidance">Review the Codex model, reasoning effort, and IANA timezone before saving. Opening FRED creates today’s conversation; your first message stays yours.</p><form id="personal-assistant-setup-form" class="ui-form-layout" aria-describedby="pa-setup-guidance"><div class="ui-field"><label class="ui-field__label" for="pa-model">Codex model</label><input class="ui-input" id="pa-model" name="model" required value="${escapeAttr(config.model || "gpt-5.6-sol")}" autocomplete="off"></div><div class="ui-field"><label class="ui-field__label" for="pa-reasoning-effort">Reasoning effort</label><input class="ui-input" id="pa-reasoning-effort" name="reasoning_effort" required value="${escapeAttr(config.reasoning_effort || "medium")}" autocomplete="off"></div><div class="ui-field"><label class="ui-field__label" for="pa-timezone">IANA timezone</label><input class="ui-input" id="pa-timezone" name="timezone" required value="${escapeAttr(config.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC")}" placeholder="Asia/Jakarta" autocomplete="off"></div><label class="pa-confirmation"><input name="confirmed" type="checkbox" required> I confirm these settings for FRED.</label><button class="ui-button" data-variant="brand" type="submit">Save FRED setup</button></form></section>`;
}

function renderPersonalAssistantReady(item) {
  return `<section class="pa-ready ui-surface"><span class="section-label">FRED is ready</span><h2>Open today’s conversation when you’re ready.</h2><p>Opening FRED does not send a model prompt. Its introduction and examples are available before you write your first message.</p><button class="ui-button" data-variant="brand" type="button" data-open-personal-assistant>Open FRED</button></section>`;
}
...
function renderPersonalAssistant() {
  ...
  const inlineActions = proposals.map(renderPersonalAssistantProposal).join("");
  const thread = agent && item.state === "active"
    ? `<div class="pa-conversation-scroll" data-preserve-scroll data-state-key="personal-assistant-thread">${starter ? renderPersonalAssistantWelcome() : renderAgentConversationView(agent, blocks, { floatingActions: false, loading: conversationLoading(agent.key), personalAssistant: true })}${inlineActions}</div>`
    : `<div class="pa-conversation-scroll" data-preserve-scroll data-state-key="personal-assistant-thread">${item.configured ? renderPersonalAssistantReady(item) : renderPersonalAssistantSetup(item)}${item.error ? feedbackMessage("Sending is paused", item.error, { intent: "danger" }) : ""}${blocked ? `<p class="pa-inline-notice">${escapeHtml({ closing: "Finishing today’s conversation. Sending is paused.", summarizing: "Saving today’s continuity. Sending is paused.", archiving: "Closing today’s conversation. Sending is paused." }[item.state] || "FRED is unavailable right now.")}</p>` : ""}</div>`;
...
function renderAgentConversationView(agent, blocks, options = {}) {
  ...
  return `
    ...
    <div class="message-list">
      ...
      ${conversationBlocks.length ? renderConversationBlocks(conversationBlocks, { agent, personalAssistant: options.personalAssistant }) : emptyHtml}
    </div>
...
els.headerMorePanel.addEventListener("click", (event) => {
  ...
  if (event.target.closest("[data-reset-personal-assistant]")) {
    confirmAction({
      title: "Reset FRED?",
      description: "This stops and archives any active FRED session, then clears FRED setup, daily continuity, and pending actions. Archived session logs remain available.",
...
function handleViewClick(event) {
  ...
  if (confirmPa) {
    ...
    mutate(async () => { await apiPost(`/personal-assistant/actions/${encodeURIComponent(id)}/confirm`, { confirmed: true }); state.personalAssistantProposals = (await apiGet("/personal-assistant/actions")).proposals || []; showGrowl("Action confirmed", "done"); }).finally(() => { state.pendingPersonalAssistantProposalIds.delete(id); state.renderedViewHtml = ""; render(); });
    return;
  }
...
els.view.addEventListener("submit", (event) => {
  event.preventDefault();
  if (event.target.id === "personal-assistant-setup-form") {
    const form = event.target;
    const values = new FormData(form);
    mutate(async () => { const response = await apiPost("/personal-assistant/setup", { model: String(values.get("model") || ""), reasoning_effort: String(values.get("reasoning_effort") || ""), timezone: String(values.get("timezone") || ""), confirmed: values.get("confirmed") === "on" }); state.personalAssistant = response.personal_assistant; showGrowl("FRED configured", "done"); }, { form });
    return;
  }
