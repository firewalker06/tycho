...
const state = {
  ...
  personalAssistantCurrentWorkState: "unknown",
  personalAssistantCurrentWorkError: "",
...
function personalAssistantPollDelay() {
  ...
  const currentWorkActive = state.personalAssistantCurrentWork?.agents?.some((entry) =>
    ["running", "executing"].includes(String(entry?.status || entry?.state || "").toLowerCase()));
  const active = Boolean(agent?.running || proposalWork || queueWork || currentWorkActive);
...
function applyPersonalAssistantItem(item) {
  ...
  if (previousContext && !personalAssistantSessionMatches(previousContext)) {
    ...
    state.personalAssistantCurrentWork = null;
    state.personalAssistantCurrentWorkState = "unknown";
...
function requestPersonalAssistantCurrentWork(routeHashAtStart = location.hash, context = null) {
  const requests = state.personalAssistantRequests.currentWork;
  const requestKey = [context?.server_key || personalAssistantServerKey(), context?.active_key || "no-session", context?.generation ?? "no-generation"].join(":");
  ...
  return personalAssistantRequest(requests, requestKey, async () => {
    ...
    const current = location.hash === routeHashAtStart && parseRoute().type === "personalAssistant" &&
      (!context || personalAssistantSessionMatches(context));
    if (!current || sequence < coordinator.currentWorkApplied) return data;
    coordinator.currentWorkApplied = sequence;
...
async function refreshPersonalAssistant(options = {}) {
  ...
  try {
    ...
    const work = [
      ...
      item?.active_key ? requestPersonalAssistantCurrentWork(routeHashAtStart, context) : null,
...
function personalAssistantProjectPickerValue(project) {
  const key = String(project?.key || "").trim();
  const name = String(project?.name || key).trim();
  return name === key ? key : `${name} (${key})`;
}

function personalAssistantProjectForPicker(picker) {
  const value = String(picker?.value || "").trim().toLowerCase();
  if (!value) return null;
  return (state.projects || []).find((project) => [project?.key, project?.name, personalAssistantProjectPickerValue(project)]
    .some((candidate) => String(candidate || "").trim().toLowerCase() === value)) || null;
}

function renderPersonalAssistantWelcome() {
  const projects = state.projects || [];
  const projectOptions = projects.map((project) => `<option value="${escapeAttr(personalAssistantProjectPickerValue(project))}"></option>`).join("");
  const projectStarter = projects.length
    ? `<li class="pa-project-starter"><div class="pa-starter-copy"><strong>Prepare an agent</strong><span>Choose a project, then describe the work you want done.</span></div><form id="pa-project-starter-form" class="pa-project-starter-form"><label class="sr-only" for="pa-project-picker">Project</label><input class="ui-input" id="pa-project-picker" data-pa-project-picker list="pa-project-options" placeholder="Find a project by name or key" required><datalist id="pa-project-options">${projectOptions}</datalist><button class="ui-button" data-variant="brand" type="submit">Prepare</button></form></li>`
    : '<li><button class="pa-suggestion" type="button" data-pa-suggestion="Set up my first project on this Tycho server"><span class="pa-starter-copy"><strong>Set up my first project</strong><span>Choose a workspace and the coding agent you want to use.</span></span></button></li>';
  return `<article class="pa-welcome"><div class="pa-welcome-heading"><h2>What would you like to do?</h2><p>FRED works on this Tycho server and asks before making changes.</p></div><ul class="pa-suggestions" aria-label="Ways to start with FRED"><li><button class="pa-suggestion" type="button" data-pa-suggestion="Show running agents and their status"><span class="pa-starter-copy"><strong>Show running agents</strong><span>See what is active, waiting, or finished.</span></span></button></li><li><button class="pa-suggestion" type="button" data-pa-suggestion="Explain a Tycho feature and how to use it"><span class="pa-starter-copy"><strong>Explain a Tycho feature</strong><span>Get a practical walkthrough for this server.</span></span></button></li>${projectStarter}</ul><section class="pa-capabilities" aria-labelledby="pa-capabilities-title"><h3 id="pa-capabilities-title">What FRED can do</h3><ul><li><strong>Explain Tycho</strong><span>Read and search its documentation, then answer questions in context.</span></li><li><strong>Inspect active work</strong><span>Review projects, agents, schedules, latest runs, and bounded logs.</span></li><li><strong>Prepare changes</strong><span>Draft projects, agents, and schedules with exact settings for your review.</span></li><li><strong>Operate agents deliberately</strong><span>Create agents without running them, then separately start, message, or stop them.</span></li><li><strong>Maintain the Tycho skill</strong><span>Prepare an install or update for your confirmation.</span></li></ul></section></article>`;
}
...
function personalAssistantCurrentWorkStateLabel(work, stateName = null) {
  stateName = String(stateName || state.personalAssistantCurrentWorkState || work?.state || "unknown").toLowerCase();
  if (stateName === "refreshing") return "Refreshing current work";
  if (stateName === "fresh") return "Fresh snapshot";
  if (stateName === "stale") return "Stale snapshot";
  if (stateName === "unavailable") return "Current work unavailable";
  if (stateName === "loading") return "Loading current work";
  return "Current work not loaded";
}
...
function personalAssistantCurrentWorkLink(entry, kind) {
  const rawKey = entry?.resource_key || entry?.key || entry?.agent_key || entry?.project_key || entry?.schedule_key;
  if (!rawKey) return "";
  const serverKey = entry?.server_key || personalAssistantServerKey();
  const parsed = parseResourceClientKey(rawKey);
  const key = parsed ? rawKey : resourceClientKey(serverKey, rawKey);
  if (kind === "agent") return routeHash({ type: "agent", key });
  if (kind === "project") return routeHash({ type: "project", key });
  if (kind === "schedule") return routeHash({ type: "scheduleForm", mode: "edit", key });
  return "";
}
...
function renderPersonalAssistantCurrentWorkEntry(entry, kind) {
  const key = entry?.key || entry?.agent_key || entry?.project_key || entry?.schedule_key || "Resource";
  const name = entry?.name || key;
  const status = entry?.status || entry?.state || (entry?.running ? "running" : "ready");
  const href = personalAssistantCurrentWorkLink(entry, kind);
  const label = href ? `<a href="${escapeAttr(href)}">${escapeHtml(name)}</a>` : `<span>${escapeHtml(name)}</span>`;
...
function renderPersonalAssistantCurrentWork(item) {
  ...
  const stateName = String(state.personalAssistantCurrentWorkState || work?.state || "unknown").toLowerCase();
  const error = state.personalAssistantCurrentWorkError;
  const observed = personalAssistantObservedTime(work?.observed_at);
  const freshness = work?.fresh_until && new Date(work.fresh_until).getTime() < Date.now() ? "stale" : stateName;
  const visibleEntry = (entry) => entry && entry.hidden !== true && entry.visible !== false;
  const agents = (Array.isArray(work?.agents) ? work.agents : []).filter(visibleEntry);
  const tracked = work?.tracked && typeof work.tracked === "object" ? work.tracked : {};
  const visibleAgentKeys = new Set(agents.map((entry) => String(entry?.key || entry?.agent_key || "")));
  const trackedAgents = (Array.isArray(tracked.agents) ? tracked.agents : [])
    .filter(visibleEntry)
    .filter((entry) => !visibleAgentKeys.has(String(entry?.key || entry?.agent_key || "")));
  const trackedProjects = (Array.isArray(tracked.projects) ? tracked.projects : []).filter(visibleEntry);
  const trackedSchedules = (Array.isArray(tracked.schedules) ? tracked.schedules : []).filter(visibleEntry);
  const statusCopy = error
    ? `FRED could not read current work: ${error}`
    : freshness === "unavailable"
      ? "FRED did not provide a current-work snapshot. Refresh to try again."
      : `${personalAssistantCurrentWorkStateLabel(work, freshness)} · observed ${observed}`;
  const groups = [
    ["Agents", "agent", agents],
    ["FRED-tracked agents", "agent", trackedAgents],
    ["FRED-tracked projects", "project", trackedProjects],
    ["FRED-tracked schedules", "schedule", trackedSchedules],
...
async function runPersonalAssistantAction(id, operation) {
  ...
  try {
    ...
    try {
      ...
      await requestPersonalAssistantCurrentWork(routeHashAtStart, context);
    } catch (error) {
...
els.view.addEventListener("submit", (event) => {
  event.preventDefault();
  if (event.target.id === "pa-project-starter-form") {
    const picker = event.target.querySelector("[data-pa-project-picker]");
    const project = personalAssistantProjectForPicker(picker);
    if (!project) {
      showGrowl("Choose a project from the suggestions", "need");
      picker?.focus();
      return;
    }
    const input = els.view.querySelector("#composer #prompt-input");
    if (input) {
      input.value = `Prepare an agent for ${project.name || project.key} (${project.key}). Ask me for the task before proposing changes.`;
      input.focus();
      input.dispatchEvent(new Event("input", { bubbles: true }));
    }
    return;
  ...
  if (["personal-assistant-setup-form", "personal-assistant-settings-form"].includes(event.target.id)) {
    ...
    mutate(async () => {
      const configured = await apiPost("/personal-assistant/setup", { model: String(values.get("model") || ""), reasoning_effort: String(values.get("reasoning_effort") || ""), timezone: String(values.get("timezone") || ""), confirmed: values.get("confirmed") === "on" });
