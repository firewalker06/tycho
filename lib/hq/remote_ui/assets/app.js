      ...
      ${refs.length ? `
        <section class="agent-pr-diff-layout">
          ...
          <section class="pr-diff-detail" aria-label="Pull request diff">
            ${renderPullRequestDiffDetail(agent, selected, diff, { openAll: fullView })}
          </section>
        </section>
      ` : ""}
    </section>
...
function renderPullRequestDiffToolbar(agent, data) {
  ...
  const disableButtons = Boolean(loading || isPullRequestDiffFetchingForAgent(agent.key));
  return `
    <section class="diff-toolbar pr-diff-toolbar" aria-label="Pull request diff actions">
      ...
      <button class="inline-icon-button" type="button" data-refresh-all-pr-diffs="${escapeAttr(agent.key)}" ${disableButtons ? "disabled" : ""}>${iconSvg("code")}<span>Refresh all diffs</span></button>
    </section>
  `;
}
  ...
  return `
    <a class="pr-diff-nav-item${active ? " active" : ""}" href="${escapeAttr(routeHash({ type: "agentPullRequests", key: agent.key, pullRequestId: item.id }))}" ${active ? 'aria-current="page"' : ""}>
      ...
      <span class="pr-diff-nav-copy">
        ...
        ${fetching ? `<span class="pr-diff-nav-loading" aria-hidden="true">${iconSvg("loaderPinwheel")}</span>` : ""}
...
function renderPullRequestDiffDetail(agent, item, diff, options = {}) {
  ...
  const summaryLabel = openAll ? "Collapse full view" : "Open full view";
  const header = `
    <section class="summary-card diff-summary-card pr-diff-summary">
      <div class="card-title">
        ...
        ${loading ? `<span class="pr-diff-title-loading" aria-hidden="true">${iconSvg("loaderPinwheel")}</span>` : ""}
        <span>${escapeHtml([item.repository, item.number ? `#${item.number}` : "", pullRequestFreshnessLabel(item)].filter(Boolean).join(" / "))}</span>
      </div>
      <div class="button-row">
        <button class="inline-icon-button" type="button" data-refresh-pr-diff="${escapeAttr(item.id)}" data-agent-key="${escapeAttr(agent.key)}" ${loading ? "disabled" : ""}>${iconSvg("rotateCcw")}<span>${snapshot ? "Refresh diff" : "Fetch diff"}</span></button>
        <button class="inline-icon-button" type="button" data-toggle-pr-diff-full-view="${escapeAttr(agent.key)}" ${loading ? "disabled" : ""}>${iconSvg("fileText")}<span>${escapeHtml(summaryLabel)}</span></button>
  ...
  if (!snapshot) return `${header}${emptyState(loading ? "Loading diff" : "Diff not fetched", loading ? "Reading the saved pull request diff snapshot." : "Fetch this pull request diff to inspect it in Tycho.")}`;
  if (!diff || diff.loading || loading) return `${header}${emptyState("Loading diff", "Reading the saved pull request diff snapshot.")}`;
  ...
  return `
    ...
    <section class="diff-viewer" aria-label="Pull request diff">
      ${files.length ? files.map((file, index) => renderProjectDiffFile(file, index, { openAll })).join("") : emptyState("No file changes", "This pull request diff snapshot has no textual file changes.")}
    </section>
  `;
}
