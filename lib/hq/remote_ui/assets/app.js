...
function renderAgentComposerPlacement(agent, skills, options = {}) {
  if (agent.latest_inquiry) {
    return { dock: renderInquiryForm(agent, options), overlay: "" };
  }
...
function renderAgentComposer(agent, skills, options = {}) {
  ...
  const modalAttributes = fullScreen
    ? ' role="dialog" aria-modal="true" aria-label="Full-screen editor" data-composer-modal'
    : "";
  const composer = `
    <form id="composer" class="composer${fullScreen ? " composer-full-screen" : ""}" data-agent-key="${escapeAttr(agent.key)}"${modalAttributes}>
...
function syncFullScreenComposerModal() {
  const modal = els.view.querySelector("[data-composer-modal]");
...
function handleFullScreenComposerKeydown(event) {
  ...
  if (event.key === "Escape") {
    ...
    setComposerFullScreen(composer.dataset.agentKey, false);
    return;
  }
  trapFullScreenComposerFocus(event, composer);
}
...
els.view.addEventListener("input", (event) => {
  ...
  const composer = event.target.closest("#composer");
  if (composer && draftableTextControl(event.target)) {
    queueComposerDraftSave(composer);
  }
});
