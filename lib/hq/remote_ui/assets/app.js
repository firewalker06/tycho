    ...
    <section class="summary-card diff-summary-card pr-diff-summary">
      <div class="card-title pr-diff-title-row">
        ...
        <div class="pr-diff-title-actions">
          ${loading
            ? `<span class="inline-icon-button pr-diff-refreshing" aria-label="Refreshing">${iconSvg("loaderPinwheel")}</span>`
            : `<button class="inline-icon-button" type="button" data-refresh-pr-diff="${escapeAttr(item.id)}" data-agent-key="${escapeAttr(agent.key)}">${iconSvg("rotateCcw")}<span>${snapshot ? "Refresh" : "Fetch"}</span></button>`
          }
          <button class="inline-icon-button" type="button" data-toggle-pr-diff-expand-all="${escapeAttr(agent.key)}" ${loading ? "disabled" : ""}>${iconSvg("hourglass")}<span>${expandLabel}</span></button>
