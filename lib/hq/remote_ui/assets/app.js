...
function renderPromptQueueEntry(agent, entry, index) {
  ...
  const editable = state === "queued" && entry.source !== "delegation_callback";
  const attachments = Array.isArray(entry.attachments) ? entry.attachments : [];
  const stateLabel = state === "failed" ? "Dispatch failed" :
  ...
  return `
    <li class="prompt-queue-entry" data-prompt-queue-entry="${escapeAttr(entry.id)}">
      ...
      ${entry.source === "delegation_callback" ? "" : `<div class="prompt-queue-entry-actions">
      ...
      ${entry.source === "delegation_callback" ? "" : `${editError ? `<div class="pa-control-error prompt-queue-control-error" role="alert" data-pa-control-error="queue-edit:${escapeAttr(entry.id)}">${escapeHtml(editError.message)}</div>` : ""}
