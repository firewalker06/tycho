...
async function ensureResponseStyle(force = false) {
  ...
  try {
    ...
    if (state.responseStyle?.drafting && next && !next.exists) next.drafting = true;
...
function renderResponseStyleEditor() {
  ...
  if (!responseStyle.exists && !responseStyle.drafting) {
    return `
      <div class="response-style-empty" data-testid="response-style-empty">
        <div>
          <strong>Response style</strong>
          <p>${escapeHtml(description)}</p>
          <span class="wrap-anywhere">${escapeHtml(responseStyle.path || "~/.tycho/config/response_style.md")}</span>
        </div>
        <button class="primary inline-icon-button" type="button" data-add-response-style data-testid="response-style-add">
          ${iconSvg("plus")}<span>Add response style</span>
        </button>
      </div>
    `;
  }
...
els.view.addEventListener("click", (event) => {
  ...
  if (event.target.closest("[data-add-response-style]")) {
    state.responseStyle = { ...(state.responseStyle || {}), content: "", bytes: 0, exists: false, drafting: true };
    render();
