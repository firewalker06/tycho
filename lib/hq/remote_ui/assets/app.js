      ...
      ${refs.length ? `
        <section class="agent-pr-diff-layout">
          ...
          <section class="pr-diff-detail" aria-label="Pull request diff">
            ${renderPullRequestDiffDetail(agent, selected, diff, { fullView })}
          </section>
        </section>
      ` : ""}
    </section>
...
function renderPullRequestDiffDetail(agent, item, diff, options = {}) {
  ...
  const summaryLabel = fullView ? "Exit full view" : "Full view";
  const header = `
  ...
  return `
    ...
    <section class="diff-viewer" aria-label="Pull request diff">
      ${files.length ? files.map((file, index) => renderProjectDiffFile(file, index)).join("") : emptyState("No file changes", "This pull request diff snapshot has no textual file changes.")}
    </section>
  `;
}
