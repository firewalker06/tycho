...
function renderAgentRelationshipRow(ownerAgent, reference, childKey) {
  ...
  const ownershipLabel = ownership === "takeover" ? "Takeover" : "Delegation";
  const generation = Number(reference?.ownership_generation || 1);
  ...
  const control = editable
    ...
    : "";
  const ownershipBadge = `<span class="agent-ownership-badge ${ownership}" title="Ownership generation ${escapeAttr(String(generation))}">${escapeHtml(ownershipLabel)} <span aria-label="generation ${escapeAttr(String(generation))}">G${escapeHtml(String(generation))}</span></span>`;
  return `
