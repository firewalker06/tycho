...
function renderResponseStyleEditor() {
  ...
  if (!responseStyle.drafting) {
    ...
    const active = responseStyle.exists && normalizedContent.length > 0;
    return `
      <div class="response-style-summary" data-testid="response-style-summary">
        <div>
          <div class="response-style-summary-heading">
            <strong>Response style</strong>
            ${active ? `<span class="pill done" data-testid="response-style-active">Active</span>` : ""}
          </div>
...
function agentSettingsHtml(agent) {
  const settings = [
    ["Agent key", agent.key],
    ["Effort", agent.reasoning_effort || "default"],
