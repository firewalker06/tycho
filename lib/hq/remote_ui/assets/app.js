  ...
  const header = `
    <section class="summary-card diff-summary-card pr-diff-summary">
      ...
      <div class="button-row">
        ...
        <button class="inline-icon-button" type="button" data-toggle-pr-diff-expand-all="${escapeAttr(agent.key)}" ${loading ? "disabled" : ""}>${iconSvg("code")}<span>${expandLabel}</span></button>
...
function renderBlockMenuAnchor(block, menuId) {
  ...
  const menuHtml = isOpen
    ? `<div class="block-menu-popover" role="menu">
        ${moreMenuButton({ label: "Copy text", icon: "copy", attrs: `data-copy="${escapeAttr(copyText)}"` })}
      </div>`
    : "";
  return `
    <div class="block-menu-anchor">
      <button class="block-menu-trigger icon-button${isOpen ? " active" : ""}" type="button"
