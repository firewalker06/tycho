...
function renderPullRequestDiffDetail(agent, item, diff, options = {}) {
  ...
  return `
    ...
    ${renderPullRequestLineSelection(agent, item, diff, selection)}
    <section class="diff-viewer" aria-label="Pull request diff">
      ${files.length ? files.map((file, index) => renderProjectDiffFile(file, index, {
        openAll: expandAll,
        pullRequestContext: { agentKey: agent.key, pullRequestId: item.id, snapshotId: diff.snapshot_id || "", selectedLines: selection.lines },
      })).join("") : emptyState("No file changes", "This pull request diff snapshot has no textual file changes.")}
    </section>
  `;
}

function renderPullRequestLineSelection(agent, item, diff, selection) {
  const count = selection.lines.length;
  return `
    <section class="pr-context-selection-bar${count ? " has-selection" : ""}" aria-live="polite" data-pr-context-selection>
      <span class="pr-context-selection-copy">
        ${iconSvg("paperclip")}
        <span><strong>${count ? `${count} line${count === 1 ? "" : "s"} selected` : "Attach diff context"}</strong><span>${count ? "This immutable range is ready to attach." : "Select a line, then Shift+select another line for a contiguous range."}</span></span>
      </span>
      <span class="pr-context-selection-actions">
        ${count ? `<button class="ui-button" type="button" data-clear-pr-diff-selection data-agent-key="${escapeAttr(agent.key)}" data-pull-request-id="${escapeAttr(item.id)}">Clear</button>` : ""}
        <button class="primary inline-icon-button ui-button" data-variant="brand" type="button" data-attach-pr-diff-selection data-agent-key="${escapeAttr(agent.key)}" data-pull-request-id="${escapeAttr(item.id)}" ${count ? "" : "disabled"}>${iconSvg("paperclip")}<span>Attach to conversation</span></button>
      </span>
    </section>
  `;
}
...
function renderSpeechModeButton(agent) {
  ...
  const speechState = active || ownsStatus ? state.speechState : available ? "idle" : "unsupported";
  const disabled = agentIsRunning(agent);
  const label = active ? "Stop speech recognition" : available ? "Start speech recognition" : "Speech recognition unavailable";
  const hint = `${label} (${speechModeShortcutLabel()})`;
  const status = speechModeStatus(speechState, state.speechMessage);
  return `<span class="speech-mode-control" data-speech-mode-control data-state="${escapeAttr(speechState)}">
    <button class="skill-toggle-button speech-mode-button" type="button" data-toggle-speech-mode aria-label="${escapeAttr(hint)}" title="${escapeAttr(hint)}" aria-keyshortcuts="${escapeAttr(speechModeShortcutLabel().replace("Cmd", "Meta").replace("Ctrl", "Control"))}" aria-pressed="${active ? "true" : "false"}" data-state="${escapeAttr(speechState)}" ${disabled ? "disabled" : ""}>${iconSvg("microphone")}<span class="sr-only">${escapeHtml(label)}</span></button>
    <span class="speech-mode-status${speechState === "idle" ? " sr-only" : ""}" data-speech-mode-status role="status" aria-live="polite">${escapeHtml(status)}</span>
  </span>`;
}
...
function updateSpeechModeControls() {
  document.querySelectorAll("[data-toggle-speech-mode]").forEach((button) => {
    ...
    if (status) {
      ...
      status.classList.toggle("sr-only", value === "idle");
    }
  });
}
  ...
  return `
    <section class="diff-hunk" aria-label="Hunk ${index + 1}">
      ...
      <div class="diff-lines">
        ${lines.map((line, lineIndex) => renderDiffLine(line, lineIndex, { ...options, hunkIndex: index })).join("")}
      </div>
    </section>
  `;
}
...
function pullRequestDiffSelection(agentKey, pullRequestId, snapshotId = "") {
  ...
  if (!current || (snapshotId && current.snapshotId !== snapshotId)) {
    state.pullRequestDiffSelections[key] = { snapshotId, anchor: null, lines: [] };
  }
...
function attachSelectedPullRequestLines(agentKey, pullRequestId) {
  ...
  selection.lines = [];
  document.querySelectorAll("[data-select-pr-diff-line]").forEach((control) => { control.checked = false; });
  render();
  window.requestAnimationFrame(() => {
    ...
    const prompt = document.querySelector(`#composer[data-agent-key="${CSS.escape(agentKey)}"] #prompt-input`);
...
function clearActivePullRequestDiffSelection() {
  ...
  const diff = state.pullRequestDiffs[pullRequestDiffKey(route.key, pullRequestId)];
  const selection = pullRequestDiffSelection(route.key, pullRequestId, diff?.snapshot_id || "");
  ...
  while (index < blocks.length) {
    ...
    if (primaryConversationBlock(block)) {
      rendered.push(systemEventBlock(block)
        ? renderSystemEventBlock(block, index)
...
function systemEventBlock(block) {
  return block?.kind === "validation_retry";
}

function renderSystemEventBlock(block, index) {
