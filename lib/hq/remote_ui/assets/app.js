...
function renderOnboarding() {
  ...
  const readyHarnesses = harnesses.filter((item) => item.ready);
  const harnessText = readyHarnesses.length
    ? `${readyHarnesses.map((item) => item.name).join(", ")} ready`
    : "No agent CLI ready yet";
  setHeader("Start Tycho", "No projects configured", "S");
  replaceView(`
    <section class="onboarding-page">
      <div class="onboarding-panel">
        <img class="onboarding-logo" src="/remote-logo.png?v=${escapeAttr(document.documentElement.dataset.assetVersion || "")}" alt="">
        <p class="eyebrow">No projects configured</p>
        <h1>Start Tycho</h1>
        <p>Tycho can create a safe local sandbox so you can try managed agents before connecting a real project.</p>
        ...
        <div class="onboarding-facts">
          <span class="chip detail">${escapeHtml(welcomePath)}</span>
          <span class="chip ${readyHarnesses.length ? "done" : "need"}">${escapeHtml(harnessText)}</span>
        </div>
        <p class="field-hint">To add a local project instead, open Tycho in the terminal and choose Add Local Project.</p>
      </div>
    </section>
  `);
}
