    ...
    <li class="prompt-queue-entry" data-prompt-queue-entry="${escapeAttr(entry.id)}">
      <div class="prompt-queue-entry-copy">
        ...
        <span>
          ...
          <small>${escapeHtml(`${sourceLabel} · ${stateLabel}${authorityLabel}${attachmentLabel}`)}</small>
        </span>
      </div>
...
function renderQueueReadConversationBlock(block, index, options = {}) {
  ...
  const required = requiredActions.length
    ? `<section class="queue-work-required-actions" aria-label="Required user instructions">
        <strong>Required user instructions</strong>
        <ol>${requiredActions.map((entry) => `<li><code>${escapeHtml(String(entry.id || ""))}</code><span>${escapeHtml(String(entry.prompt || ""))}</span></li>`).join("")}</ol>
      </section>`
    : "";
  const detail = entries.length
    ? `${required}<ol class="prompt-queue-list queue-read-list">
        ${entries.map((entry, entryIndex) => renderPromptQueueEntry(options.agent || {}, entry, entryIndex, { readOnly: true })).join("")}
      </ol>`
