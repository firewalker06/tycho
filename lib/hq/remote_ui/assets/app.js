...
function renderConversationBlocks(blocks, options = {}) {
  ...
  while (index < blocks.length) {
    ...
    if (primaryConversationBlock(block)) {
      rendered.push(block?.kind === "validation_retry"
        ? renderValidationRetryBlock(block, index)
...
function primaryConversationBlock(block) {
  if (block?.kind === "validation_retry") return true;
...
function renderValidationRetryBlock(block, index) {
  ...
  return `
    <details class="message-group validation-retry-block" data-state-key="${escapeAttr(stateKey)}">
      <summary>
