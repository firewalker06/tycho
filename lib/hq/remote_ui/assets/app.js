...
function renderPersonalAssistantSetup(item) {
  ...
  return `<section class="pa-setup" aria-labelledby="pa-setup-title"><span class="section-label">FRED setup</span><h2 id="pa-setup-title">Choose how FRED should work</h2><p>Review the Codex model, reasoning effort, and IANA timezone before saving. Opening FRED creates today’s conversation; your first message stays yours.</p><form id="personal-assistant-setup-form"><label>Codex model<input name="model" required value="${escapeAttr(config.model || "gpt-5.6-sol")}" autocomplete="off"></label><label>Reasoning effort<input name="reasoning_effort" required value="${escapeAttr(config.reasoning_effort || "medium")}" autocomplete="off"></label><label>IANA timezone<input name="timezone" required value="${escapeAttr(config.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC")}" placeholder="Asia/Jakarta" autocomplete="off"></label><label class="pa-confirmation"><input name="confirmed" type="checkbox" required> I confirm these settings for FRED.</label><button class="ui-button" data-variant="brand" type="submit">Save FRED setup</button></form></section>`;
}

function renderPersonalAssistantReady(item) {
  return `<section class="pa-ready"><span class="section-label">FRED is ready</span><h2>Open today’s conversation when you’re ready.</h2><p>Opening FRED does not send a model prompt. Its introduction and examples are available before you write your first message.</p><button class="ui-button" data-variant="brand" type="button" data-open-personal-assistant>Open FRED</button></section>`;
}
