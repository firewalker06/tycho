...
function renderAgentGroup(projectKey, agents) {
  ...
  return `
    <section class="agent-group ui-collection-group ui-surface">
      <div class="group-title">
        <div class="agent-group-project">
          ${renderAgentGroupProjectTitle(projectKey, projectName, !!project)}
          <span>${escapeHtml(projectMeta)}</span>
        </div>
        ${project ? `<button class="inline-icon-button agent-group-create ui-button" type="button" data-create-agent="${escapeAttr(projectKey)}" aria-label="New agent for ${escapeAttr(projectName)}">${iconSvg("plus")}<span>New agent</span></button>` : ""}
      </div>
      ${agents.map(renderAgentRow).join("")}
    </section>
  `;
