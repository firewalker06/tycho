...
function renderPullRequestDiffDetail(agent, item, diff, options = {}) {
  ...
  if (!snapshot) return `${header}${loading ? tychoLoadingState("Loading PR diff", { className: "pr-diff-loading-state" }) : emptyState("Diff not fetched", "Fetch this pull request diff to inspect it in Tycho.")}`;
...
function attachmentViewerActions(id, attachment) {
  ...
  return `<div class="attachment-viewer-actions">${copy}${contentCopy}${downloadAction}${refresh}${remove}</div>`;
}
...
function attachmentContentCopyAction(id, attachment) {
  ...
  return `<button class="inline-icon-button" type="button" data-copy-attachment-content="${escapeAttr(id)}">${iconSvg("copy")}<span>${label}</span></button>`;
}
