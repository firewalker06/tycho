...
async function updateTycho() {
  ...
  const confirmed = await confirmAction({
    ...
    description: "Homebrew will update the Tycho installation on this server. Restart the Remote server and scheduler daemon afterward using their existing controls.",
  ...
  try {
    ...
    const detail = result?.update?.detail || "Tycho updated";
    setConnection("Tycho updated; restart Remote and scheduler daemon");
    showGrowl(`${detail} Restart Remote and the scheduler daemon to load it.`, "done");
...
els.headerMorePanel.addEventListener("click", (event) => {
  ...
  if (event.target.closest("[data-update-tycho]")) {
    closeHeaderMore();
    updateTycho();
  }
});
