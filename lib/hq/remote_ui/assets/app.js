  ...
  return `
    <form id="session-loop-settings-form" class="session-loop-settings-form">
      <div class="section-label">
        <div><strong>Session loop defaults</strong><span>Quick schedule presets</span></div>
        <button class="inline-icon-button" type="submit">${iconSvg("check")}<span>Save loop settings</span></button>
      </div>
...
function sessionLoopTemplateRow(template = {}, index = 0) {
  return `
    <section class="field-card session-loop-template-row" data-session-loop-template-row>
      <input name="template_key" type="hidden" value="${escapeAttr(template.key || "")}">
      ...
      <input id="session-loop-template-name-${index}" name="template_name" type="text" value="${escapeAttr(template.name || "")}" placeholder="Wait for PR review" required>
      <label class="field-label" for="session-loop-template-prompt-${index}">Prompt</label>
      <textarea id="session-loop-template-prompt-${index}" name="template_prompt" rows="5" required>${escapeHtml(template.prompt || "")}</textarea>
    </section>
  `;
}
