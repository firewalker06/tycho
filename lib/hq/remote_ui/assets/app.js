const TOP_TABS = ["now", "agents", "personal-assistant", "settings"];
...
function renderPersonalAssistant() {
  ...
  const settings = `<details class="pa-settings"><summary>Settings</summary><div><span>${escapeHtml(item.config?.timezone || "Asia/Jakarta")}</span><span>${escapeHtml(item.config?.model || "Recommended settings")}</span><span>${escapeHtml(item.config?.reasoning_effort || "")}</span></div><p>Tycho keeps this daily conversation and its continuity automatically.</p></details>`;
