...
function renderAgentSummaryContent(content, menuScope = "agent-summary", sections = [], agent = null) {
  return `${renderMarkdown(content, {
    ...
    menuScope,
  })}${renderSummarySections(sections, agent)}`;
}
...
function renderSummarySections(sections, agent = null) {
  ...
  return `<div class="summary-sections" aria-label="Structured summary details">${normalized.map((section, index) => {
    if (section.type === "text") return `<p class="summary-section-text">${escapeHtml(section.text).replace(/\n/g, "<br>")}</p>`;
...
function renderRunSummaryMessageContent(content, agent, block = {}, options = {}) {
  ...
  const attachmentMenu = renderSummaryAttachmentMenu(
    agent,
    summaryAttachmentsOutsideSections(block?.metadata?.attachments, block?.metadata?.summary_sections),
    summaryId
  );
  ...
  return `
    <div class="message-content summary-message-content">
      ...
      ${renderSummarySections(block?.metadata?.summary_sections, agent)}
