...
function renderScheduleForm(route) {
  ...
  replaceView(`
    <form id="schedule-form" class="agent-form schedule-form" data-ds-form="lifecycle" data-mode="${editing ? "edit" : "create"}" data-schedule-key="${escapeAttr(schedule?.key || "")}">
...
function renderScheduleMessageForm(route) {
  ...
  replaceView(`
    <form id="schedule-message-form" class="agent-form schedule-message-form" data-ds-form="lifecycle" data-schedule-key="${escapeAttr(schedule.key)}">
  ...
  return `
    <form id="session-loop-settings-form" class="session-loop-settings-form">
      ...
      <div class="session-loop-default-grid">
        <div class="session-loop-field">
          <label class="field-label" for="session-loop-default-interval">Default interval</label>
          <input id="session-loop-default-interval" name="interval_minutes" type="number" min="1" max="59" step="1" value="${escapeAttr(String(settings.interval_minutes || 10))}" required>
          <span class="field-hint">Minutes between loop runs.</span>
        </div>
        <div class="session-loop-field">
          <label class="field-label" for="session-loop-default-end-time">Default stop time</label>
          <input id="session-loop-default-end-time" name="end_time" type="time" value="${escapeAttr(settings.end_time || "23:59")}" required>
          <span class="field-hint">Local time on the Tycho server.</span>
        </div>
      </div>
      <div class="form-section-heading"><strong>Prompt templates</strong><span>Optional starting points</span></div>
      ...
      <div class="response-style-form-footer session-loop-settings-footer">
        ...
        <div class="response-style-form-actions session-loop-settings-actions">
...
function sessionLoopTemplateRow(template = {}, index = 0) {
  ...
  return `
    <section class="session-loop-template-row" data-session-loop-template-row>
      ...
      <input id="session-loop-template-name-${index}" name="template_name" data-state-key="session-loop-template:${escapeAttr(stateKey)}:name" type="text" value="${escapeAttr(template.name || "")}" placeholder="Wait for PR review" required>
      <label class="field-label" for="session-loop-template-prompt-${index}">Prompt</label>
      <textarea id="session-loop-template-prompt-${index}" name="template_prompt" data-state-key="session-loop-template:${escapeAttr(stateKey)}:prompt" rows="5" required>${escapeHtml(template.prompt || "")}</textarea>
    </section>
  `;
}
  ...
  return `
    <div class="focused-view-toolbar">
      <details class="focused-view-menu" data-focused-view-menu data-overlay-menu data-overlay-key="focused-view:${escapeAttr(key)}" data-active-view-mode="${escapeAttr(activeMode)}" data-state-key="focused-view-menu:${escapeAttr(key)}">
        <summary aria-label="View layout: ${escapeAttr(active.label)}" title="View layout: ${escapeAttr(active.label)}" aria-haspopup="menu" data-view-icon="${escapeAttr(active.icon)}">
          ${iconSvg(active.icon)}
...
function renderAgentForm(route) {
  ...
  replaceView(`
    <form id="agent-form" class="agent-form" data-ds-form="lifecycle" data-mode="${editing ? "edit" : cloning ? "clone" : "create"}" data-project-key="${escapeAttr(project.key)}" data-agent-key="${escapeAttr(agent?.key || "")}" data-project-name="${escapeAttr(project.name || project.key)}">
      <div class="form-section-heading"><strong>Configuration</strong><span>Prompt and runtime</span></div>
      <section class="field-card ds-field ds-surface">
...
function renderAgentLoopForm(key) {
  ...
  replaceView(`
    <form id="agent-loop-form" class="agent-form agent-loop-form" data-agent-key="${escapeAttr(agent.key)}">
      ...
      <div class="form-section-heading"><strong>Loop timing</strong><span>Server local time</span></div>
      ...
      <div class="form-section-heading"><strong>Run prompt</strong><span>Preview and refine before starting</span></div>
      <section class="field-card">
...
function renderProjectForm(key) {
  ...
  replaceView(`
    <form id="project-form" class="agent-form project-form" data-ds-form="lifecycle" data-project-key="${escapeAttr(project.key)}">
      <section class="detail-card ds-surface">
...
function renderAgentScheduleMenu(agent, schedule) {
  ...
  return `
    <details class="header-schedule-control ${scheduledAgentIconStatusClass(agent, schedule)}" data-header-schedule-menu data-overlay-menu data-overlay-key="header-schedule:${escapeAttr(agent.key)}" data-agent-key="${escapeAttr(agent.key)}">
      <summary aria-label="Schedule actions" title="Schedule actions" aria-haspopup="menu">
