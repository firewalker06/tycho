...
function agentSplitRoute(route) {
  if (route.type === "agentSummary" || route.type === "agentAttachment") return true;
  ...
  replaceView(`
    <form id="schedule-form" class="agent-form schedule-form" data-mode="${editing ? "edit" : "create"}" data-schedule-key="${escapeAttr(schedule?.key || "")}">
      ...
      <section class="field-card" data-schedule-message-mode="file">
        ...
        <label class="field-label" for="schedule-message-file-content">Message file markdown</label>
        <textarea id="schedule-message-file-content" name="message_file_content" rows="12"></textarea>
      </section>
      <section class="field-card">
...
function renderAgent(key, options = {}) {
  ...
  if (options.splitWorkspace && focusedMode) {
    ...
    const detailKind = summaryMode ? "summary" : "attachment";
...
els.view.addEventListener("change", (event) => {
  ...
  const scheduleMessageFile = event.target.closest("#schedule-message-file");
  if (scheduleMessageFile) {
    const form = scheduleMessageFile.closest("form");
    const editor = scheduleMessageFileContentInput(form);
    const nextPath = String(scheduleMessageFile.value || "").trim();
    const previousPath = editor?.dataset.messageFileLoadedPath || "";
    if (editor && previousPath && previousPath !== nextPath) {
      editor.value = "";
    }
    if (editor) {
      editor.dataset.messageFileLoaded = "";
      editor.dataset.messageFileLoadedPath = "";
      if (!nextPath) {
        editor.value = "";
      }
    }
    if (form) {
      void loadScheduleMessageFileContent(form);
    }
    return;
  }
...
els.view.addEventListener("submit", (event) => {
  ...
  if (event.target.id === "schedule-form") {
    ...
    const messageFile = String(formData.get("message_file") || "").trim();
    const messageFileContent = String(formData.get("message_file_content") || "");
    ...
    mutate(async () => {
      ...
      if (nextSchedule?.key && messageSource === "file" && messageFile) {
        await apiPut(`/schedules/${encodeURIComponent(nextSchedule.key)}/message_file`, { message_file: messageFile, content: messageFileContent });
      }
      clearFormDraft(form);
      navigate({ type: "tab", tab: "now" });
...
function syncScheduleMessageFields(form) {
  ...
  if (source === "file") void loadScheduleMessageFileContent(form);
}

function scheduleMessageFileContentInput(form) {
  return form?.querySelector("#schedule-message-file-content");
}

async function loadScheduleMessageFileContent(form) {
  if (!form || form.dataset.mode !== "edit") return;
  const key = form.dataset.scheduleKey;
  const source = form.querySelector("[data-schedule-message-source]")?.value || "inline";
  if (!key || source !== "file") return;

  const editor = scheduleMessageFileContentInput(form);
  if (!editor) return;

  const messageFile = String(form.querySelector("#schedule-message-file")?.value || "").trim();
  if (!messageFile) return;
  if (editor.dataset.messageFileLoadedPath === messageFile && editor.dataset.messageFileLoaded === "true") return;
  if (editor.value && editor.value.length > 0 && editor.dataset.messageFileLoadedPath === messageFile) {
    editor.dataset.messageFileLoaded = "true";
    return;
  }

  editor.dataset.messageFileLoaded = "loading";
  try {
    const encodedPath = encodeURIComponent(messageFile);
    const data = await apiGet(`/schedules/${encodeURIComponent(key)}/message_file?path=${encodedPath}`);
    editor.value = String(data.content || "");
    editor.dataset.messageFileLoadedPath = messageFile;
    editor.dataset.messageFileLoaded = "true";
  } catch (error) {
    editor.dataset.messageFileLoaded = "";
    setConnection(error.message);
