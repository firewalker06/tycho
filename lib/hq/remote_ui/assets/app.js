const TOP_TABS = ["now", "agents", "reviews", "settings"];
...
const els = {
  ...
  reviewsNav: document.getElementById("reviews-nav-button"),
...
function currentTopTab(route) {
  ...
  if (route.type === "pullRequestReview") return "reviews";
...
async function refresh(options = {}) {
  ...
  try {
    ...
    els.reviewsNav?.classList.toggle("hidden", !githubReviewAvailable());
...
async function ensureRouteData(options = {}) {
  const route = parseRoute();
  if (route.type === "tab" && route.tab === "reviews") {
    await ensurePullRequestInbox(options.forcePullRequests || options.force);
  }
  if (route.type === "pullRequestReview") {
    await ensurePullRequestReview(route.id, options.forcePullRequests || options.force);
  }
...
function renderCurrentView() {
  ...
  } else if (route.type === "pullRequestReview") {
    renderPullRequestReview(route.id);
  } else if (route.tab === "reviews") {
  ...
  const summary = waiting.length > 0
    ? `
      <section class="summary-card attention">
        ...
        <button class="primary ui-button" data-variant="brand" type="button" data-first-waiting>Answer ${escapeHtml(waiting[0]?.name || "first request")}</button>
      </section>
    `
    : "";
    ...
    <div class="detail-row schedule-row">
      ...
      <div class="schedule-row-end">
        <div class="compact-actions schedule-row-actions">
          <button class="inline-icon-button schedule-run-button ui-button" type="button" data-schedule-action="run" data-schedule-key="${escapeAttr(schedule.key)}" aria-label="Run schedule now" title="Run schedule now">${iconSvg("sportShoe")}<span>Run now</span></button>
  ...
  if (["running", "stale", "untracked"].includes(status)) {
    return `
      <div class="compact-actions schedule-daemon-actions">
        <button class="inline-icon-button ui-button" type="button" data-scheduler-action="restart" data-confirm="Restart scheduler daemon?">${iconSvg("rotateCcw")}<span>Restart daemon</span></button>
        <button class="danger inline-icon-button ui-button" data-variant="danger" type="button" data-scheduler-action="stop" data-confirm="Stop scheduler daemon?">${iconSvg("x")}<span>Stop daemon</span></button>
      </div>
    `;
  }
...
function renderServerRow(server) {
  ...
  const actions = server.local && active
    ? `<button class="inline-icon-button ui-button" type="button" disabled>${iconSvg("check")}<span>Active</span></button>`
...
function renderAgentGroup(projectKey, agents) {
  ...
  const projectStatus = project?.status || "configured";
  const countLabel = `${agents.length} ${agents.length === 1 ? "agent" : "agents"}`;
  const groupClassName = ["agent-group", agents.length ? "" : "empty"].filter(Boolean).join(" ");
  return `
    <section class="${groupClassName}">
      <div class="group-title">
        ...
        <span class="agent-group-count" aria-label="${escapeAttr(`${countLabel}, ${projectStatus}`)}" title="${escapeAttr(`${countLabel} / ${projectStatus}`)}">${agents.length}</span>
...
els.back.addEventListener("click", () => {
  const route = parseRoute();
  if (route.type === "pullRequestReview") navigate({ type: "tab", tab: "reviews" });
  else if (route.type === "agent") navigate({ type: "tab", tab: "agents" });
