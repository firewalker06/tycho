...
function parseDelegatedAgentReply(entry) {
  ...
  let payload;
  try {
    payload = JSON.parse(String(entry?.prompt || ""));
  } catch (_error) {
    return null;
  }
  ...
  const reports = Array.isArray(payload.reports) ? payload.reports :
    payload.report && typeof payload.report === "object" ? [payload.report] :
      payload.child && typeof payload.child === "object" ? [payload] : [];
  const recognized = ["delegated_agent_reports", "delegated_agent_report", "delegation_report"].includes(payload.type) ||
    Boolean(payload.child && payload.status);
  if (!recognized || !reports.length || !reports.every((report) => report && !Array.isArray(report) && typeof report === "object")) return null;
...
function renderDelegatedAgentReport(report, stateKey) {
  const child = report.child && typeof report.child === "object" ? report.child : {};
  const name = String(child.name || child.agent_key || "Delegated agent").trim() || "Delegated agent";
  ...
  const inquiryText = inquiry?.message ? `\n\n### Input requested\n\n${String(inquiry.message).trim()}` : "";
  ...
  return `<section class="delegated-agent-report">
    ...
    <div class="markdown-message-content delegated-agent-report-content">${renderMarkdown(`${summary}${inquiryText}`, {
...
function renderDelegatedAgentReportAttachment(attachment) {
  ...
  return `<li><a href="${escapeAttr(href)}" target="_blank" rel="noreferrer">${escapeHtml(title)}</a></li>`;
}
