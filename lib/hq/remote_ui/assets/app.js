  ...
  try {
    ...
    if (refreshShell) {
      ...
      if (parseRoute().type === "personalAssistant" && state.personalAssistant?.configured &&
          !state.personalAssistant.active_key && ["ready", "dormant"].includes(state.personalAssistant.state)) {
  ...
  return `<section class="detail-card" id="settings-personal-assistant" data-personal-assistant-state="${escapeAttr(item.state)}">
    <div class="detail-card-body" style="padding-top: 12px;">
      ...
      ${item.configured ? `<div class="kv-grid">
        ...
        ${copyableKv("Continuity", item.active_key ? `Daily session ${item.active_date || ""}` : "No active session")}
      </div>` : ""}
      ${personalAssistantReadinessNotice()}
      ${item.configured ? `<div class="pa-settings-actions"><button class="ui-button" type="button" data-open-personal-assistant>Open conversation</button><button class="ui-button" type="button" data-edit-personal-assistant-settings>${editing ? "Editing settings" : "Edit settings"}</button>${active ? '<button class="ui-button" type="button" data-restart-personal-assistant>Start fresh conversation</button>' : ""}</div>${editing ? personalAssistantConfigForm(item, { settings: true }) : ""}` : `<button class="ui-button" data-variant="brand" type="button" data-initiate-personal-assistant>Set up FRED</button>`}
...
function renderPersonalAssistantProposal(proposal) {
  ...
  const pending = proposal.state === "awaiting_confirmation";
  const busy = state.pendingPersonalAssistantProposalIds?.has(proposal.id);
  const receipt = proposal.state === "executed" ? "Completed" : proposal.state === "rejected" ? "Rejected" : proposal.state === "failed" ? `Could not complete: ${proposal.error || "Unknown error"}` : proposal.state === "executing" ? "Working…" : "";
  ...
  return `<article class="pa-inline-action ${pending ? "pending" : "receipt"}" data-proposal-id="${escapeAttr(proposal.id)}" data-proposal-run="${escapeAttr(proposal.source_run_id || "")}" data-proposal-state="${escapeAttr(proposal.state)}">
    <span class="pa-action-label">${pending ? "Approval needed" : "Action receipt"}</span>
    <dl><div><dt>Action</dt><dd>${escapeHtml(copy.label)}</dd></div><div><dt>Target</dt><dd>${escapeHtml(String(copy.target))}</dd></div><div><dt>Will do</dt><dd>${escapeHtml(personalAssistantProposalInstruction(proposal))}</dd></div>${settings}<div><dt>Effect</dt><dd>${escapeHtml(copy.effect)}</dd></div></dl>
    ${pending ? `<p class="pa-action-confirmation">This change runs only after you confirm it.</p><div class="pa-action-controls"><button class="inline-icon-button ui-button" data-variant="brand" type="button" data-confirm-pa-proposal="${escapeAttr(proposal.id)}" aria-label="${escapeAttr(confirmLabel)}" ${busy ? "disabled" : ""}>${iconSvg("thumbsUp")}<span>${busy ? "Confirming…" : confirmLabel}</span></button><button class="inline-icon-button ui-button" type="button" data-reject-pa-proposal="${escapeAttr(proposal.id)}" aria-label="Reject proposed action" ${busy ? "disabled" : ""}>${iconSvg("thumbsDown")}<span>Reject</span></button></div>` : `<p class="pa-action-receipt" role="status">${escapeHtml(receipt)}</p>${renderPersonalAssistantResult(proposal)}${proposal.recovery ? `<div class="pa-action-controls">${proposal.recovery.action === "verify" ? `<button class="ui-button" type="button" data-verify-pa-proposal="${escapeAttr(proposal.id)}">Check whether it completed</button><button class="ui-button" type="button" data-pa-suggestion="Investigate action ${escapeAttr(proposal.id)} (${escapeAttr(copy.label)}) for ${escapeAttr(String(copy.target))}; do not repeat it.">Ask FRED to investigate</button>` : ""}${proposal.recovery.action === "replace" ? `<button class="ui-button" type="button" data-pa-suggestion="Prepare a safe replacement for action ${escapeAttr(proposal.id)}: ${escapeAttr(copy.label)} for ${escapeAttr(String(copy.target))}.">Ask FRED to revise</button>` : ""}</div>` : ""}`}
    <details class="pa-technical-details"><summary>Technical details</summary><pre>${escapeHtml(JSON.stringify({ proposal_id: proposal.id, type: proposal.type, arguments: proposal.arguments }, null, 2))}</pre></details>
  </article>`;
}
...
function renderPersonalAssistantUnattachedProposals(proposals, rendered) {
  ...
  return `<section class="pa-action-feed" aria-label="FRED actions"><span class="pa-action-feed-label">Actions from recent FRED replies</span>${unmatched.map(renderPersonalAssistantProposal).join("")}</section>`;
}
...
function renderPersonalAssistantContinuity(item, proposals) {
  ...
  const historyHtml = history.length ? `<p><strong>Recent history</strong></p><ul>${history.slice(0, 6).map((entry) => `<li><span>${escapeHtml(entry.active_date || entry.closed_at || "Previous conversation")}</span><span>${escapeHtml(entry.summary || "Archived handoff")}</span><button class="ui-button" type="button" data-open-pa-history="${escapeAttr(entry.id)}">View handoff</button></li>`).join("")}</ul>${historyEntry ? `<div class="pa-history-entry"><strong>${escapeHtml(historyEntry.active_date || "Archived handoff")}</strong><pre>${escapeHtml(personalAssistantContinuityPreview(historyEntry))}</pre></div>` : ""}` : "";
  ...
  return `<details class="pa-continuity"><summary>History and context <span>Next handoff: ${escapeHtml(rolloverText)}</span></summary><div class="pa-continuity-body"><p><strong>Prior handoff</strong></p><p>${escapeHtml(personalAssistantContinuityPreview(item.continuity))}</p>${item.settings_apply ? `<p>${escapeHtml(item.settings_apply)}</p>` : ""}${trackedHtml}${historyHtml}${recoveryHtml}</div></details>`;
}
...
function renderPersonalAssistant() {
  ...
  setMainHeaderMore("personal-assistant");
  setHeader("FRED", "Friendly Robot for Execution Dispatcher", "A");
  ...
  const thread = agent
    ? `<div class="pa-conversation-scroll" data-preserve-scroll data-state-key="personal-assistant-thread">${continuity}${starter ? renderPersonalAssistantWelcome(item) : renderAgentConversationView(agent, blocks, { floatingActions: false, loading: conversationLoading(agent.key), personalAssistant: true, personalAssistantProposals: proposals, renderedPersonalAssistantProposals: renderedProposals })}${renderPersonalAssistantUnattachedProposals(proposals, renderedProposals)}${item.error ? feedbackMessage("Sending is paused", item.error, { intent: "danger" }) : ""}${blocked ? `<p class="pa-inline-notice">${escapeHtml({ closing: "Finishing today’s conversation. Your draft is kept for the next conversation.", summarizing: "Saving today’s continuity. Your draft is kept for the next conversation.", archiving: "Closing today’s conversation. Your draft is kept for the next conversation." }[item.state] || "FRED is unavailable right now.")}</p>` : ""}</div>`
    : `<div class="pa-conversation-scroll" data-preserve-scroll data-state-key="personal-assistant-thread">${item.configured ? `${continuity}<section class="pa-ready ui-surface"><h2>FRED is ready</h2><p>Open today’s conversation when you are ready. Opening does not send a message.</p><button class="ui-button" data-variant="brand" type="button" data-open-personal-assistant>Open conversation</button></section>` : renderPersonalAssistantSetup(item)}${item.error ? feedbackMessage("Sending is paused", item.error, { intent: "danger" }) : ""}</div>`;
  const settings = `<details id="personal-assistant-settings-menu" class="pa-settings" data-overlay-menu data-overlay-key="personal-assistant-settings"><summary aria-label="Settings actions" title="Settings actions" aria-haspopup="menu">${iconSvg("ellipsis")}<span class="sr-only">Settings actions</span></summary><div class="pa-settings-menu ui-overlay-surface" data-overlay-kind="menu"><div class="more-menu" role="menu" aria-label="Settings actions">${moreMenuButton({ label: "Settings", icon: "settings", attrs: "data-open-settings" })}</div></div></details>`;
  const dock = agent && item.state === "active" ? renderAgentComposerPlacement(agent, [], {}).dock : "";
  replaceView(`<section class="personal-assistant-page" data-personal-assistant-state="${escapeAttr(item.state)}"><header class="pa-header"><button class="pa-tycho-nav header-mark" type="button" data-agent-switcher aria-label="Open agent switcher" title="Open agent switcher" aria-expanded="${state.unreadPanelOpen ? "true" : "false"}" aria-controls="unread-agents-panel">${brandLogoHtml(unreadAgents().length)}</button><strong><img class="fred-avatar" src="/fred-avatar.png?v=${escapeAttr(document.documentElement.dataset.assetVersion || "")}" alt="FRED">FRED</strong>${settings}</header>${thread}<footer class="pa-composer-row">${dock}</footer></section>`);
}
...
function syncUnreadAlert() {
  ...
  positionPersonalAssistantSwitcher();
}

function agentSwitcherMark() {
  return document.querySelector(".personal-assistant-page [data-agent-switcher]") || els.mark;
}

function positionPersonalAssistantSwitcher() {
  if (!document.body.querySelector(".personal-assistant-page") || !state.unreadPanelOpen) {
    els.unreadPanel.style.removeProperty("top");
    els.unreadPanel.style.removeProperty("left");
    els.unreadPanel.style.removeProperty("right");
    return;
  }

  const bounds = agentSwitcherMark().getBoundingClientRect();
  const maximumWidth = Math.min(420, window.innerWidth - 24);
  const left = Math.max(12, Math.min(Math.floor(bounds.left), window.innerWidth - maximumWidth - 12));
  els.unreadPanel.style.top = `${Math.ceil(bounds.bottom + 8)}px`;
  els.unreadPanel.style.left = `${left}px`;
  els.unreadPanel.style.right = "auto";
}
...
function handleViewClick(event) {
  if (event.target.closest("[data-agent-switcher]")) {
    toggleUnreadPanel();
    return;
  }
  ...
  if (event.target.closest("[data-restart-personal-assistant]")) {
    confirmAction({
      title: "Start a fresh FRED conversation?",
      description: "Tycho archives the current conversation and starts a new one with the same settings. Your history stays available.",
      confirmLabel: "Start fresh conversation",
    }).then((confirmed) => {
      if (!confirmed) return;
      mutate(async () => {
        ...
        showGrowl("Fresh FRED conversation started", "done");
      });
    });
    return;
  }
  ...
  if (confirmPa) {
    const id = confirmPa.dataset.confirmPaProposal;
    if (state.pendingPersonalAssistantProposalIds.has(id)) return;
    state.pendingPersonalAssistantProposalIds.add(id); state.renderedViewHtml = ""; render();
    mutate(async () => {
      await apiPost(`/personal-assistant/actions/${encodeURIComponent(id)}/confirm`, { confirmed: true });
      state.personalAssistantProposals = (await apiGet("/personal-assistant/actions")).proposals || [];
      const agent = state.personalAssistant?.active_key && (state.agentDetails[state.personalAssistant.active_key] || state.personalAssistant.agent);
      if (agent) await ensureConversation(agent, true);
      showGrowl("Action confirmed", "done");
    }).finally(() => { state.pendingPersonalAssistantProposalIds.delete(id); state.renderedViewHtml = ""; render(); });
    return;
  }
  const rejectPa = event.target.closest("[data-reject-pa-proposal]");
  if (rejectPa) {
    const id = rejectPa.dataset.rejectPaProposal;
    if (state.pendingPersonalAssistantProposalIds.has(id)) return;
    state.pendingPersonalAssistantProposalIds.add(id); state.renderedViewHtml = ""; render();
    mutate(async () => { await apiPost(`/personal-assistant/actions/${encodeURIComponent(id)}/reject`, {}); state.personalAssistantProposals = (await apiGet("/personal-assistant/actions")).proposals || []; showGrowl("Action rejected", "done"); }).finally(() => { state.pendingPersonalAssistantProposalIds.delete(id); state.renderedViewHtml = ""; render(); });
    return;
  }
  const verifyPa = event.target.closest("[data-verify-pa-proposal]");
  if (verifyPa) {
    const id = verifyPa.dataset.verifyPaProposal;
    if (state.pendingPersonalAssistantProposalIds.has(id)) return;
    state.pendingPersonalAssistantProposalIds.add(id); state.renderedViewHtml = ""; render();
    mutate(async () => {
      const response = await apiPost(`/personal-assistant/actions/${encodeURIComponent(id)}/verify`, {});
      state.personalAssistantProposals = (await apiGet("/personal-assistant/actions")).proposals || [];
      showGrowl(response?.proposal?.state === "executed" ? "Action outcome confirmed" : "Action state checked", "done");
    }).finally(() => { state.pendingPersonalAssistantProposalIds.delete(id); state.renderedViewHtml = ""; render(); });
    return;
  }
  ...
  if (historyButton) {
    ...
    mutate(async () => {
      const response = await apiGet(`/personal-assistant/history/${encodeURIComponent(id)}`);
  ...
  if (event.target.closest("[data-open-personal-assistant]")) {
    mutate(async () => {
      const response = await apiPost("/personal-assistant/open", {});
      state.personalAssistant = response.personal_assistant;
      if (state.personalAssistant?.agent) state.agentDetails[state.personalAssistant.agent.key] = state.personalAssistant.agent;
      showGrowl("FRED opened", "done");
    });
    return;
  }
...
window.addEventListener("resize", positionPersonalAssistantSwitcher);
