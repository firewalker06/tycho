...
function renderPersonalAssistant() {
  ...
  const blocked = item.state !== "active" && item.configured;
  ...
  const thread = agent && item.state === "active"
    ...
    : `<div class="pa-conversation-scroll" data-preserve-scroll data-state-key="personal-assistant-thread">${renderPersonalAssistantWelcome()}${item.error ? feedbackMessage("Sending is paused", item.error, { intent: "danger" }) : ""}${blocked ? `<p class="pa-inline-notice">${escapeHtml({ closing: "Finishing today’s conversation. Sending is paused.", summarizing: "Saving today’s continuity. Sending is paused.", archiving: "Closing today’s conversation. Sending is paused." }[item.state] || "FRED is unavailable right now.")}</p>` : ""}</div>`;
  const settings = `<details id="personal-assistant-settings-menu" class="pa-settings" data-overlay-menu data-overlay-key="personal-assistant-settings"><summary aria-label="Settings actions" title="Settings actions" aria-haspopup="menu">${iconSvg("ellipsis")}<span class="sr-only">Settings actions</span></summary><div class="pa-settings-menu ui-overlay-surface" role="menu" aria-label="Settings actions" data-overlay-kind="menu">${moreMenuHtml([moreMenuButton({ label: "Settings", sublabel: "Open app settings", icon: "settings", attrs: "data-open-settings" })])}<div class="pa-settings-details"><span>${escapeHtml(item.config?.timezone || "Asia/Jakarta")}</span><span>${escapeHtml(item.config?.model || "Recommended settings")}</span><span>${escapeHtml(item.config?.reasoning_effort || "")}</span><p>Tycho keeps this daily conversation and its continuity automatically.</p></div></div></details>`;
  const dock = agent && item.state === "active" ? renderAgentComposerPlacement(agent, [], {}).dock : `<form id="personal-assistant-start-form" class="pa-start-composer"><textarea name="prompt" rows="2" aria-label="Message FRED" placeholder="Message FRED"></textarea><button class="ui-button" data-variant="brand" type="submit" ${blocked ? "disabled" : ""}>${item.configured ? "Open FRED" : "Send"}</button></form>`;
...
els.view.addEventListener("submit", (event) => {
  event.preventDefault();
  if (event.target.id === "personal-assistant-start-form") {
    const form = event.target;
    const prompt = String(new FormData(form).get("prompt") || "").trim();
    mutate(async () => {
      let assistant = state.personalAssistant;
      if (!assistant?.configured) assistant = (await apiPost("/personal-assistant/setup", { confirmed: true, model: "gpt-5.6-sol", reasoning_effort: "medium", timezone: "Asia/Jakarta" })).personal_assistant;
      if (!assistant?.active_key) assistant = (await apiPost("/personal-assistant/open", {})).personal_assistant;
      state.personalAssistant = assistant;
      if (assistant.active_key && prompt) {
        const response = await apiPost(`/agents/${encodeURIComponent(assistant.active_key)}/messages`, { prompt, start: true, attachments: [], pull_request_contexts: [] });
        if (response.agent) upsertAgent(response.agent);
        if (Array.isArray(response.conversation)) state.conversations[assistant.active_key] = { revision: response.agent?.revision, blocks: response.conversation, loaded: true, loading: false };
      }
      showGrowl(assistant?.configured ? "FRED ready" : "FRED opened", "done");
    }, { form });
    return;
  }
