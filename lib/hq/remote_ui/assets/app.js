...
async function confirmRemoveRemoteServer(key) {
  ...
  const label = server?.name || key || "this server";
...
function render(options = {}) {
  const previous = state.preserveLiveEditorDuringRender;
  state.preserveLiveEditorDuringRender = options.preserveLiveEditor === true;
  try {
    renderCurrentView();
  } finally {
    state.preserveLiveEditorDuringRender = previous;
  }
}
...
function renderCurrentView() {
  ...
  els.header.querySelector(".header-row")?.classList.toggle("ds-detail-header", subpage && !onboarding);
...
function replaceView(html) {
  ...
  if (!reconciled) els.view.innerHTML = html;
  ...
  const summary = waiting.length > 0
    ? `
      <section class="summary-card attention">
        ...
        <button class="primary ds-button" data-variant="brand" type="button" data-first-waiting>Answer ${escapeHtml(waiting[0]?.name || "first request")}</button>
      </section>
    `
    : "";
        ...
        <details class="schedule-details" data-state-key="schedule-now-details">
          ...
          <div class="schedule-details-body">
            <div class="compact-actions schedule-management-actions">
              <button class="primary inline-icon-button ds-button" data-variant="brand" type="button" data-new-schedule>${iconSvg("plus")}<span>New schedule</span></button>
            </div>
    ...
    <div class="detail-row schedule-row">
      ...
      <div class="schedule-row-end">
        <div class="compact-actions schedule-row-actions">
          <button class="inline-icon-button schedule-run-button ds-button" type="button" data-schedule-action="run" data-schedule-key="${escapeAttr(schedule.key)}" aria-label="Run schedule now" title="Run schedule now">${iconSvg("sportShoe")}<span>Run now</span></button>
          <button class="inline-icon-button schedule-toggle-button ds-button" type="button" data-schedule-action="${toggleAction}" data-schedule-key="${escapeAttr(schedule.key)}" aria-label="${escapeAttr(toggleLabel)} schedule" title="${escapeAttr(toggleLabel)} schedule">${iconSvg(toggleIcon)}<span>${toggleLabel}</span></button>
          ${targetKey ? `<button class="inline-icon-button schedule-session-button ds-button" type="button" data-open-agent="${escapeAttr(targetKey)}" aria-label="Open scheduled session" title="Open scheduled session">${iconSvg("robot")}<span>Session</span></button>` : ""}
...
function renderScheduleActionsMenu(schedule) {
  ...
  return `
    <details class="schedule-action-menu" data-overlay-menu data-overlay-key="schedule:${escapeAttr(schedule.key)}">
      ...
      <div class="schedule-action-popover ds-overlay-surface" data-overlay-kind="menu">
        ${moreMenuHtml(items)}
      </div>
    </details>
  `;
...
function renderScheduleDaemonActions(daemon, schedules) {
  ...
  if (["running", "stale", "untracked"].includes(status)) {
    return `
      <div class="compact-actions schedule-daemon-actions">
        <button class="inline-icon-button ds-button" type="button" data-scheduler-action="restart" data-confirm="Restart scheduler daemon?">${iconSvg("rotateCcw")}<span>Restart daemon</span></button>
        <button class="danger inline-icon-button ds-button" data-variant="danger" type="button" data-scheduler-action="stop" data-confirm="Stop scheduler daemon?">${iconSvg("x")}<span>Stop daemon</span></button>
      </div>
    `;
  }
  ...
  return `
    <div class="compact-actions schedule-daemon-actions">
      <button class="primary inline-icon-button ds-button" data-variant="brand" type="button" data-scheduler-action="start" ${hasSchedules ? "" : "disabled"}>${iconSvg("loaderPinwheel")}<span>Start daemon</span></button>
    </div>
  `;
}
...
function renderScheduleForm(route) {
  ...
  replaceView(`
    <form id="schedule-form" class="agent-form ds-form-layout schedule-form" data-ds-form="lifecycle" data-mode="${editing ? "edit" : "create"}" data-schedule-key="${escapeAttr(schedule?.key || "")}">
      <div class="form-section-heading ds-form-section-heading"><strong>Identity</strong><span>Stable schedule details</span></div>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="schedule-key">Key</label>
        <input class="ds-input" id="schedule-key" name="key" type="text" value="${escapeAttr(values.key)}" autocomplete="off" aria-describedby="schedule-key-help" ${editing ? "readonly" : "required"}>
        <span class="field-hint ds-field__description" id="schedule-key-help">${editing ? "Schedule keys are fixed after creation." : "Use a short stable key, such as weekly-maintenance."}</span>
      </section>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="schedule-name">Name</label>
        <input class="ds-input" id="schedule-name" name="name" type="text" value="${escapeAttr(values.name)}" autocomplete="off" placeholder="Weekly maintenance">
      </section>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="schedule-project">Project</label>
        <select class="ds-input" id="schedule-project" name="project_key" required>
          ${scheduleProjectOptions(values.projectKey)}
        </select>
      </section>
      <div class="form-section-heading ds-form-section-heading"><strong>Timing</strong><span>When recurring work starts</span></div>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="schedule-cron">Cron</label>
        <input class="ds-input" id="schedule-cron" name="cron" type="text" value="${escapeAttr(values.cron)}" autocomplete="off" aria-describedby="schedule-cron-help" required>
        <span class="field-hint ds-field__description" id="schedule-cron-help">${escapeHtml(humanizeCron(values.cron))}</span>
      </section>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="schedule-timezone">Timezone</label>
        <select class="ds-input" id="schedule-timezone" name="timezone">
          <option value="local" ${values.timezone === "local" ? "selected" : ""}>Local</option>
          <option value="UTC" ${values.timezone === "UTC" ? "selected" : ""}>UTC</option>
        </select>
      </section>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="schedule-ends-at">Ends at</label>
        <input class="ds-input" id="schedule-ends-at" name="ends_at" type="datetime-local" value="${escapeAttr(values.endsAt)}" aria-describedby="schedule-ends-at-help">
        <span class="field-hint ds-field__description" id="schedule-ends-at-help">Optional. Loop schedules stop automatically after this time.</span>
      </section>
      <div class="form-section-heading ds-form-section-heading"><strong>Agent</strong><span>Session identity and context</span></div>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="schedule-agent-name">Agent name</label>
        <input class="ds-input" id="schedule-agent-name" name="agent_name" type="text" value="${escapeAttr(values.agentName)}" autocomplete="off" placeholder="Defaults to schedule name">
      </section>
      <div class="form-section-heading ds-form-section-heading"><strong>Recurring messages</strong><span>Stable context and per-run request</span></div>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="schedule-system-message">System message</label>
        <textarea class="ds-input" id="schedule-system-message" name="system_message" rows="6" aria-describedby="schedule-system-message-help">${escapeHtml(values.systemMessage)}</textarea>
        <span class="field-hint ds-field__description" id="schedule-system-message-help">Stable context for this schedule-owned agent session.</span>
      </section>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="schedule-message">Run message</label>
        <textarea class="ds-input" id="schedule-message" name="message" rows="8" aria-describedby="schedule-message-help">${escapeHtml(values.message)}</textarea>
        <span class="field-hint ds-field__description" id="schedule-message-help">Sent as the user message every time this schedule runs.</span>
      </section>
      <div class="button-row form-actions ds-form-actions">
        <button class="ds-button" type="button" data-cancel-schedule-form>Cancel</button>
        <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit" value="save">${iconSvg(editing ? "pencil" : "plus")}<span>${editing ? "Save schedule" : "Create schedule"}</span></button>
      </div>
    </form>
  `);
}
...
function renderScheduleMessageForm(route) {
  ...
  replaceView(`
    <form id="schedule-message-form" class="agent-form ds-form-layout schedule-message-form" data-ds-form="lifecycle" data-schedule-key="${escapeAttr(schedule.key)}">
      ...
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="schedule-message-content">Message sent each run</label>
        <textarea class="ds-input" id="schedule-message-content" name="content" rows="18" spellcheck="true" aria-describedby="schedule-message-content-help">${escapeHtml(message.content || "")}</textarea>
        <span class="field-hint ds-field__description" id="schedule-message-content-help">Sent as the user message every time this schedule runs.</span>
      </section>
      <div class="button-row form-actions ds-form-actions">
        <button class="ds-button" type="button" data-cancel-schedule-message-form>Cancel</button>
        <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit" value="save">${iconSvg("pencil")}<span>Save message</span></button>
      </div>
    </form>
  `);
}
...
function renderAgents(options = {}) {
  ...
  replaceView(`
    <section class="ds-searchable-collection" aria-label="Managed agents">
      <div class="top-actions ds-collection-toolbar">
        <label class="search-box ds-search-field" for="agent-filter">
          <span class="nav-mark" aria-hidden="true">${iconSvg("search")}</span>
          <input class="ds-search-field__input" id="agent-filter" type="search" value="${escapeAttr(state.filters.agents)}" placeholder="Filter agents and projects" aria-label="Filter agents and projects" data-filter="agents" aria-describedby="agent-results-summary">
        </label>
      ...
      <p class="ds-collection-summary" id="agent-results-summary" role="status">${escapeHtml(resultSummary)}</p>
      ${renderBulkArchiveBar()}
      <div class="ds-collection-results">
        ${results}
      </div>
    </section>
  `);
...
function agentSortMenuHtml() {
  ...
  return `
    <details class="agent-sort-menu" data-agent-sort-menu data-overlay-menu data-overlay-key="agent-sort">
      ...
      <div class="agent-sort-options ds-overlay-surface" role="menu" aria-label="Sort agents" data-overlay-kind="menu">
        ${options}
      </div>
    </details>
  `;
...
function renderFlatAgentList(agents, query) {
  ...
  return `
    <section class="agent-group agent-flat-list ds-collection-group ds-surface" aria-label="Agents">
    ...
    <section class="onboarding-page">
      <div class="onboarding-panel">
        ...
        <div class="onboarding-actions">
          <button class="primary inline-icon-button ds-button" data-variant="brand" type="button" data-create-welcome-sandbox>${iconSvg("folder")}<span>Create Welcome Sandbox</span></button>
        </div>
...
function renderSetup() {
  ...
  const settingsPanel = (key, html) => `
    <div class="settings-section-panel ds-section-panel" id="settings-panel-${escapeAttr(key)}" data-settings-section="${escapeAttr(key)}">
      ${html}
    </div>
  `;
  replaceView(`
    <div class="settings-section-nav-shell">
      <nav class="settings-section-nav ds-section-nav" aria-label="Jump to Settings section">
  ...
  return `
    <form id="session-loop-settings-form" class="session-loop-settings-form">
      ...
      <div class="session-loop-default-grid">
        <div class="session-loop-field ds-field">
          <label class="field-label ds-field__label" for="session-loop-default-interval">Default interval</label>
          <input class="ds-input" id="session-loop-default-interval" name="interval_minutes" type="number" min="1" max="59" step="1" value="${escapeAttr(String(settings.interval_minutes || 10))}" required aria-describedby="session-loop-default-interval-help">
          <span class="field-hint ds-field__description" id="session-loop-default-interval-help">Minutes between loop runs.</span>
        </div>
        <div class="session-loop-field ds-field">
          <label class="field-label ds-field__label" for="session-loop-default-end-time">Default stop time</label>
          <input class="ds-input" id="session-loop-default-end-time" name="end_time" type="time" value="${escapeAttr(settings.end_time || "23:59")}" required aria-describedby="session-loop-default-end-time-help">
          <span class="field-hint ds-field__description" id="session-loop-default-end-time-help">Local time on the Tycho server.</span>
        </div>
      </div>
      <div class="form-section-heading ds-form-section-heading"><strong>Prompt templates</strong><span>Optional starting points</span></div>
      <div class="session-loop-template-list" data-session-loop-template-list>
        ${templates.map(sessionLoopTemplateRow).join("")}
      </div>
      <button class="inline-icon-button ds-button" type="button" data-add-session-loop-template>${iconSvg("plus")}<span>Add prompt template</span></button>
      <div class="response-style-form-footer session-loop-settings-footer">
        <span>Stored in ~/.tycho/config/hq.yml</span>
        <div class="response-style-form-actions ds-form-actions session-loop-settings-actions">
          <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit">${iconSvg("check")}<span>Save loop settings</span></button>
        </div>
      </div>
    </form>
  `;
  ...
  return `
    <section class="session-loop-template-row" data-session-loop-template-row>
      ...
      <div class="session-loop-template-heading">
        ...
        <button type="button" class="icon-button ds-button ds-icon-button" data-remove-session-loop-template aria-label="Remove prompt template" title="Remove prompt template">${iconSvg("trash2")}</button>
      </div>
      <input class="ds-input" id="session-loop-template-name-${index}" name="template_name" data-state-key="session-loop-template:${escapeAttr(stateKey)}:name" type="text" value="${escapeAttr(template.name || "")}" placeholder="Wait for PR review" required>
      <label class="field-label ds-field__label" for="session-loop-template-prompt-${index}">Prompt</label>
      <textarea class="ds-input" id="session-loop-template-prompt-${index}" name="template_prompt" data-state-key="session-loop-template:${escapeAttr(stateKey)}:prompt" rows="5" required>${escapeHtml(template.prompt || "")}</textarea>
    </section>
  `;
}
...
function renderResponseStyleEditor() {
  ...
  if (responseStyle.error) {
    return `
      <div class="notice need ds-alert" data-intent="danger" role="alert" data-testid="response-style-error">
        <div>
          <strong class="ds-alert__title">Response style unavailable</strong>
          <p>${escapeHtml(responseStyle.error)}</p>
          <button class="ds-button" type="button" data-refresh-response-style>Retry</button>
        </div>
      </div>
    `;
  }
  ...
  if (!responseStyle.drafting) {
    ...
    return `
      <div class="response-style-summary ds-surface" data-testid="response-style-summary">
        <div>
        ...
        <div class="response-style-summary-actions">
          <button class="inline-icon-button ds-button" type="button" data-open-response-style data-testid="response-style-action">
            ${iconSvg("squarePen")}<span>${responseStyle.exists ? "Edit response style" : "Add response style"}</span>
          </button>
          ${responseStyle.exists ? `<button class="danger icon-button ds-button ds-icon-button" data-variant="danger" type="button" data-delete-response-style data-testid="response-style-delete" aria-label="Remove response style" title="Remove response style">${iconSvg("trash2")}</button>` : ""}
        </div>
      </div>
    `;
  }
  ...
  return `
    <form class="response-style-form ds-surface ds-stack" id="response-style-form" data-testid="response-style-form">
      <div class="response-style-form-heading">
        <div>
          <strong>Response style</strong>
      ...
      <div class="ds-field">
        <label class="ds-field__label" for="response-style-input">Global policy</label>
        <textarea class="ds-input" id="response-style-input" name="content" rows="9" spellcheck="true" aria-describedby="response-style-help" data-testid="response-style-input">${escapeHtml(responseStyle.content || "")}</textarea>
        <span class="ds-field__description" id="response-style-help">Applied to cold and resumed managed-agent runs unless a project or template overrides it.</span>
      </div>
      <div class="response-style-form-footer">
        ...
        <div class="response-style-form-actions ds-form-actions">
          <button class="ds-button" type="button" data-cancel-response-style>Cancel</button>
          <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit" data-testid="response-style-save">
...
function renderHarnessRefreshButton() {
  ...
  return `
    <button class="inline-icon-button harness-refresh-button ds-button ${loading ? "loading" : ""}" type="button" data-refresh-harnesses ${loading ? "disabled" : ""} aria-label="Refresh harness catalogs">
    ...
    <details class="harness-details" ${ready ? "" : "open"}>
      ...
      <div class="harness-detail-body">
        ...
        <details class="harness-catalog-editor" data-state-key="harness-catalog-editor:${escapeAttr(item.name)}">
          ...
          <form class="harness-catalog-form ds-form-layout ${saving ? "form-pending" : ""}" data-ds-form="settings" data-harness-catalog-form="${escapeAttr(item.name)}" aria-busy="${saving ? "true" : "false"}">
            <label class="ds-field" for="${escapeAttr(`${fieldPrefix}-models`)}">
              <span class="ds-field__label">Custom models</span>
              <textarea class="ds-input" id="${escapeAttr(`${fieldPrefix}-models`)}" name="models" rows="3" spellcheck="false" aria-describedby="${escapeAttr(`${fieldPrefix}-models-help`)}" ${saving ? "disabled" : ""}>${escapeHtml(configuredModels.join("\n"))}</textarea>
              <span class="ds-field__description" id="${escapeAttr(`${fieldPrefix}-models-help`)}">One model ID per line. Discovered models remain available.</span>
            </label>
            <label class="ds-field" for="${escapeAttr(`${fieldPrefix}-efforts`)}">
              <span class="ds-field__label">Custom efforts</span>
              <textarea class="ds-input" id="${escapeAttr(`${fieldPrefix}-efforts`)}" name="reasoning_efforts" rows="3" spellcheck="false" aria-describedby="${escapeAttr(`${fieldPrefix}-efforts-help`)}" ${saving ? "disabled" : ""}>${escapeHtml(configuredEfforts.join("\n"))}</textarea>
              <span class="ds-field__description" id="${escapeAttr(`${fieldPrefix}-efforts-help`)}">One reasoning-effort value per line.</span>
            </label>
            <div class="ds-form-actions">
              <button class="primary inline-icon-button ds-button" data-variant="brand" data-loading="${saving ? "true" : "false"}" type="submit" ${saving ? "disabled" : ""}>
    ...
    <section class="detail-card" id="settings-servers">
      <div class="detail-card-body" style="padding-top: 12px;">
        <div class="section-label server-section-label">
          ...
          <button class="inline-icon-button ds-button" type="button" data-toggle-server-form aria-expanded="${state.serverFormOpen ? "true" : "false"}">
...
function renderServerRow(server) {
  ...
  const actions = server.local
    ? `<button class="inline-icon-button ds-button" type="button" disabled>${iconSvg("check")}<span>Active</span></button>`
    : renderServerActionsMenu(server, active);
  ...
  return `
    <div class="server-list-item ${server.local ? "local-server-list-item" : "remote-server-list-item"}">
      <div class="detail-row">
        <span class="status-mark ${className}" ${statusMarkAttributes(className)} aria-hidden="true">${iconSvg(active ? "check" : "activity")}</span>
        <div>
...
function renderServerActionsMenu(server, active) {
  ...
  const items = [
    moreMenuButton({
      label: active ? "Active server" : "Switch to",
      icon: active ? "check" : "activity",
      attrs: `data-select-server="${key}"`,
      disabled: active,
    }),
    moreMenuButton({
  ...
  return `
    <details class="server-action-menu" data-server-action-menu data-overlay-menu data-overlay-key="server:${key}">
      ...
      <div class="server-action-popover ds-overlay-surface" data-overlay-kind="menu">
        ${moreMenuHtml(items)}
      </div>
    </details>
  `;
...
function renderServerTokenForm(server) {
  ...
  return `
    <form class="server-token-form ds-form-layout" data-ds-form="settings" data-server-token-form="${escapeAttr(server.key)}">
      <label class="ds-field" for="${escapeAttr(fieldId)}">
        <span class="ds-field__label">Remote token</span>
        <input class="ds-input" id="${escapeAttr(fieldId)}" type="password" name="token" autocomplete="new-password" placeholder="Token for ${escapeAttr(server.name || server.key)}" aria-describedby="${escapeAttr(helpId)}" required>
        <span class="ds-field__description" id="${escapeAttr(helpId)}">Stored only in this browser and sent to the selected Remote server.</span>
      </label>
      <div class="server-connection-actions ds-form-actions">
        <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit">${iconSvg("check")}<span>Save token</span></button>
      </div>
    </form>
  `;
}
...
function renderAddServerForm() {
  return `
    <form id="server-connection-form" class="server-connection-form ds-form-layout" data-ds-form="settings">
      <div class="server-form-heading ds-form-section-heading">
        <strong>Add server</strong>
        <span>Loopback or tailnet peers</span>
      </div>
      <div class="server-connection-grid">
        <label class="ds-field" for="server-name-input">
          <span class="ds-field__label">Name</span>
          <input class="ds-input" id="server-name-input" type="text" name="name" placeholder="tycho-peer" autocomplete="off" aria-describedby="server-name-input-help">
          <span class="ds-field__description" id="server-name-input-help">Optional label shown in the server list.</span>
        </label>
        <label class="ds-field" for="server-url-input">
          <span class="ds-field__label">URL</span>
          <input class="ds-input" id="server-url-input" type="text" name="url" inputmode="url" placeholder="http://127.0.0.1:7374/" autocomplete="url" aria-describedby="server-url-input-help" required>
          <span class="ds-field__description" id="server-url-input-help">Loopback or Tailscale URL for a Tycho Remote server.</span>
        </label>
        <label class="ds-field" for="server-token-input">
          <span class="ds-field__label">Remote token</span>
          <input class="ds-input" id="server-token-input" type="password" name="token" autocomplete="new-password" placeholder="Optional" aria-describedby="server-token-input-help">
          <span class="ds-field__description" id="server-token-input-help">Optional. Stored only in this browser.</span>
        </label>
      </div>
      <div class="server-connection-actions ds-form-actions">
        <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit">${iconSvg("plus")}<span>Add server</span></button>
      </div>
    </form>
  `;
}
...
function renderPushSetup(setup) {
  ...
  const pushActions = enabledCount > 0
    ? `
      ...
      <button class="danger ds-button" data-variant="danger" type="button" data-disable-push ${support.supported ? "" : "disabled"}>Disable notifications</button>
    `
    : `<button class="primary inline-icon-button ds-button" data-variant="brand" type="button" data-enable-push ${canEnable ? "" : "disabled"}>${iconSvg("check")}<span>Enable notifications</span></button>`;
  ...
  return `
    <div class="focused-view-toolbar">
      <details class="focused-view-menu" data-focused-view-menu data-overlay-menu data-overlay-key="focused-view:${escapeAttr(key)}" data-active-view-mode="${escapeAttr(activeMode)}" data-state-key="focused-view-menu:${escapeAttr(key)}">
        <summary class="ds-detail-header__action" aria-label="View layout: ${escapeAttr(active.label)}" title="View layout: ${escapeAttr(active.label)}" aria-haspopup="menu" data-view-icon="${escapeAttr(active.icon)}">
          ${iconSvg(active.icon)}
          <span class="sr-only">${escapeHtml(active.label)}</span>
        </summary>
        <div class="focused-view-menu-popover ds-overlay-surface" data-overlay-kind="menu">
...
function renderSummaryAttachmentMenu(agent, attachments = [], summaryId = "") {
  ...
  return `
    <details class="summary-attachment-menu" data-summary-attachment-menu data-overlay-menu data-overlay-key="summary-attachments:${escapeAttr(summaryId || agent.key)}"${stateKey}>
      ...
      <div class="summary-attachment-menu-popover ds-overlay-surface" data-overlay-kind="menu">
...
function tychoLoadingState(label, options = {}) {
  ...
  const className = ["empty-state", "ds-empty-state", "ds-loading-state", "tycho-loading-state", options.className || ""].filter(Boolean).join(" ");
...
function renderPullRequestDiffToolbar(agent, data) {
  ...
  const refreshAllBtn = fetching
    ...
    : `<button class="inline-icon-button ds-button" type="button" data-refresh-all-pr-diffs="${escapeAttr(agent.key)}" ${disableButtons ? "disabled" : ""}>${iconSvg("rotateCcw")}<span>Fetch all diffs</span></button>`;
  return `
    <section class="diff-toolbar pr-diff-toolbar" aria-label="Pull request diff actions">
      <button class="inline-icon-button ds-button" type="button" data-refresh-agent-prs="${escapeAttr(agent.key)}" ${disableButtons ? "disabled" : ""}>${iconSvg("rotateCcw")}<span>Refresh metadata</span></button>
      ${refreshAllBtn}
      <button class="inline-icon-button ds-button" type="button" data-toggle-pr-diff-full-view="${escapeAttr(agent.key)}">${iconSvg("fileText")}<span>${fullView ? "Exit full view" : "Full view"}</span></button>
    </section>
  `;
}
...
function renderPullRequestDiffDetail(agent, item, diff, options = {}) {
  ...
  const snapshotActions = snapshot ? `
    <button class="inline-icon-button ds-button" type="button" data-refresh-pr-diff="${escapeAttr(item.id)}" data-agent-key="${escapeAttr(agent.key)}" ${loading ? "disabled" : ""}>${iconSvg("rotateCcw")}<span>Refresh</span></button>
    <button class="inline-icon-button ds-button" type="button" data-toggle-pr-diff-expand-all="${escapeAttr(agent.key)}" ${loading ? "disabled" : ""}>${iconSvg(expandIcon)}<span>${expandLabel}</span></button>
  ` : "";
  const header = `
    <section class="summary-card diff-summary-card pr-diff-summary">
      <div class="card-title pr-diff-title-row">
        ...
        <div class="pr-diff-title-actions">
          ${snapshotActions}
          ${item.url ? `<a class="inline-icon-button ds-button" href="${escapeAttr(item.url)}" target="_blank" rel="noreferrer">${iconSvg("externalLink")}<span>Open PR</span></a>` : ""}
        </div>
      </div>
    </section>
  `;
  ...
  if (!snapshot) return `${header}${loading ? tychoLoadingState("Loading PR diff", { className: "pr-diff-loading-state" }) : `
    <section class="empty-state ds-empty-state pr-diff-empty-state" data-state="empty">
      <strong>Diff not fetched</strong>
      <p>Fetch this pull request diff to inspect it in Tycho.</p>
      <button class="primary inline-icon-button ds-button" data-variant="brand" type="button" data-refresh-pr-diff="${escapeAttr(item.id)}" data-agent-key="${escapeAttr(agent.key)}">${iconSvg("rotateCcw")}<span>Fetch diff</span></button>
    </section>
  `}`;
...
function renderAgentComposer(agent, skills, options = {}) {
  ...
  const composer = `
    <form id="composer" class="composer${fullScreen ? " composer-full-screen" : ""}" data-agent-key="${escapeAttr(agent.key)}" data-agent-running="${agentIsRunning(agent) ? "true" : "false"}"${modalAttributes}>
      ${fullScreen ? `<button class="icon-button composer-close-button ds-button ds-icon-button" type="button" data-toggle-composer-full-screen="${escapeAttr(agent.key)}" aria-label="Close full-screen editor" title="Close full-screen editor">${iconSvg("x")}</button>` : ""}
...
function renderInquiryForm(agent, options = {}) {
  ...
  return `
    <form id="inquiry-form" class="inquiry-form${fullScreen ? " inquiry-form-full-screen" : ""}" data-ds-form="inquiry" data-agent-key="${escapeAttr(agent.key)}" data-inquiry-id="${escapeAttr(inquiryId)}" aria-describedby="inquiry-validation" novalidate${modalAttributes}>
      ${fullScreen
        ? `<button class="icon-button inquiry-close-button ds-button ds-icon-button" type="button" data-toggle-inquiry-full-screen="${escapeAttr(agent.key)}" aria-label="Close full-screen inquiry" title="Close full-screen inquiry">${iconSvg("x")}</button>`
        : `<button class="inquiry-full-screen-button ds-button ds-icon-button" type="button" data-toggle-inquiry-full-screen="${escapeAttr(agent.key)}" aria-label="Open full-screen inquiry" title="Full-screen inquiry">${iconSvg("maximize2")}</button>`}
      <div class="inquiry-form-content">
        ...
        <footer class="inquiry-footer">
          ...
          <div class="send-row">
            ...
            <div class="send-actions">
              <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit" data-agent-key="${escapeAttr(agent.key)}" disabled>${iconSvg("check")}<span>Send answer</span></button>
            </div>
          </div>
        </footer>
      </div>
...
function renderInquiryFeedbackField() {
  return `
    <section class="field-card inquiry-field inquiry-feedback-field ds-field ds-surface" data-inquiry-feedback-field>
      <label class="ds-field__label" for="inquiry-feedback"><span class="inquiry-field-label">Leave feedback</span> <span class="inquiry-requirement">Optional</span></label>
      <textarea class="ds-input" id="inquiry-feedback" name="feedback" rows="3" data-inquiry-feedback aria-describedby="inquiry-feedback-help" placeholder="Add anything else you want the agent to know."></textarea>
      <span class="field-hint ds-field__description" id="inquiry-feedback-help">Included with the structured answer as additional context.</span>
    </section>
  `;
}
...
function renderInquiryField(field, index) {
  ...
  const label = type === "multi_select"
    ? `<div class="ds-field__label inquiry-group-label" id="${escapeAttr(labelId)}"><span class="inquiry-field-label">${escapeHtml(field.label || key)}</span> <span class="inquiry-requirement${field.required ? " required" : ""}">${escapeHtml(requirement)}</span></div>`
    : `<label class="ds-field__label" id="${escapeAttr(labelId)}" for="${escapeAttr(id)}"><span class="inquiry-field-label">${escapeHtml(field.label || key)}</span> <span class="inquiry-requirement${field.required ? " required" : ""}">${escapeHtml(requirement)}</span></label>`;
  return `
    <section class="field-card inquiry-field ds-field ds-surface" data-inquiry-field="${escapeAttr(key)}">
      ${label}
      ${field.description ? `<span class="field-hint ds-field__description" id="${escapeAttr(descriptionId)}">${escapeHtml(field.description)}</span>` : ""}
      ${input}
      <span class="inquiry-field-error ds-field__error" id="${escapeAttr(errorId)}" data-inquiry-field-error></span>
    </section>
  `;
}
...
function renderInquiryInput(field, id, key, required, relationships = {}) {
  ...
  if (type === "select" || type === "boolean") {
    ...
    return `
      <select class="ds-input" id="${escapeAttr(id)}" name="${escapeAttr(key)}" data-inquiry-control${describedBy}${errorMessage} ${required}>
  ...
  if (type === "number" || type === "integer") {
    return `<input class="ds-input" id="${escapeAttr(id)}" name="${escapeAttr(key)}" type="number" data-inquiry-control${describedBy}${errorMessage} ${type === "integer" ? 'step="1"' : ""} ${required}>`;
  }

  if (type === "multiline") {
    return `<textarea class="ds-input" id="${escapeAttr(id)}" name="${escapeAttr(key)}" rows="4" data-inquiry-control${describedBy}${errorMessage} ${required}></textarea>`;
  }

  return `<input class="ds-input" id="${escapeAttr(id)}" name="${escapeAttr(key)}" type="text" data-inquiry-control${describedBy}${errorMessage} ${required}>`;
}
...
function agentComposerAction(agent) {
  if (agentIsRunning(agent)) {
    return `<button class="danger ds-button" data-variant="danger" type="button" data-agent-action="stop" data-agent-key="${escapeAttr(agent.key)}">Stop agent</button>`;
  }

  return `<button class="primary ds-button" data-variant="brand" type="submit" data-agent-key="${escapeAttr(agent.key)}">Send prompt</button>`;
}
...
function renderAgentForm(route) {
    ...
    replaceView(feedbackMessage(
      ...
      `Running agents keep their current process state locked. Stop this agent before ${action} it.`,
      {
        ...
        actions: `<div class="button-row">
          ...
          <button class="primary ds-button" data-variant="brand" type="button" data-agent-action="stop" data-agent-key="${escapeAttr(agent.key)}">Stop agent</button>
        </div>`,
      }
    ));
    return;
  ...
  replaceView(`
    <form id="agent-form" class="agent-form ds-form-layout" data-ds-form="lifecycle" data-mode="${editing ? "edit" : cloning ? "clone" : "create"}" data-project-key="${escapeAttr(project.key)}" data-agent-key="${escapeAttr(agent?.key || "")}" data-project-name="${escapeAttr(project.name || project.key)}">
      <div class="form-section-heading ds-form-section-heading"><strong>Identity and workspace</strong><span>Name and execution context</span></div>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="agent-name">Name</label>
        <input class="ds-input" id="agent-name" name="name" type="text" value="${escapeAttr(values.name)}" autocomplete="off" required>
      </section>
      <section class="detail-card ds-surface">
        <div class="detail-card-body" style="padding-top: 12px;">
          <div class="detail-row">
      ...
      <details class="agent-advanced-config ds-surface">
        <summary>
          <span>
        ...
        <div class="agent-advanced-fields">
          <section class="field-card ds-field ds-surface">
            <label class="field-label ds-field__label" for="agent-template">Prompt Template</label>
            <select class="ds-input" id="agent-template" name="template_key" data-agent-template-select aria-describedby="agent-template-help">
              ${templates.map((template) => agentTemplateOptionHtml(template, template.key === values.templateKey)).join("")}
            </select>
            <span class="field-hint ds-field__description" id="agent-template-help">${escapeHtml(selectedTemplate.prompt_preview || "Template defaults are loaded from the project configuration.")}</span>
          </section>
          <section class="field-card ds-field ds-surface">
            <label class="field-label ds-field__label" for="agent-response-style">Response style</label>
            <select class="ds-input" id="agent-response-style" name="response_style_mode" data-agent-response-style-select aria-describedby="agent-response-style-help">
              ${agentResponseStyleChoiceOptions(templateResponseStyleSource(selectedTemplate), values.responseStyleMode)}
            </select>
            <span class="field-hint ds-field__description" id="agent-response-style-help">Independent from Prompt Template. Global uses the active policy from Settings.</span>
          </section>
          <section class="field-card ds-field ds-surface">
            <label class="field-label ds-field__label" for="agent-harness">Harness</label>
            <select class="ds-input" id="agent-harness" name="agent" data-agent-harness-select>
          ...
          <section class="field-card ds-field ds-surface">
            <label class="field-label ds-field__label" for="agent-model">Model</label>
            <select class="ds-input" id="agent-model" name="model" data-agent-model-select>
              ${modelChoiceOptions(values.harness, values.model)}
            </select>
          </section>
          <section class="field-card ds-field ds-surface">
            <label class="field-label ds-field__label" for="agent-reasoning-effort">Effort</label>
            <select class="ds-input" id="agent-reasoning-effort" name="reasoning_effort" data-agent-reasoning-effort-select>
              ${reasoningEffortChoiceOptions(values.harness, values.model, values.reasoningEffort)}
            </select>
          </section>
        </div>
      </details>
      <div class="form-section-heading ds-form-section-heading"><strong>Instructions</strong><span>Prompt sent to the agent</span></div>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="agent-prompt">Prompt</label>
        <textarea class="ds-input" id="agent-prompt" name="prompt" rows="9" required>${escapeHtml(values.prompt)}</textarea>
      </section>
      ...
      <div class="button-row form-actions ds-form-actions">
        <button class="ds-button" type="button" data-cancel-agent-form>Cancel</button>
        ${editing ? `
          <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit" value="save">${iconSvg("pencil")}<span>Save agent</span></button>
        ` : cloning ? `
          <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit" value="clone">${iconSvg("copyPlus")}<span>Clone and archive</span></button>
        ` : `
          <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit" value="create-run">${iconSvg("loaderPinwheel")}<span>Create and run</span></button>
          <details class="secondary-submit-menu">
            <summary>More options</summary>
            <button class="inline-icon-button ds-button" type="submit" value="create">${iconSvg("plus")}<span>Create without running</span></button>
          </details>
        `}
      </div>
    </form>
...
function renderAgentArchive(key) {
  ...
  replaceView(`
    <section class="notice archive-dialog">
      ...
      <div class="button-row form-actions ds-form-actions archive-actions">
        <button class="danger inline-icon-button ds-button" data-variant="danger" type="button" data-agent-action="archive" data-agent-key="${escapeAttr(agent.key)}">${iconSvg("archive")}<span>Archive</span></button>
        <button class="inline-icon-button ds-button" type="button" data-clone-agent="${escapeAttr(agent.key)}">${iconSvg("copyPlus")}<span>Clone instead</span></button>
...
function renderAgentLoopForm(key) {
  ...
  replaceView(`
    <form id="agent-loop-form" class="agent-form ds-form-layout agent-loop-form" data-agent-key="${escapeAttr(agent.key)}">
      ...
      <div class="form-section-heading ds-form-section-heading"><strong>Loop timing</strong><span>Server local time</span></div>
      ...
      <div class="form-section-heading ds-form-section-heading"><strong>Run prompt</strong><span>Preview and refine before starting</span></div>
      <section class="field-card">
      ...
      <div class="button-row form-actions ds-form-actions">
        ...
        <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit">${iconSvg("rotateCcw")}<span>Start loop now</span></button>
      </div>
    </form>
  `);
}
  ...
  replaceView(`
    ...
    <section class="agent-group project-overview-list">
      <div class="agent-row project-overview-row">
        ...
        ${statusBadge(titleFromKey(project.status || "configured"), projectStatusClass(project))}
      </div>
      <div class="agent-row project-overview-row">
...
function renderProjectDiffScopeSwitch(project, currentScope, options = {}) {
  ...
  return `
    <section class="diff-toolbar" aria-label="Diff scope">
      ...
      <button class="inline-icon-button ds-button" type="button" data-toggle-project-diff-expand-all="${escapeAttr(projectDiffKey(project.key, currentScope))}">${iconSvg(expandIcon)}<span>${expandLabel}</span></button>
      <button class="inline-icon-button ds-button" type="button" data-refresh>${iconSvg("rotateCcw")}<span>Refresh</span></button>
    </section>
  `;
}
...
function renderProjectForm(key) {
  ...
  replaceView(`
    <form id="project-form" class="agent-form ds-form-layout project-form" data-ds-form="lifecycle" data-project-key="${escapeAttr(project.key)}">
      <section class="detail-card ds-surface">
      ...
      <div class="form-section-heading ds-form-section-heading"><strong>Editable defaults</strong><span>Applied to new agents</span></div>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="project-name">Name</label>
        <input class="ds-input" id="project-name" name="name" type="text" value="${escapeAttr(values.name)}" autocomplete="off" required>
      </section>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="project-group">Group</label>
        <input class="ds-input" id="project-group" name="group" type="text" value="${escapeAttr(values.group)}" autocomplete="off" list="project-group-options">
        <datalist id="project-group-options">
          ${groups.map((group) => `<option value="${escapeAttr(group)}"></option>`).join("")}
        </datalist>
      </section>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="project-harness">Default harness</label>
        <select class="ds-input" id="project-harness" name="agent" data-project-harness-select>
          ${agentHarnessOptions().map((option) => `<option value="${escapeAttr(option)}" ${option === values.agent ? "selected" : ""}>${escapeHtml(option)}</option>`).join("")}
        </select>
      </section>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="project-model">Default model</label>
        <select class="ds-input" id="project-model" name="model" data-project-model-select>
          ${modelChoiceOptions(values.agent, values.model)}
        </select>
      </section>
      <section class="field-card ds-field ds-surface">
        <label class="field-label ds-field__label" for="project-reasoning-effort">Default effort</label>
        <select class="ds-input" id="project-reasoning-effort" name="reasoning_effort" data-project-reasoning-effort-select>
          ${reasoningEffortChoiceOptions(values.agent, values.model, values.reasoningEffort)}
        </select>
      </section>
      <div class="button-row form-actions ds-form-actions">
        <button class="ds-button" type="button" data-cancel-project-form>Cancel</button>
        <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit" value="save">${iconSvg("pencil")}<span>Save project</span></button>
      </div>
    </form>
  `);
}
  ...
  return `
    <section class="agent-group project-agent-group">
      <div class="group-title">
        ...
        <button class="inline-icon-button agent-group-create ds-button" type="button" data-create-agent="${escapeAttr(project.key)}">${iconSvg("plus")}<span>New agent</span></button>
      </div>
      ${agents.map(renderAgentRow).join("")}
    </section>
  `;
...
function renderAgentGroup(projectKey, agents) {
  ...
  return `
    <section class="agent-group ds-collection-group ds-surface">
      <div class="group-title">
        <div class="agent-group-project">
          ${renderAgentGroupProjectTitle(projectKey, projectName, !!project)}
          <span>${escapeHtml(projectMeta)}</span>
        </div>
        ${project ? `<button class="inline-icon-button agent-group-create ds-button" type="button" data-create-agent="${escapeAttr(projectKey)}" aria-label="New agent for ${escapeAttr(projectName)}">${iconSvg("plus")}<span>New agent</span></button>` : ""}
      </div>
      ${agents.map(renderAgentRow).join("")}
    </section>
  `;
...
function renderAgentRow(agent) {
  ...
  return `
    <button class="agent-row" type="button" data-open-agent="${escapeAttr(agent.key)}">
      ...
      ${agent.unread ? statusBadge("Unread", "need") : statusBadge(statusLabel(agent), statusClass(agent))}
    </button>
  `;
}
...
function renderSelectableAgentRow(agent) {
  ...
  return `
    <label class="agent-row selectable-agent-row ${selected ? "selected" : ""} ${disabled ? "disabled" : ""}">
      ...
      ${statusBadge(disabled ? "Running" : titleFromKey(agent.last_result || "archiveable"), disabled ? "detail" : statusClass(agent))}
    </label>
  `;
}
...
function renderRunSummaryMessageContent(content, agent, block = {}, options = {}) {
  ...
  const openButton = agentKey
    ? `<button class="inline-icon-button summary-message-open ds-button" type="button" data-open-agent-summary="${escapeAttr(agentKey)}" data-open-agent-summary-id="${escapeAttr(summaryId)}" aria-label="Open full summary">${iconSvg("scanText")}<span>Open</span></button>`
    : "";
...
function renderPendingAttachment(agentKey, attachment) {
  ...
  return `
    <div class="pending-attachment" data-pending-attachment="${escapeAttr(attachment.id)}">
      ...
      <button class="icon-button ds-button ds-icon-button" type="button" data-remove-prompt-attachment="${escapeAttr(attachment.id)}" data-agent-key="${escapeAttr(agentKey)}" aria-label="Remove attachment" title="Remove attachment">${iconSvg("x")}</button>
    </div>
  `;
}
...
function renderAttachment(attachment) {
  ...
  const actions = `
    <span class="attachment-actions">
      ${download && id ? `<button class="icon-button ds-button ds-icon-button" type="button" data-download-attachment="${escapeAttr(id)}" data-download-filename="${escapeAttr(title || "attachment")}" aria-label="Download attachment" title="Download attachment">${iconSvg("download")}</button>` : ""}
      ${id ? `<button class="icon-button danger ds-button ds-icon-button" data-variant="danger" type="button" data-delete-attachment="${escapeAttr(id)}" aria-label="Delete attachment" title="Delete attachment">${iconSvg("trash2")}</button>` : ""}
    </span>
  `;
...
function attachmentViewerHtml(id, options = {}) {
    ...
    return `
      <section class="${sectionClass}"${sectionState}>
        <div class="${bodyClass}">
          ...
          ${href ? `<a class="button primary button-link ds-button" data-variant="brand" href="${escapeAttr(href)}" target="_blank" rel="noreferrer">Open link</a>` : ""}
        </div>
      </section>
    `;
  }
  ...
  const download = attachmentDownloadHref(attachment);
  const downloadButton = download && id ? `<button class="button primary button-link ds-button" data-variant="brand" type="button" data-download-attachment="${escapeAttr(id)}" data-download-filename="${escapeAttr(title || "attachment")}">Download file</button>` : "";
  const openFileButton = attachment.blob_path ? `<a class="button primary button-link ds-button" data-variant="brand" href="${escapeAttr(attachment.blob_path)}" target="_blank" rel="noreferrer">Open file</a>` : "";
  ...
  return `
    <section class="${sectionClass}"${sectionState}>
      <div class="${bodyClass}">
        <div class="attachment-viewer-heading">
          <div class="section-label"><strong>${escapeHtml(format === "markdown" ? "Markdown" : "File")}</strong>${options.embedded ? "" : `<span>${escapeHtml(agent?.name || attachmentKindLabel(kind))}</span>`}</div>
          ${actions}
        </div>
        ${target ? `<p class="attachment-target wrap-anywhere">${escapeHtml(attachmentTargetLabel(target))}</p>` : ""}
  ...
  return `
    <div class="attachment-viewer-actions">
      <details class="attachment-viewer-menu" data-attachment-viewer-menu data-overlay-menu data-overlay-key="attachment-viewer:${escapeAttr(id || target)}" data-state-key="attachment-viewer-menu:${escapeAttr(id || target)}">
        ...
        <div class="attachment-viewer-menu-popover ds-overlay-surface" data-overlay-kind="menu">
          ${moreMenuHtml(items)}
        </div>
      </details>
    </div>
...
function emptyState(title, body, options = {}) {
  const className = ["empty-state", "ds-empty-state", options.className || ""].filter(Boolean).join(" ");
...
function feedbackMessage(title, body, options = {}) {
  ...
  const className = ["notice", "ds-alert", options.className || ""].filter(Boolean).join(" ");
  ...
  return `
    <section class="${escapeAttr(className)}" data-intent="${intent}" ${announcement}>
      <div>
        <strong class="ds-alert__title">${escapeHtml(title)}</strong>
        <p>${escapeHtml(body)}</p>
        ${options.actions || ""}
      </div>
    </section>
...
function kv(label, value, options = {}) {
  ...
  const copyButton = options.copyable
    ? `
      <button class="icon-button kv-copy-button ds-button ds-icon-button" type="button" data-copy="${copyData}" aria-label="Copy ${copyLabel}" title="Copy ${copyLabel}" ${disabledAttr}>
        ${iconSvg("copy")}
      </button>
    `
    : "";
...
function renderAgentScheduleMenu(agent, schedule) {
  ...
  return `
    <details class="header-schedule-control ${scheduledAgentIconStatusClass(agent, schedule)}" data-header-schedule-menu data-overlay-menu data-overlay-key="header-schedule:${escapeAttr(agent.key)}" data-agent-key="${escapeAttr(agent.key)}">
      <summary class="ds-detail-header__action" aria-label="Schedule actions" title="Schedule actions" aria-haspopup="menu">
        ${iconSvg("calendarCheck2")}
      </summary>
      <div class="header-schedule-popover ds-overlay-surface" role="menu" aria-label="Schedule actions for ${escapeAttr(scheduleName)}" data-overlay-kind="menu">
        ${moreMenuHtml(items)}
      </div>
    </details>
  `;
...
function confirmAction(options = {}) {
  ...
  dialog.innerHTML = `
    <section class="ds-confirmation-dialog__panel">
      <span class="ds-confirmation-dialog__mark" aria-hidden="true">!</span>
      <div class="ds-confirmation-dialog__content">
        <h2 class="ds-heading" data-level="2" id="confirmation-title">${escapeHtml(title)}</h2>
        <p class="ds-text" id="confirmation-description">${escapeHtml(description)}</p>
      </div>
      <div class="ds-confirmation-dialog__actions">
        <button class="ds-button" type="button" data-confirmation-cancel>${escapeHtml(cancelLabel)}</button>
        <button class="ds-button" data-variant="${intent === "danger" ? "danger" : "brand"}" type="button" data-confirmation-accept>${escapeHtml(confirmLabel)}</button>
      </div>
    </section>
  `;
...
function renderUnreadAgentsPanel(agents = quickSwitchAgents(), unreadCount = unreadAgents().length) {
  ...
  els.unreadPanel.innerHTML = `
    ...
    <form class="unread-panel-search" role="search" data-unread-agent-search-form>
      ...
      <button class="icon-button ds-button ds-icon-button ${state.unreadPanelQuery ? "" : "hidden"}" type="button" data-clear-unread-agent-search aria-label="Clear agent search" title="Clear agent search">${iconSvg("x")}</button>
    </form>
...
function statusBadge(label, className, kind = "pill") {
  ...
  return `<span class="${escapeAttr(kind)} ${escapeAttr(legacyClass)} ds-badge ds-status" data-intent="${escapeAttr(statusIntent(legacyClass))}">${escapeHtml(label)}</span>`;
}

function metadataBadge(label, kind = "pill") {
  const legacyKind = kind === "chip" ? "chip" : "pill";
  return `<span class="${legacyKind} detail ds-badge ds-metadata-badge" data-kind="metadata">${escapeHtml(label)}</span>`;
}
...
els.view.addEventListener("click", (event) => {
  ...
  const selectedServer = event.target.closest("[data-select-server]");
  if (selectedServer) {
    closeDetailsOverlay(selectedServer.closest("[data-server-action-menu]"));
    setActiveServerKey(selectedServer.dataset.selectServer);
    return;
  }
...
async function runAgentAction(agentAction) {
  ...
  if (confirmation) {
    const confirmed = await confirmAction({
      title: confirmation,
      description: "The agent state will change immediately.",
...
async function archiveSelectedAgents() {
  ...
  const confirmed = await confirmAction({
    ...
    description: "Archived agents leave the active workspace. Running agents are skipped and remain active.",
  ...
  els.quickAgentDialog.innerHTML = `
    <form id="quick-agent-form" class="quick-agent-form" method="dialog" data-project-key="${escapeAttr(project.key)}">
      <header class="quick-agent-header">
        ...
        <button type="button" class="icon-button ds-button ds-icon-button" data-quick-agent-close aria-label="Close">
      ...
      <footer class="quick-agent-actions">
        ...
        <button class="primary inline-icon-button ds-button" data-variant="brand" type="submit">${iconSvg("loaderPinwheel")}<span>Create and run</span></button>
      </footer>
    </form>
  `;
}
