...
function personalAssistantHandoffSection(label, value) {
  ...
  return `<div class="pa-handoff-section"><strong>${escapeHtml(label)}</strong>${items.length === 1 ? `<p>${escapeHtml(items[0])}</p>` : `<ul>${items.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>`}</div>`;
}
...
function personalAssistantHandoffHtml(handoff, { archived = false } = {}) {
  ...
  const summaryHtml = summary ? `<div class="pa-handoff-section"><strong>Summary</strong><p>${escapeHtml(String(summary))}</p></div>` : "";
...
function renderPersonalAssistantContinuity(item, proposals) {
  ...
  const trackedHtml = tracked.length ? `<p><strong>Tracked agents</strong></p><ul>${tracked.map((agent) => `<li><a href="${escapeAttr(routeHash({ type: "agent", key: agent.agent_key }))}">${escapeHtml(agent.name || agent.agent_key)}</a><span>${escapeHtml(agent.awaiting_input ? "needs input" : (agent.running ? "running" : (agent.status || agent.last_result || "prepared")))}</span></li>`).join("")}</ul>` : "";
  ...
  const historyRows = history.slice(0, 6).map((entry) => {
    ...
    return `<li><span>${escapeHtml(personalAssistantHistoryDate(entry))}</span><span>${escapeHtml(entry.summary || "Saved handoff")}</span><button class="ui-button" type="button" data-open-pa-history="${escapeAttr(entry.id)}">View saved summary</button>${conversationHref ? `<a class="ui-button" href="${escapeAttr(conversationHref)}">Open conversation</a>` : ""}</li>`;
  }).join("");
  const historyDetail = displayedHistory
    ? `<div class="pa-history-entry" aria-live="polite"><strong>Saved summary / handoff · read-only</strong>${personalAssistantHandoffHtml(displayedHistory, { archived: true })}<p>This saved handoff contains continuity fields, not a transcript. Return to the active conversation to continue.</p>${archivedConversationLink}<a class="ui-button" href="#personal-assistant" data-pa-back-active>Back to active FRED</a></div>`
    : state.personalAssistantHistoryLoading
      ? `<div class="pa-history-entry" aria-live="polite"><p class="pa-history-notice" role="status">Loading the saved summary…</p><a class="ui-button" href="#personal-assistant" data-pa-back-active>Back to active FRED</a></div>`
      : "";
  const historyHtml = history.length ? `<p><strong>Saved summaries and handoffs</strong></p><ul class="pa-history-list">${historyRows}</ul>${historyDetail}` : historyDetail;
  ...
  const settingsHtml = `<div class="pa-handoff-section"><strong>Daily boundary and settings</strong><p>Daily rollover uses ${escapeHtml(String(activeSettings.timezone || item.active_timezone || item.config?.timezone || "the configured timezone"))}; next handoff is ${escapeHtml(rolloverText)}.</p><p>Settings source: ${escapeHtml(settingsText || "the current server configuration")}. ${escapeHtml(item.settings_apply || "Changes apply to a new FRED conversation.")}</p></div>`;
  ...
  const expiredHtml = expired.length
    ...
    : "";
  return `<details class="pa-continuity"><summary>History and context <span>Next handoff: ${escapeHtml(rolloverText)}</span></summary><div class="pa-continuity-body"><div class="pa-handoff-section"><strong>Prior continuity</strong>${personalAssistantHandoffHtml(item.continuity)}</div>${settingsHtml}${trackedHtml}${expiredHtml}${historyHtml}${historyNotice}${recoveryHtml}</div></details>`;
}
