...
function serverDisplayName(server) {
  ...
  return server.local ? "This server" : (server.name || server.key || "Peer server");
}
...
function serverIdentityBadge(resource, options = {}) {
  ...
  return `<span class="server-identity-badge">
    ${metadataBadge(serverDisplayName(server), "chip")}
    ${statusBadge(serverHealthLabel(server), serverHealthClass(server), "chip")}
  </span>`;
}
...
function normalizeAttachmentResource(attachment, server, agentKey = "") {
  ...
  return {
    ...
    id: normalizedId,
    resource_id: rawId,
    agent_key: agentKey || attachment.agent_key,
    server_key: owner.key,
    server_name: owner.name || owner.key,
    server_local: Boolean(owner.local),
...
function applyResourceCatalog(data) {
  ...
  state.agents.forEach((agent) => {
    ...
    if (detail) Object.assign(detail, agent);
  });
...
function pollDelay() {
  ...
  if (state.servers.some((server) => server.refreshing || server.status === "loading")) return 500;
  const route = parseRoute();
...
async function refresh(options = {}) {
  ...
  try {
    ...
    const refreshAccepted = await requestAllServerResourceRefreshes();
    if (refreshAccepted) schedule(250);
  } catch (error) {
    state.failureCount += 1;
    if (error.status === 401) {
...
function renderSetup() {
  const setup = state.setup;
  setHeader("Settings", `This server / ${connectionText()}`, "S");
  if (!setup) {
...
function renderServerRow(server) {
  ...
  return `
    <div class="server-list-item ${server.local ? "local-server-list-item" : "remote-server-list-item"}">
      <div class="detail-row">
        <span class="status-mark ${className}" ${statusMarkAttributes(className)} aria-hidden="true">${iconSvg(server.local ? "home" : "activity")}</span>
        <div>
...
function renderAgentLoopForm(key) {
  ...
  if (agent.server_key !== "local") {
    ...
    replaceView(feedbackMessage(
      "Schedules belong to this server",
    ...
    <section class="${groupClassName}">
      <div class="group-title">
        <div class="agent-group-project">
          ...
          ${serverIdentityBadge(project || agents[0])}
        </div>
...
function renderAgentCard(agent) {
  return `
    <button class="card ${resourceStaleClass(agent)}" type="button" data-open-agent="${escapeAttr(agent.key)}">
      <div class="card-title">
        <strong>${escapeHtml(agent.name || agent.key)}</strong>
        <span>${escapeHtml(agentMeta(agent))}</span>
      </div>
      ${statusBadge(statusLabel(agent), statusClass(agent))}
      ${serverIdentityBadge(agent)}
...
function statusLabel(agent) {
  if (resourceIsStale(agent)) return "Stale";
...
function projectStatusLabel(project) {
  if (resourceIsStale(project)) return "Stale";
