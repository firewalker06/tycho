...
function renderPersonalAssistantSettings(assistant) {
  const item = assistant || { state: "unconfigured", configured: false, introduction: "Loading Personal Assistant…", config: {} };
  ...
  return `<section class="detail-card" id="settings-personal-assistant" data-personal-assistant-state="${escapeAttr(item.state)}">
    <div class="detail-card-body" style="padding-top: 12px;">
      <div class="section-label"><strong>Personal Assistant</strong><span>Tycho-owned daily conversation</span></div>
      ...
      ${item.configured ? `<button class="ui-button" type="button" data-open-personal-assistant ${active ? "disabled" : ""}>${active ? "Conversation active" : "Open today's conversation"}</button>` : `<p class="field-hint">Setup is available through the Personal Assistant API and requires an exact confirmation before Tycho saves it.</p>`}
...
function renderPersonalAssistantProposal(proposal) {
  ...
  return `<article class="pa-inline-action ${pending ? "pending" : "receipt"}" data-proposal-id="${escapeAttr(proposal.id)}" data-proposal-run="${escapeAttr(proposal.source_run_id || "")}" data-proposal-state="${escapeAttr(proposal.state)}">
    ...
    ${pending ? `<div class="pa-action-controls"><button class="ui-button" data-variant="brand" type="button" data-confirm-pa-proposal="${escapeAttr(proposal.id)}" ${busy ? "disabled" : ""}>${busy ? "Confirming…" : `Confirm ${escapeHtml(copy.label.toLowerCase())}`}</button><button class="ui-button" type="button" data-reject-pa-proposal="${escapeAttr(proposal.id)}" ${busy ? "disabled" : ""}>Reject</button></div>` : `<p class="pa-action-receipt" role="status">${escapeHtml(receipt)}</p>`}
...
function renderPersonalAssistant() {
  const item = state.personalAssistant || { state: "unconfigured", configured: false, config: {}, introduction: "Loading Personal Assistant…" };
  const proposals = state.personalAssistantProposals || [];
  setHeader("Personal Assistant", "", "A");
  ...
  const dock = agent && item.state === "active" ? renderAgentComposerPlacement(agent, [], {}).dock : `<form id="personal-assistant-start-form" class="pa-start-composer"><textarea name="prompt" rows="2" aria-label="Message Personal Assistant" placeholder="Message Personal Assistant"></textarea><button class="ui-button" data-variant="brand" type="submit" ${blocked ? "disabled" : ""}>${item.configured ? "Open chat" : "Send"}</button></form>`;
  replaceView(`<section class="personal-assistant-page" data-personal-assistant-state="${escapeAttr(item.state)}"><header class="pa-header"><strong><span class="pa-status-dot" aria-hidden="true"></span>Personal Assistant</strong>${settings}</header>${thread}<footer class="pa-composer-row">${dock}</footer></section>`);
}
...
function renderSetup() {
  ...
  const settingsSections = [
    ...
    ["personal-assistant", "Assistant", "settings-personal-assistant"],
...
function handleViewClick(event) {
  ...
  if (event.target.closest("[data-open-personal-assistant]")) {
    mutate(async () => {
      ...
      showGrowl("Personal Assistant opened", "done");
    });
    return;
  }
...
els.view.addEventListener("submit", (event) => {
  ...
  if (event.target.id === "personal-assistant-start-form") {
    ...
    mutate(async () => {
      ...
      showGrowl(assistant?.configured ? "Personal Assistant ready" : "Personal Assistant opened", "done");
    }, { form });
    return;
  }
