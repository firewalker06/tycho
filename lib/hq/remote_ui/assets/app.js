...
async function ensureProjectWorkspace(projectKey, path = "", options = {}) {
  ...
  const requestId = (state.projectWorkspaceRequests[key] || 0) + 1;
  state.projectWorkspaceRequests[key] = requestId;
  ...
  try {
    ...
    const data = await apiGet(`/projects/${encodeURIComponent(projectKey)}/workspace?${query.toString()}`);
    if (state.projectWorkspaceRequests[key] !== requestId) return;
...
async function ensureProjectWorkspacePreview(projectKey, path, force = false) {
  ...
  const requestId = (state.projectWorkspacePreviewRequests[key] || 0) + 1;
  state.projectWorkspacePreviewRequests[key] = requestId;
  state.projectWorkspacePreviews[key] = { path, loading: true };
  try {
    const query = new URLSearchParams({ path: String(path || "") });
    const data = await apiGet(`/projects/${encodeURIComponent(projectKey)}/workspace/preview?${query.toString()}`);
    if (state.projectWorkspacePreviewRequests[key] !== requestId) return;
...
function renderProjectWorkspaceListing(route, listing) {
  ...
  if (listing.error && !(listing.entries || []).length) {
    return workspaceFeedback(listing.errorCode, listing.error, "directory");
  }
  ...
  return `
    ...
    ${listing.next_offset !== null && listing.next_offset !== undefined ? `<button class="workspace-load-more ui-button" type="button" data-load-more-workspace="${escapeAttr(route.key)}" data-workspace-path="${escapeAttr(route.path || "")}" ${listing.loading ? "disabled" : ""}>${listing.loading ? "Loading…" : "Load more"}</button>` : ""}
...
function renderProjectWorkspacePreview(route, preview) {
  ...
  if (preview.error) {
    return `<div id="workspace-preview-title">${workspaceFeedback(preview.errorCode, preview.error, "file")}</div>`;
  }
  ...
  return `
    <div class="workspace-pane-header">
      ...
      <button type="button" data-close-workspace-preview aria-label="Close file preview">${iconSvg("x")}</button>
    </div>
...
function workspaceFeedback(code, message, kind) {
  const labels = {
...
els.view.addEventListener("click", (event) => {
  ...
  if (refreshWorkspaceButton) {
    const route = parseRoute();
    if (route.type === "projectWorkspace") {
      ensureProjectWorkspace(route.key, route.path, { force: true }).then(render);
    }
    return;
  }
  ...
  if (loadMoreWorkspaceButton) {
    ensureProjectWorkspace(
      loadMoreWorkspaceButton.dataset.loadMoreWorkspace,
      loadMoreWorkspaceButton.dataset.workspacePath || "",
      { append: true }
    ).then(render);
    return;
  }
