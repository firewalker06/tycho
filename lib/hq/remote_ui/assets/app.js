...
function renderPersonalAssistant() {
  ...
  const thread = agent && item.state === "active"
    ? `<div class="pa-conversation-scroll" data-preserve-scroll data-state-key="personal-assistant-thread">${renderAgentConversationView(agent, blocks, { floatingActions: false, loading: conversationLoading(agent.key), personalAssistant: true })}${inlineActions}</div>`
  ...
  replaceView(`<section class="personal-assistant-page" data-personal-assistant-state="${escapeAttr(item.state)}"><header class="pa-header"><button class="pa-tycho-nav" type="button" data-tab="agents" aria-label="Go to Tycho agents" title="Go to agents"><img class="brand-logo" src="/remote-logo.png?v=${escapeAttr(document.documentElement.dataset.assetVersion || "")}" alt=""></button><strong><img class="fred-avatar" src="/fred-avatar.png?v=${escapeAttr(document.documentElement.dataset.assetVersion || "")}" alt="FRED">FRED</strong>${settings}</header>${thread}<footer class="pa-composer-row">${dock}</footer></section>`);
}
...
function handleViewClick(event) {
  ...
  if (suggestion) {
    const input = els.view.querySelector("#personal-assistant-start-form textarea");
