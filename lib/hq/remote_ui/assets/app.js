...
function renderPullRequestDiffDetail(agent, item, diff, options = {}) {
  ...
  const viewerBody = reusablePullRequestDiffViewer(agent.key, item.id, viewerStateKey, viewerVersion)
    ? ""
    : files.length
...
function reusablePullRequestDiffViewer(agentKey, pullRequestId, stateKey, version) {
  if (!state.renderedViewHtml || state.renderedRouteKey !== routeStateKey(parseRoute())) return null;
  const viewer = document.querySelector(
    `[data-pr-diff-viewer][data-agent-key="${CSS.escape(agentKey)}"][data-pull-request-id="${CSS.escape(pullRequestId)}"]`
  );
  return viewer && pollContentKey(viewer) === `${stateKey}:${version}` ? viewer : null;
}
