const TOP_TABS = ["now", "personal-assistant", "agents", "settings"];
...
// Default tail window applied by the server to GET /agents/:key/conversation
// (see CONVERSATION_TAIL_DEFAULT_LIMIT in lib/hq/remote_server.rb). A huge
// finished agent's full transcript must never be fetched/rendered in one pass
// on initial load -- see Fizzy #226 (iOS Safari PWA content-process crash).
const CONVERSATION_DEFAULT_TAIL_LIMIT = 200;
...
const ICONS = {
  ...
  bellOff: `
    <svg class="ui-icon" aria-hidden="true" viewBox="0 0 24 24">
      ...
      <path d="M18.63 18H4a1 1 0 0 1-.74-1.67C4.59 14.96 6 13.5 6 9a6 6 0 0 1 .08-1"></path>
  ...
  thumbsUp: `
    <svg class="ui-icon" aria-hidden="true" viewBox="0 0 24 24">
      <path d="M7 10v12"></path><path d="M15 5.88 14 10h5.83a2 2 0 0 1 1.92 2.56l-2.33 8A2 2 0 0 1 17.5 22H4a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2.76a2 2 0 0 0 1.79-.89L12 2.26A2.5 2.5 0 0 1 15 5.88Z"></path>
    </svg>
  `,
  thumbsDown: `
    <svg class="ui-icon" aria-hidden="true" viewBox="0 0 24 24">
      <path d="M17 14V2"></path><path d="M9 18.12 10 14H4.17a2 2 0 0 1-1.92-2.56l2.33-8A2 2 0 0 1 6.5 2H20a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-2.76a2 2 0 0 0-1.79.89L12 21.74A2.5 2.5 0 0 1 9 18.12Z"></path>
    </svg>
  `,
  gitCommit: `
    <svg class="ui-icon" aria-hidden="true" viewBox="0 0 24 24">
...
const state = {
  ...
  projectWorkspaceSearchTerms: {},
  projectWorkspacePreviews: {},
  projectWorkspaceRequests: {},
  projectWorkspaceSearchRequests: {},
  projectWorkspaceSearchTimers: {},
  projectWorkspacePreviewRequests: {},
  projectWorkspaceImageUrls: {},
  projectWorkspaceImageErrors: {},
  projectWorkspaceEditing: {},
  pullRequests: {},
  pullRequestDiffs: {},
  ...
  personalAssistantHistoryLoading: false,
  personalAssistantHistoryError: "",
  personalAssistantSettingsEditing: false,
  personalAssistantProposals: [],
  personalAssistantActionPreflights: {},
  personalAssistantActionPreflightErrors: {},
  personalAssistantActionErrors: {},
  personalAssistantActionStateOverrides: {},
  ...
  pendingInquirySubmissionKeys: new Set(),
  pendingInquirySubmissionDrafts: new Map(),
  inquirySubmissionErrors: new Map(),
...
const els = {
  ...
  headerDetailClose: document.getElementById("header-detail-close"),
  ...
  desktopSettings: document.getElementById("desktop-settings"),
...
function navigateProjectWorkspace(button) {
  clearProjectWorkspaceSearch(button.dataset.openProjectWorkspace);
  navigate({
...
function currentTopTab(route) {
  if (route.type === "personalAssistant") return "personal-assistant";
...
function personalAssistantPollDelay() {
  ...
  const proposalWork = state.personalAssistantProposals?.some((proposal) =>
    ["queued", "executing", "verifying", "running"].includes(proposal?.state));
...
function applyPersonalAssistantItem(item) {
  ...
  if (previousContext && !personalAssistantSessionMatches(previousContext)) {
    ...
    state.personalAssistantActionPreflights = {};
    state.personalAssistantActionPreflightErrors = {};
    state.personalAssistantActionErrors = {};
    state.personalAssistantActionStateOverrides = {};
    state.personalAssistantHistoryEntry = null;
    state.personalAssistantHistoryLoading = false;
    state.personalAssistantHistoryError = "";
    state.personalAssistantRequests.coordinator.historySequence += 1;
    state.personalAssistantRequests.coordinator.historySelection = "";
...
function requestPersonalAssistantActions(routeHashAtStart = location.hash, context = null) {
  ...
  const coordinator = requests.coordinator;
  const sequence = ++coordinator.actionSequence;
  const request = personalAssistantRequest(requests.actions, requestKey, async () => {
    ...
    if (!current || sequence < coordinator.actionApplied) return data;
    coordinator.actionApplied = sequence;
    state.personalAssistantActionErrors = { ...state.personalAssistantActionErrors };
    delete state.personalAssistantActionErrors.read;
    state.personalAssistantProposals = Array.isArray(data.proposals) ? data.proposals : [];
    state.personalAssistantProposals.forEach((proposal) => {
      if (proposal?.preflight && typeof proposal.preflight === "object") {
        state.personalAssistantActionPreflights[proposal.id] = proposal.preflight;
      }
      if (proposal?.state && ["queued", "executing", "verifying", "executed", "failed", "rejected", "expired"].includes(proposal.state)) {
        delete state.personalAssistantActionStateOverrides[proposal.id];
      }
    });
    void requestPersonalAssistantPreflights(state.personalAssistantProposals, routeHashAtStart, context);
    return data;
  });
  return request.catch((error) => {
    if (location.hash === routeHashAtStart && parseRoute().type === "personalAssistant" &&
        (!context || personalAssistantSessionMatches(context))) {
      state.personalAssistantActionErrors = { read: error.message };
...
function requestPersonalAssistantPreflights(proposals, routeHashAtStart, context) {
  const pending = (Array.isArray(proposals) ? proposals : []).filter((proposal) => {
    const proposalState = String(proposal?.state || "").toLowerCase();
    return ["awaiting_confirmation", "queued", "executing", "verifying", "failed"].includes(proposalState) &&
      !state.personalAssistantActionPreflights[proposal.id] && !state.personalAssistantActionPreflightErrors[proposal.id];
  });
  return Promise.all(pending.map((proposal) => requestPersonalAssistantPreflight(proposal.id, { routeHashAtStart, context }).catch(() => null)));
}

function requestPersonalAssistantPreflight(proposalId, options = {}) {
  const id = String(proposalId || "").trim();
  if (!id) return Promise.resolve(null);
  const routeHashAtStart = options.routeHashAtStart || location.hash;
  const context = options.context || personalAssistantSessionContext();
  const requests = state.personalAssistantRequests.preflights;
  const requestKey = [context?.server_key || personalAssistantServerKey(), context?.active_key || "no-session", context?.generation ?? "no-generation", id].join(":");
  return personalAssistantRequest(requests, requestKey, async () => {
    const data = await apiGet(`/personal-assistant/actions/${encodeURIComponent(id)}/preflight`);
    const current = location.hash === routeHashAtStart && parseRoute().type === "personalAssistant" &&
      (!context || personalAssistantSessionMatches(context));
    if (!current) return data.preflight || data;
    const preflight = data.preflight || data;
    if (preflight && typeof preflight === "object") {
      state.personalAssistantActionPreflights[id] = preflight;
      delete state.personalAssistantActionPreflightErrors[id];
      state.renderedViewHtml = "";
      render({ preserveLiveEditor: true });
    }
    return preflight;
    ...
    if (location.hash === routeHashAtStart && parseRoute().type === "personalAssistant" &&
        (!context || personalAssistantSessionMatches(context))) {
      state.personalAssistantActionPreflightErrors[id] = error.message;
      state.renderedViewHtml = "";
      render({ preserveLiveEditor: true });
    }
    throw error;
...
async function refresh(options = {}) {
  ...
  try {
    ...
    if (refreshShell) {
      const [setupData, schedulesData, personalAssistantData] = await Promise.all([
        apiGet("/setup"),
        apiGet("/schedules"),
        apiGet("/personal-assistant"),
      ]);
      state.setup = setupData.setup || null;
      state.schedules = (schedulesData.schedules || []).map(normalizeLocalSchedule);
      state.scheduleDaemon = schedulesData.daemon || null;
...
async function ensureRouteData(options = {}) {
  ...
  if (route.type === "projectWorkspace") {
    ...
    if (route.file) await ensureProjectWorkspaceImage(route.key, route.file, options.forceProjectWorkspacePreview || options.force);
  }
...
function projectWorkspaceSearchKey(projectKey, query = "") {
  return `${projectKey}\u0000${String(query || "")}`;
}

function clearProjectWorkspaceSearch(projectKey) {
  const timer = state.projectWorkspaceSearchTimers[projectKey];
  if (timer) window.clearTimeout(timer);
  delete state.projectWorkspaceSearchTimers[projectKey];
  delete state.projectWorkspaceSearchTerms[projectKey];
}
...
async function ensureProjectWorkspaceSearch(projectKey, query, options = {}) {
  const normalizedQuery = String(query || "").trim();
  if (!normalizedQuery) return;

  const key = projectWorkspaceSearchKey(projectKey, normalizedQuery);
  const cached = state.projectWorkspaceSearches[key];
  if (!options.force && !options.append && cached && !cached.loading) return;

  const offset = options.append ? Number(cached?.next_offset || 0) : 0;
  const params = new URLSearchParams({ query: normalizedQuery, offset: String(offset), limit: "100" });
  await performProjectWorkspaceRequest({
    requests: state.projectWorkspaceSearchRequests,
    key,
    start: () => {
      state.projectWorkspaceSearches[key] = { ...(options.append ? cached : {}), query: normalizedQuery, loading: true, error: null };
    },
    fetchData: () => apiGet(`/projects/${encodeURIComponent(projectKey)}/workspace/search?${params.toString()}`),
    success: (data) => {
      const workspace = data.workspace || { query: normalizedQuery, entries: [] };
      state.projectWorkspaceSearches[key] = {
        ...workspace,
        entries: options.append ? [...(cached?.entries || []), ...(workspace.entries || [])] : (workspace.entries || []),
        loading: false,
        error: null,
...
function queueProjectWorkspaceSearch(projectKey, query) {
  const normalizedQuery = String(query || "").trim();
  const timer = state.projectWorkspaceSearchTimers[projectKey];
  if (timer) window.clearTimeout(timer);
  if (!normalizedQuery) {
  ...
  state.projectWorkspaceSearchTimers[projectKey] = window.setTimeout(() => {
    delete state.projectWorkspaceSearchTimers[projectKey];
    ensureProjectWorkspaceSearch(projectKey, normalizedQuery).then(render);
    render();
  }, 500);
}
...
async function ensureProjectWorkspaceImage(projectKey, path, force = false) {
  const key = projectWorkspaceKey(projectKey, path);
  const preview = state.projectWorkspacePreviews[key];
  if (preview?.format !== "image") return;
  if (!force && state.projectWorkspaceImageUrls[key]) return;
  if (force) revokeProjectWorkspaceImage(key);

  delete state.projectWorkspaceImageErrors[key];
  try {
    const query = new URLSearchParams({ path: String(path || "") });
    const blob = await apiBlob(`/projects/${encodeURIComponent(projectKey)}/workspace/image?${query.toString()}`);
    state.projectWorkspaceImageUrls[key] = URL.createObjectURL(blob);
  } catch (error) {
    state.projectWorkspaceImageErrors[key] = error.message;
  }
}

function revokeProjectWorkspaceImage(key) {
  const current = state.projectWorkspaceImageUrls[key];
  if (current) URL.revokeObjectURL(current);
  delete state.projectWorkspaceImageUrls[key];
}
      ...
      try {
        ...
        if (loadSnapshot) {
          receiveConversationSnapshot(
            ...
            {
              initial: true,
              digest: data.conversation_digest,
              totalBlockCount: data.conversation_block_count,
              hasOlder: data.conversation_has_older === true,
            }
          );
...
function renderCurrentView() {
  const route = parseRoute();
  // FRED is useful before a first project exists, so it remains reachable while
  // the project-only welcome screen is active.
  const onboarding = onboardingActive() && route.type !== "personalAssistant";
  const subpage = route.type !== "tab" && route.type !== "personalAssistant";
  const workspaceRoute = agentWorkspaceRoute(route);
  const personalAssistantRoute = route.type === "personalAssistant";
  ...
  els.desktopSettings.classList.toggle("hidden", onboarding || subpage);
  ...
  els.view.classList.toggle("agent-detail", !onboarding && Boolean(workspaceRoute || personalAssistantRoute));
  ...
  syncHeaderDetailCloseRoute(route);
  ...
  } else if (route.type === "personalAssistant") {
...
function captureViewState(preserved = new Map()) {
  ...
  document.querySelectorAll("[data-summary-attachment-menu-popover][data-state-key]").forEach((popover) => {
    snapshot.summaryAttachmentScroll[popover.dataset.stateKey] = {
      top: popover.scrollTop,
      left: popover.scrollLeft,
    };
  });
...
function restoreViewState(snapshot, preserved = new Map()) {
  ...
  document.querySelectorAll("[data-summary-attachment-menu-popover][data-state-key]").forEach((popover) => {
    const stored = snapshot.summaryAttachmentScroll?.[popover.dataset.stateKey];
    if (!stored) return;

    popover.scrollTop = stored.top || 0;
    popover.scrollLeft = stored.left || 0;
  });
...
function formStateKey(form) {
  ...
  const fredComposer = form.id === "composer" && parseRoute().type === "personalAssistant";
  if (form.dataset.agentKey && !fredComposer) parts.push(`agent:${form.dataset.agentKey}`);
...
function formDraftRouteKey(form) {
  ...
  if (agentKey && form.id === "composer" && parseRoute().type === "personalAssistant") return "personal-assistant";
...
function transferPersonalAssistantDraftArtifacts(previousKey, nextKey) {
  ...
  const attachments = state.pendingPromptAttachments[from];
  if (attachments?.length && !state.pendingPromptAttachments[to]?.length) {
    state.pendingPromptAttachments[to] = attachments;
    delete state.pendingPromptAttachments[from];
  }
}
...
function restoreFormDrafts() {
  els.view.querySelectorAll("form").forEach((form) => {
    ...
    if (!raw) {
      restoreInquirySubmissionDraft(form);
      return;
    }
...
function inquirySubmissionKey(agentKey, inquiryId) {
  const key = String(agentKey || "").trim();
  const id = String(inquiryId || "").trim();
  return key && id ? `${key}:${id}` : "";
}

function inquirySubmissionPending(agentKey, inquiryId) {
  const key = inquirySubmissionKey(agentKey, inquiryId);
  return Boolean(key && state.pendingInquirySubmissionKeys.has(key));
}

function inquirySubmissionError(agentKey, inquiryId) {
  const key = inquirySubmissionKey(agentKey, inquiryId);
  return key ? state.inquirySubmissionErrors.get(key) || "" : "";
}

function saveInquirySubmissionDraft(form) {
  const key = inquirySubmissionKey(form?.dataset?.agentKey, form?.dataset?.inquiryId);
  if (!key) return;

  const controls = Array.from(form.querySelectorAll("input:not([type='file']):not([type='hidden']), select, textarea"));
  state.pendingInquirySubmissionDrafts.set(key, controls.map((control) => ({
...
function restoreInquirySubmissionDraft(form) {
  const key = inquirySubmissionKey(form?.dataset?.agentKey, form?.dataset?.inquiryId);
  const draft = key ? state.pendingInquirySubmissionDrafts.get(key) : null;
  if (!draft) return;

  Array.from(form.querySelectorAll("input:not([type='file']):not([type='hidden']), select, textarea")).forEach((control, index) => {
    const value = draft[index];
    if (!value) return;
    if (value.type === "checkbox" || value.type === "radio") control.checked = Boolean(value.checked);
    else control.value = String(value.value ?? "");
  });
}

function focusInquirySubmissionStatus(inquiryId) {
  window.requestAnimationFrame(() => {
    els.view.querySelector(`[data-inquiry-submission-status="${CSS.escape(String(inquiryId || ""))}"]`)?.focus({ preventScroll: true });
  });
}

function focusInquiryForm(agentKey, inquiryId) {
  window.requestAnimationFrame(() => {
    const form = els.view.querySelector(`#inquiry-form[data-agent-key="${CSS.escape(String(agentKey || ""))}"][data-inquiry-id="${CSS.escape(String(inquiryId || ""))}"]`);
    form?.querySelector("input:not([type='hidden']), select, textarea")?.focus({ preventScroll: true });
  });
}

function restoreInquirySubmissionFailure(agentKey, inquiryId) {
  state.expandedInquiryKeys.add(`${agentKey}:${inquiryId}`);
  focusInquiryForm(agentKey, inquiryId);
}
    ...
    <div class="detail-row schedule-row">
      ...
      <div class="schedule-row-end">
        <div class="compact-actions schedule-row-actions">
          ...
          <button class="inline-icon-button schedule-archive-button ui-button" type="button" data-archive-schedule-session="${escapeAttr(schedule.key)}" aria-label="Archive ${escapeAttr(schedule.name || schedule.key)} session" title="Archive current session">${iconSvg("folderDown")}<span>Archive</span></button>
...
function renderScheduleDaemonActions(daemon, schedules) {
  ...
  const archiveAllItem = moreMenuButton({
    label: "Archive all sessions",
    sublabel: "Keep every schedule configured",
    icon: "folderDown",
    attrs: "data-archive-all-schedule-sessions",
  ...
  if (["running", "stale", "untracked"].includes(status)) {
    ...
    return `
      <details class="schedule-action-menu schedule-daemon-menu" data-overlay-menu data-overlay-key="schedule-daemon">
        <summary class="schedule-action-trigger" aria-label="Schedule list actions" title="Schedule list actions" aria-haspopup="menu">${iconSvg("ellipsis")}</summary>
        <div class="schedule-action-popover ui-overlay-surface" data-overlay-kind="menu">
          ${moreMenuHtml(items)}
        </div>
      </details>
  ...
  return `
    <button class="primary inline-icon-button ui-button" data-variant="brand" type="button" data-scheduler-action="start" ${hasSchedules ? "" : "disabled"}>${iconSvg("loaderPinwheel")}<span>Start daemon</span></button>
    <details class="schedule-action-menu schedule-daemon-menu" data-overlay-menu data-overlay-key="schedule-daemon">
      <summary class="schedule-action-trigger" aria-label="Schedule list actions" title="Schedule list actions" aria-haspopup="menu">${iconSvg("ellipsis")}</summary>
      <div class="schedule-action-popover ui-overlay-surface" data-overlay-kind="menu">
        ${moreMenuHtml([archiveAllItem])}
      </div>
    </details>
  `;
}
...
function settingsMoreMenuHtml(setup, key) {
  const routeActions = [
    moreMenuButton({
      label: "Settings",
      sublabel: key === "settings" ? "Current view" : "Open app settings",
      icon: "settings",
...
function personalAssistantConfigForm(item, { settings = false } = {}) {
  const config = item?.config || {};
  const model = String(config.model || "gpt-5.6-sol");
  const effort = String(config.reasoning_effort || "medium");
  const timezone = String(config.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC");
  const personality = String(config.personality || "balanced");
  const externalEventsPrompt = String(config.external_events_prompt || "");
  const personalityOptions = Array.isArray(item?.personality_options) && item.personality_options.length
    ? item.personality_options
    : [{ value: "balanced", label: "Balanced", description: "FRED’s clear, practical default." }];
  const modelOptions = modelSuggestionsForHarness("codex");
  const knownModels = modelOptions;
  const knownEfforts = reasoningEffortSuggestionsForModel("codex", model);
  const efforts = knownEfforts;
  const timezoneOptions = [...new Set([timezone, Intl.DateTimeFormat().resolvedOptions().timeZone, "Asia/Jakarta", "America/Los_Angeles", "America/New_York", "Europe/London", "UTC"].filter(Boolean))];
  const formId = settings ? "personal-assistant-settings-form" : "personal-assistant-setup-form";
  const verb = settings ? "Save FRED settings" : "Start FRED";
  const modelOptionHtml = knownModels.map((value) => {
    const normalized = String(value?.value || value || "").trim();
    const label = String(value?.label || normalized).trim();
    return normalized ? `<option value="${escapeAttr(normalized)}" ${normalized === model ? "selected" : ""}>${escapeHtml(label)}</option>` : "";
  }).join("") + (!knownModels.some((value) => String(value?.value || value || "").trim() === model) ? `<option value="${escapeAttr(model)}" selected>${escapeHtml(model)}</option>` : "");
  const effortOptionHtml = efforts.map((value) => `<option value="${escapeAttr(String(value))}" ${String(value) === effort ? "selected" : ""}>${escapeHtml(String(value))}</option>`).join("") + (!efforts.some((value) => String(value) === effort) ? `<option value="${escapeAttr(effort)}" selected>${escapeHtml(effort)}</option>` : "");
  const modelControl = knownModels.length ? `<select class="ui-input" id="${settings ? "pa-settings-model" : "pa-model"}" name="model" required data-pa-model-select>${modelOptionHtml}</select>` : `<input class="ui-input" id="${settings ? "pa-settings-model" : "pa-model"}" name="model" required value="${escapeAttr(model)}" autocomplete="off">`;
  const effortControl = efforts.length ? `<select class="ui-input" id="${settings ? "pa-settings-effort" : "pa-reasoning-effort"}" name="reasoning_effort" required data-pa-reasoning-effort>${effortOptionHtml}</select>` : `<input class="ui-input" id="${settings ? "pa-settings-effort" : "pa-reasoning-effort"}" name="reasoning_effort" required value="${escapeAttr(effort)}" autocomplete="off">`;
  const personalityOptionHtml = personalityOptions.map((option, index) => {
    const value = String(option?.value || "");
    const inputId = `${settings ? "pa-settings" : "pa"}-personality-${index}`;
    return `<label class="pa-personality-option" for="${escapeAttr(inputId)}"><input id="${escapeAttr(inputId)}" type="radio" name="personality" value="${escapeAttr(value)}" ${value === personality ? "checked" : ""} required><span><strong>${escapeHtml(String(option?.label || value))}</strong><small>${escapeHtml(String(option?.description || ""))}</small></span></label>`;
  }).join("");
  return `<form id="${formId}" class="ui-form-layout pa-config-form" aria-describedby="pa-setup-guidance">
    <div class="ui-field"><label class="ui-field__label" for="${settings ? "pa-settings-model" : "pa-model"}">Codex model</label>${modelControl}</div>
    <div class="ui-field"><label class="ui-field__label" for="${settings ? "pa-settings-effort" : "pa-reasoning-effort"}">Reasoning effort</label>${effortControl}</div>
    <div class="ui-field"><label class="ui-field__label" for="${settings ? "pa-settings-timezone" : "pa-timezone"}">Timezone <span class="field-label-note">(daily handoff)</span></label><input class="ui-input" id="${settings ? "pa-settings-timezone" : "pa-timezone"}" name="timezone" required list="pa-timezones" value="${escapeAttr(timezone)}" autocomplete="off"><datalist id="pa-timezones">${timezoneOptions.map((value) => `<option value="${escapeAttr(value)}"></option>`).join("")}</datalist></div>
    <fieldset class="ui-field pa-personality-field"><legend class="ui-field__label">Personality</legend><p class="field-hint" id="${settings ? "pa-settings-personality-hint" : "pa-personality-hint"}">Changes FRED’s stable voice and interaction style, not its safety or approval rules.</p><div class="pa-personality-options" aria-describedby="${settings ? "pa-settings-personality-hint" : "pa-personality-hint"}">${personalityOptionHtml}</div></fieldset>
    <div class="ui-field"><label class="ui-field__label" for="${settings ? "pa-settings-external-events" : "pa-external-events"}">External event recommendations <span class="field-label-note">(optional)</span></label><textarea class="ui-input" id="${settings ? "pa-settings-external-events" : "pa-external-events"}" name="external_events_prompt" rows="4" maxlength="4000" aria-describedby="pa-external-events-hint">${escapeHtml(externalEventsPrompt)}</textarea><p class="field-hint" id="pa-external-events-hint">FRED uses this on the server during the daily handoff. Clear it to disable external-event recommendations.</p></div>
    <label class="pa-confirmation"><input name="confirmed" type="checkbox" required> I confirm these settings for FRED.</label>
    <div class="ui-form-actions"><button class="ui-button" data-variant="brand" type="submit">${escapeHtml(verb)}</button>${settings ? '<button class="ui-button" type="button" data-cancel-personal-assistant-settings>Cancel</button>' : ""}</div>
  </form>`;
}

function personalAssistantReadiness(item) {
  const codex = harnessSetupItem("codex");
  if (!codex) return "Codex readiness will be checked when you send the first message.";
  if (codex.ready === false) return `Codex is not ready: ${codex.detail || "check the executable and try again."}`;
  if (codex.auth_ready === false) return "Codex is installed, but its account setup needs attention before the first reply.";
  return "Codex is available on this server. Account access is checked when it runs.";
}

function personalAssistantReadinessNotice() {
  const codex = harnessSetupItem("codex");
  if (!codex || (codex.ready !== false && codex.auth_ready !== false)) return "";
  return `<p class="field-hint pa-readiness-warning">${escapeHtml(personalAssistantReadiness())}</p>`;
}

function renderPersonalAssistantSettings(assistant) {
  const item = assistant || { state: "unconfigured", configured: false, introduction: "Loading FRED…", config: {} };
  const active = item.state === "active";
  const editing = state.personalAssistantSettingsEditing;
  return `<section class="detail-card" id="settings-personal-assistant" data-personal-assistant-state="${escapeAttr(item.state)}">
    <div class="detail-card-body" style="padding-top: 12px;">
      <div class="section-label"><strong>FRED</strong><span>Friendly Robot for Execution Dispatcher</span></div>
      ${statusBadge(item.state, active ? "done" : item.state === "unconfigured" ? "need" : "pending", "chip")}
      ${item.configured ? `<div class="kv-grid">
        ${copyableKv("Model", item.config?.model || "Choose Codex model")}
        ${copyableKv("Reasoning", item.config?.reasoning_effort || "Choose effort")}
        ${copyableKv("Timezone", item.config?.timezone || "IANA timezone required")}
        ${copyableKv("Personality", item.personality_options?.find((option) => option.value === item.config?.personality)?.label || "Balanced")}
        ${copyableKv("External events", item.config?.external_events_prompt || "Disabled")}
        ${copyableKv("Continuity", item.active_key ? `Daily session ${personalAssistantObservedTime(item.active_date)}` : "No active session")}
      </div>` : ""}
      ${personalAssistantReadinessNotice()}
      ${item.configured ? `<div class="pa-settings-actions"><a class="ui-button" href="#personal-assistant">Go to FRED</a><button class="ui-button" type="button" data-edit-personal-assistant-settings>${editing ? "Editing settings" : "Edit settings"}</button></div>${editing ? personalAssistantConfigForm(item, { settings: true }) : ""}${active ? `<details class="pa-lifecycle"><summary>Conversation lifecycle</summary><p class="field-hint">Restart FRED only to clear today’s context or apply changed settings now. Tycho archives the current thread and keeps a compact handoff.</p><button class="ui-button" type="button" data-restart-personal-assistant>Restart FRED with updated settings</button></details>` : ""}` : `<button class="ui-button" data-variant="brand" type="button" data-initiate-personal-assistant>Set up FRED</button>`}
      ${item.configured ? `<details class="pa-danger-zone"><summary>Reset FRED</summary><p class="field-hint">Deletes the active FRED session, its logs, setup, continuity, and pending actions. History retained by the server is described before confirmation.</p><button class="ui-button" data-variant="danger" type="button" data-reset-personal-assistant>Reset FRED</button></details>` : ""}
      ${item.settings_apply ? `<p class="field-hint">${escapeHtml(item.settings_apply)}</p>` : active ? "<p class=\"field-hint\">Changes apply to the next FRED conversation; this one keeps its launch settings.</p>" : ""}
    </div>
  </section>`;
}

function personalAssistantActionCopy(proposal) {
  const type = String(proposal?.type || "");
  const args = proposal?.arguments || {};
  const target = args.name || args.agent_key || args.project_key || args.path || args.query || "Tycho";
  const label = {
    install_or_update_tycho_skill: "Install or update skill", create_agent: "Create agent", message_agent: "Message agent",
    start_agent: "Start agent", stop_agent: "Stop agent", create_project: "Create project", update_project: "Update project",
    create_schedule: "Create schedule", pause_schedule: "Pause schedule", resume_schedule: "Resume schedule", inspect_schedules: "Inspect schedules", inspect_agent_run: "Inspect agent run", inspect_agent_log: "Inspect agent log",
  }[type] || titleFromKey(type);
  const effect = {
    install_or_update_tycho_skill: "Makes the skill available to Tycho.", create_agent: "Creates a new managed agent.",
    message_agent: "Sends this message to the agent.", start_agent: "Changes the agent from idle to running.",
    stop_agent: "Stops the running agent.", create_project: "Creates a project with these settings.", update_project: "Updates the project settings.",
    create_schedule: "Creates this schedule.", pause_schedule: "Pauses this schedule.", resume_schedule: "Resumes this schedule.", inspect_schedules: "Shows configured schedules.", inspect_agent_run: "Shows the latest run status.", inspect_agent_log: "Shows recent agent log output.",
  }[type] || proposal.description || "Makes the requested change.";
  return { label, target, effect };
}

function personalAssistantProposalInstruction(proposal) {
  const args = proposal?.arguments || {};
  if (proposal?.type === "create_agent") return args.prompt || proposal.description || "Create the configured agent.";
  if (proposal?.type === "message_agent") return args.prompt || proposal.description || "Send the configured message.";
  if (proposal?.type === "install_or_update_tycho_skill") return `${args.action || "Update"} the Tycho skill for ${args.harness || "the selected harness"}.`;
  return proposal?.description || "Perform the requested action.";
}

function personalAssistantProposalSettings(proposal) {
  const args = proposal?.arguments || {};
  const fields = proposal?.type === "create_agent"
    ? [["Project", args.project_key], ["Harness", args.agent], ["Model", args.model], ["Effort", args.reasoning_effort]]
    : proposal?.type === "create_project"
      ? [["Project key", args.key], ["Name", args.name], ["Path", args.path], ["Group", args.group], ["Harness", args.agent], ["Model", args.model], ["Effort", args.reasoning_effort]]
      : proposal?.type === "update_project"
        ? [["Project", args.project_key], ["Name", args.name], ["Group", args.group], ["Harness", args.agent], ["Model", args.model], ["Effort", args.reasoning_effort]]
        : ["create_schedule", "pause_schedule", "resume_schedule"].includes(proposal?.type)
          ? [["Schedule", args.name || args.key || args.schedule_key], ["Project", args.project_key], ["When", args.cron], ["Timezone", args.timezone], ["Agent", args.agent_name], ["Message", args.message], ["System instruction", args.system_message]]
          : [];
  return fields
...
function personalAssistantActionPreflight(proposal) {
  if (proposal?.id && Object.hasOwn(state.personalAssistantActionPreflights || {}, proposal.id)) {
    return state.personalAssistantActionPreflights[proposal.id];
  }
  return proposal?.preflight || null;
}

function personalAssistantPreviewValue(value) {
  if (value === null || value === undefined || value === "") return "Not changed";
  ...
  if (typeof value === "object") {
    ...
    return named === undefined ? "Details available" : personalAssistantPreviewValue(named);
  }
  return String(value);
}

function personalAssistantPreviewSettings(value) {
  if (value === null || value === undefined || value === "") return "Not changed";
  if (typeof value !== "object" || Array.isArray(value)) return personalAssistantPreviewValue(value);
  const fields = [
    ["Name", value.name], ["Key", value.key], ["Group", value.group], ["Harness", value.agent || value.harness],
    ["Model", value.model], ["Effort", value.reasoning_effort], ["Timezone", value.timezone],
    ["When", value.cron], ["Enabled", value.enabled],
  ];
  const summary = fields
    .filter(([, item]) => item !== null && item !== undefined && item !== "")
    .map(([label, item]) => `${label}: ${personalAssistantPreviewValue(item)}`)
    .join(" · ");
  return summary || "Details available";
}

function personalAssistantPreviewTarget(details) {
  const candidate = details?.target || details?.project || details?.schedule || details?.resolved_target;
  if (candidate && typeof candidate === "object") return personalAssistantPreviewValue(candidate);
  return personalAssistantPreviewValue(details?.name || details?.key || details?.resolved_server);
}

function personalAssistantPreviewRows(proposal, preflight) {
  const details = preflight?.details;
  if (!details || typeof details !== "object") return "";
  const rows = [];
  if (details.resolved_server) rows.push(["Server", personalAssistantPreviewValue(details.resolved_server)]);
  const target = personalAssistantPreviewTarget(details);
  if (target && target !== "Not changed") rows.push(["Resolved target", target]);
  if (Object.hasOwn(details, "before") || Object.hasOwn(details, "after")) {
    rows.push(["Project before", personalAssistantPreviewSettings(details.before)]);
    rows.push(["Project after", personalAssistantPreviewSettings(details.after)]);
  }
  [["Harness", details.harness], ["Model", details.model], ["Effort", details.reasoning_effort]].forEach(([label, value]) => {
    if (value !== undefined && value !== null && value !== "") rows.push([label, personalAssistantPreviewValue(value)]);
  });
  if (Object.hasOwn(details, "starts")) {
    rows.push(["Prepared agent", details.starts === false ? "Yes — creation only; it will not start the agent" : "No"]);
  }
  if (details.status !== undefined) rows.push(["Current status", personalAssistantPreviewValue(details.status)]);
  if (Object.hasOwn(details, "running")) rows.push(["Running now", personalAssistantPreviewValue(details.running)]);
  if (details.schedule && typeof details.schedule === "object") {
    const schedule = details.schedule;
    rows.push(["Schedule", personalAssistantPreviewValue(schedule.name || schedule.key)]);
    rows.push(["When", personalAssistantPreviewValue(schedule.cron)]);
    rows.push(["Timezone", personalAssistantPreviewValue(schedule.timezone)]);
    rows.push(["Next run", personalAssistantPreviewValue(schedule.next_run)]);
    if (Object.hasOwn(details, "enabled_before")) rows.push(["Enabled", `${personalAssistantPreviewValue(details.enabled_before)} → ${personalAssistantPreviewValue(details.enabled_after)}`]);
  }
  return rows.map(([label, value]) => `<div><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(value)}</dd></div>`).join("");
}

function personalAssistantActionStatus(proposal) {
  const stateName = String(proposal?.state || "unknown").toLowerCase();
  return {
    confirming: "Confirming",
    checking: "Checking current receipt",
    processing: "Working",
    awaiting_confirmation: "Approval needed",
    ready: "Prepared",
    queued: "Queued",
    executing: "Executing",
    ...
    executed: "Completed",
    rejected: "Rejected",
    failed: proposal?.code === "outcome_unknown" || proposal?.recovery?.state === "outcome_unknown" ? "Outcome unknown" : "Failed",
    unknown: "Outcome unknown",
    expired: "Expired",
  }[stateName] || "Status unavailable";
}

function personalAssistantActionReceiptCopy(proposal) {
  const stateName = String(proposal?.state || "unknown").toLowerCase();
  if (stateName === "confirming") return "Confirming — sending the exact confirmation to Tycho; completion is not recorded yet.";
  if (stateName === "processing") return "Working — waiting for Tycho to finish.";
  if (stateName === "executed") return "Completed — Tycho confirmed this change.";
  if (stateName === "queued") return "Queued — Tycho accepted this request; it is not finished yet.";
  if (stateName === "executing") return "Running — Tycho is working on it; it is not finished yet.";
  if (stateName === "verifying") return "Verifying outcome — Tycho is checking what happened; completion is not confirmed yet.";
  if (stateName === "ready") return "Prepared — Tycho has this request ready to run.";
  if (stateName === "rejected") return "Rejected — Tycho will not run this request.";
  if (stateName === "expired") return "Expired — this request belongs to an earlier conversation and cannot be approved.";
  if (stateName === "unknown") return "Outcome unknown — Tycho could not confirm whether it happened.";
  if (stateName === "failed" && (proposal?.code === "outcome_unknown" || proposal?.recovery?.state === "outcome_unknown")) return "Outcome unknown — Tycho could not confirm whether it happened.";
  if (stateName === "failed") return `Failed — ${proposal?.error || "Tycho could not complete this action."}`;
  return "The action state is unavailable; review the server receipt before trying anything else.";
}

function personalAssistantActionRecovery(proposal, copy) {
  const override = String(state.personalAssistantActionStateOverrides?.[proposal?.id] || "").toLowerCase();
  const serverProposal = proposal?.id ? personalAssistantActionProposal(proposal.id) : proposal;
  const serverState = String(serverProposal?.state || proposal?.state || "").toLowerCase();
  if (override === "checking") {
    return `<p class="pa-action-loading" role="status">Checking the current receipt before offering another confirmation.</p>`;
  }
  if (override === "unknown" && serverState === "awaiting_confirmation") {
    return `<div class="pa-action-recovery"><p>Confirmation delivery was interrupted. Check the current receipt before confirming this exact action again.</p><div class="pa-action-controls"><button class="ui-button" type="button" data-check-pa-action="${escapeAttr(proposal.id)}">Check current receipt</button></div></div>`;
  }
  const recovery = proposal?.recovery || {};
  const action = recovery.action || (recovery.state === "outcome_unknown" ? "verify" : "");
  if (action === "verify") {
    return `<div class="pa-action-controls"><button class="ui-button" type="button" data-verify-pa-proposal="${escapeAttr(proposal.id)}">Check whether it completed</button><button class="ui-button" type="button" data-pa-suggestion="Investigate action ${escapeAttr(proposal.id)} (${escapeAttr(copy.label)}) for ${escapeAttr(String(copy.target))}; do not repeat it.">Ask FRED to investigate</button></div>`;
  }
  if (action === "replace" || recovery.state === "replacement_available") {
    return `<div class="pa-action-controls"><button class="ui-button" type="button" data-pa-suggestion="Prepare a safe replacement for action ${escapeAttr(proposal.id)}: ${escapeAttr(copy.label)} for ${escapeAttr(String(copy.target))}.">Ask FRED to revise</button></div>`;
  }
  return "";
}

function personalAssistantResultAgent(result) {
  if (!result || typeof result !== "object") return null;
  const candidate = result.agent || result.created_agent || result;
  return candidate?.key ? candidate : null;
}

function renderPersonalAssistantResult(proposal) {
  const result = proposal?.result;
  if (!result || typeof result !== "object") return "";
  const agent = personalAssistantResultAgent(result);
  if (proposal.type === "read_docs" && result.content !== undefined) {
    const excerpt = String(result.content).slice(0, 1000);
    const full = String(result.content);
    return `<div class="pa-action-result"><strong>${escapeHtml(result.path || proposal.arguments?.path || "Documentation")}</strong><pre>${escapeHtml(excerpt)}${full.length > excerpt.length ? "…" : ""}</pre>${full.length > excerpt.length ? `<details><summary>Read more</summary><pre>${escapeHtml(full)}</pre></details>` : ""}</div>`;
  }
  if (proposal.type === "search_docs") {
    const rows = Array.isArray(result.results) ? result.results : [];
    return `<div class="pa-action-result"><strong>${rows.length ? "Documentation matches" : "No documentation matches"}</strong>${rows.length ? `<ul>${rows.map((row) => `<li><code>${escapeHtml(row.path || "docs")}</code><span>${escapeHtml(row.match || "")}</span><button class="ui-button" type="button" data-pa-suggestion="Read ${escapeAttr(row.path || "")}">Read</button></li>`).join("")}</ul>` : ""}</div>`;
  }
  if (["inspect_agents", "inspect_projects", "inspect_schedules"].includes(proposal.type)) {
    const records = Array.isArray(result.agents) ? result.agents : (Array.isArray(result.projects) ? result.projects : (Array.isArray(result.schedules) ? result.schedules : []));
    const isAgents = proposal.type === "inspect_agents";
    const type = isAgents ? "agents" : proposal.type === "inspect_projects" ? "projects" : "schedules";
    return `<div class="pa-action-result"><strong>${records.length ? `${records.length} ${type}` : `No ${type} found`}</strong>${records.length ? `<ul>${records.slice(0, 12).map((record) => {
      const key = record.key || record.name || "Unknown";
      const label = record.name && record.key ? `${record.name} · ${record.key}` : key;
      const detail = isAgents ? (record.status || (record.running ? "running" : "ready")) : (record.path || record.group || record.cron || "configured");
      const href = isAgents && record.key ? routeHash({ type: "agent", key: record.key }) : (proposal.type === "inspect_projects" && record.key ? routeHash({ type: "project", key: record.key }) : (proposal.type === "inspect_schedules" && record.key ? routeHash({ type: "scheduleForm", mode: "edit", key: record.key }) : ""));
      return `<li>${href ? `<a href="${escapeAttr(href)}">${escapeHtml(label)}</a>` : `<span>${escapeHtml(label)}</span>`}<span>${escapeHtml(detail)}</span></li>`;
    }).join("")}${records.length > 12 ? "<li>More results are available in Tycho.</li>" : ""}</ul>` : ""}</div>`;
  }
  if (proposal.type === "inspect_agent_run" && result.run) {
    const run = result.run;
    return `<div class="pa-action-result"><strong>${escapeHtml(result.agent?.name || result.agent?.key || "Agent")} run</strong><span>${escapeHtml(run.running ? "Running" : (run.summary || run.last_result || "No recent result"))}</span></div>`;
  }
  if (proposal.type === "inspect_agent_log" && Array.isArray(result.tail)) {
    return `<div class="pa-action-result"><strong>Recent agent log</strong><pre>${escapeHtml(result.tail.slice(-12).join("\n"))}</pre></div>`;
  }
  if (agent) {
    const status = agent.status || (agent.running ? "running" : "prepared");
    return `<div class="pa-action-result"><strong>${proposal.type === "create_agent" ? "Agent created" : "Agent updated"}</strong><span><a href="${escapeAttr(routeHash({ type: "agent", key: agent.key }))}">Open ${escapeHtml(agent.name || agent.key)}</a> · ${escapeHtml(status)}</span>${proposal.type === "create_agent" && !agent.running ? `<span>Prepared only; approve Start agent when you want it to run.</span><button class="ui-button" type="button" data-pa-suggestion="Prepare a start action for agent ${escapeAttr(agent.key)}. Do not start it yet.">Prepare start</button>` : ""}</div>`;
  }
  const project = result.project;
  if (project?.key) return `<div class="pa-action-result"><strong>Project ${proposal.type === "create_project" ? "created" : "updated"}</strong><span><a href="${escapeAttr(routeHash({ type: "project", key: project.key }))}">Open ${escapeHtml(project.name || project.key)}</a></span></div>`;
  const schedule = result.schedule;
  if (schedule?.key) return `<div class="pa-action-result"><strong>Schedule updated</strong><span><a href="${escapeAttr(routeHash({ type: "scheduleForm", mode: "edit", key: schedule.key }))}">Open ${escapeHtml(schedule.name || schedule.key)}</a></span></div>`;
  return "";
}

function renderPersonalAssistantProposal(proposal) {
  const copy = personalAssistantActionCopy(proposal);
  const stateName = String(state.personalAssistantActionStateOverrides?.[proposal?.id] || proposal?.state || "unknown").toLowerCase();
  const displayedProposal = stateName === String(proposal?.state || "").toLowerCase() ? proposal : { ...proposal, state: stateName };
  const completed = stateName === "executed";
  const expired = personalAssistantProposalExpired(proposal);
  const pending = stateName === "awaiting_confirmation" && !expired;
  const busy = state.pendingPersonalAssistantProposalIds?.has(proposal.id);
  const preflight = personalAssistantActionPreflight(proposal);
  const preflightError = state.personalAssistantActionPreflightErrors?.[proposal.id];
  const actionError = state.personalAssistantActionErrors?.[proposal.id];
  const digest = preflight?.digest || proposal?.digest || "";
  const token = preflight?.precondition_token || preflight?.precondition?.token || "";
  const available = preflight?.details?.available !== false && preflight?.prepared !== false;
  const canConfirm = pending && !busy && Boolean(digest && token && preflight && available && !preflightError);
  const confirmLabel = copy.label.replace(/^Install or update skill$/, "Install skill");
  const settings = personalAssistantProposalSettings(proposal);
  const previewRows = personalAssistantPreviewRows(proposal, preflight);
  const previewHtml = preflight
    ? `<section class="pa-action-preview" aria-label="Server-owned action preview"><strong>Exact server preview</strong><dl>${previewRows || `<div><dt>Availability</dt><dd>${escapeHtml(preflight.details?.reason || "No additional preview details")}</dd></div>`}</dl><p class="pa-preview-note">This preview describes what Tycho will do on this server. It does not authorize the action. Codex runs with the server’s configured process permissions.</p></section>`
    : pending ? `<p class="pa-action-loading" role="status">Loading the exact server preview before confirmation…</p>` : "";
  const unavailableReason = preflight?.details?.reason || "The server says this target is not currently available.";
  const previewErrorHtml = preflightError
    ? `<div class="pa-preview-error"><p class="pa-control-error" role="alert">Could not load the exact server preview: ${escapeHtml(preflightError)} Confirmation is unavailable until the preview can be verified.</p><button class="ui-button" type="button" data-retry-pa-preflight="${escapeAttr(proposal.id)}">Retry preview</button></div>`
    : preflight && !available
      ? `<div class="pa-preview-error"><p class="pa-control-error" role="alert">Preview unavailable: ${escapeHtml(unavailableReason)} Confirmation is unavailable until the server reports an available preview.</p><button class="ui-button" type="button" data-retry-pa-preflight="${escapeAttr(proposal.id)}">Refresh preview</button></div>`
      : "";
  const actionErrorHtml = actionError
  ...
  const completedDetails = completed
    ? `<details class="pa-action-original"><summary>View original request and preview</summary>${proposalDetails}${previewHtml}</details>`
    : `${previewHtml}`;
  const technical = {
    proposal_id: proposal.id,
    type: proposal.type,
    proposal_digest: digest || undefined,
    precondition_token: token || undefined,
    active_key: preflight?.active_key || proposal.active_key || undefined,
    generation: preflight?.generation ?? proposal.generation,
    arguments: proposal.arguments,
    preflight,
  };
  return `<article class="pa-inline-action ${pending || ["queued", "executing", "verifying"].includes(stateName) ? "pending" : "receipt"}" data-proposal-id="${escapeAttr(proposal.id)}" data-proposal-run="${escapeAttr(proposal.source_run_id || "")}" data-proposal-state="${escapeAttr(stateName)}">
    <span class="pa-action-label">${escapeHtml(pending && busy ? "Confirming" : actionStatus)}</span>
    ${completedSummary}${completedDetails}${previewErrorHtml}${actionErrorHtml}${receiptHtml}
    <details class="pa-technical-details"><summary>Technical details</summary><dl>${digest ? `<div><dt>Proposal digest</dt><dd><code>${escapeHtml(digest)}</code></dd></div>` : ""}${token ? `<div><dt>Precondition token</dt><dd><code>${escapeHtml(token)}</code></dd></div>` : ""}</dl><pre>${escapeHtml(JSON.stringify(technical, (_key, value) => value === undefined ? undefined : value, 2))}</pre></details>
  </article>`;
}
...
function personalAssistantRecommendationContext(item, entry, index) {
  ...
  return {
    ...
    description: String(entry?.description || prompt).trim(),
    prompt,
  };
}
...
function renderPersonalAssistantRecommendations(item, blocks = [], options = {}) {
  ...
  const items = Array.isArray(recommendations.items)
    ...
    : [];
  const source = String(recommendations.source || "starter");
  const guidance = source === "daily_handoff"
    ? "Generated after yesterday’s handoff from recent work and useful signals."
    : source === "fallback"
      ? "Yesterday’s handoff was incomplete, so FRED added safe fallback prompts."
      : source === "stale"
  ...
  const buttons = items.map((entry, index) => {
    ...
    return `<li><button class="pa-suggestion ui-button${selected ? " selected" : ""}" type="button" aria-label="${escapeAttr(label)}" title="${escapeAttr(recommendation.prompt)}" data-pa-suggestion="${escapeAttr(recommendation.prompt)}" data-pa-recommendation="${escapeAttr(JSON.stringify(recommendation))}" ${disabled ? "disabled aria-disabled=\"true\"" : ""}><span class="pa-starter-copy"><strong>${escapeHtml(recommendation.title)}</strong><span>${escapeHtml(recommendation.description)}</span></span>${selected ? '<span class="pa-recommendation-state">Selected</span>' : ""}</button></li>`;
  }).join("");
  const body = buttons
...
function personalAssistantProposalMatchesBlock(proposal, block) {
  ...
  const source = String(proposal?.source_run_id || "");
  if (!source) return false;
  return [block?.run_id, block?.id, metadata.run_id, metadata.personal_assistant_action_source_run_id, metadata.session_id, metadata.thread_id, metadata.turn_id]
    .some((value) => String(value || "") === source);
}

function personalAssistantProposalHtmlAfterBlock(block, proposals, rendered) {
  const matching = proposals.filter((proposal) => !rendered.has(proposal.id) && personalAssistantProposalMatchesBlock(proposal, block));
  matching.forEach((proposal) => rendered.add(proposal.id));
  return matching.map(renderPersonalAssistantProposal).join("");
}

function renderPersonalAssistantUnattachedProposals(proposals, rendered) {
  const unmatched = proposals.filter((proposal) => !rendered.has(proposal.id));
  const readError = state.personalAssistantActionErrors?.read;
  const errorHtml = readError ? `<p class="pa-control-error" role="alert">FRED action receipts could not be refreshed: ${escapeHtml(readError)}. The cards below may be stale; do not confirm a changed preview.</p>` : "";
  if (!unmatched.length) return errorHtml;
  return `${errorHtml}<section class="pa-action-feed" aria-label="FRED actions"><span class="pa-action-feed-label">Actions from recent FRED replies</span>${unmatched.map(renderPersonalAssistantProposal).join("")}</section>`;
}

function personalAssistantObservedTime(value) {
  if (!value) return "time unavailable";
  const text = String(value);
  if (/^\d{4}-\d{2}-\d{2}$/.test(text)) {
    const [year, month, day] = text.split("-").map(Number);
    const dateOnly = new Date(year, month - 1, day);
    return Number.isNaN(dateOnly.getTime()) ? text : dateOnly.toLocaleDateString(undefined, { dateStyle: "medium" });
  }
  const date = new Date(text);
  return Number.isNaN(date.getTime()) ? String(value) : date.toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" });
}

function personalAssistantHistoryDate(entry) {
  if (entry?.active_date) return personalAssistantObservedTime(entry.active_date);
  if (entry?.closed_at) return personalAssistantObservedTime(entry.closed_at);
  return "Previous conversation";
}

function personalAssistantHistoryConversationLink(entry) {
  const archive = entry?.archived_conversation;
  if (!archive || archive.read_only !== true) return "";
  const rawKey = archive.agent_key || entry?.agent_key || entry?.archived_agent_key;
  if (!rawKey) return "";
  const serverKey = archive.server_key || entry?.server_key || state.personalAssistant?.server_key || personalAssistantServerKey();
  const key = parseResourceClientKey(rawKey) ? rawKey : resourceClientKey(serverKey, rawKey);
  return routeHash({ type: "agent", key });
}

function personalAssistantHistoryRequestIsCurrent(id, sequence, routeHashAtStart, context) {
  const coordinator = state.personalAssistantRequests.coordinator;
  return sequence === coordinator.historySequence &&
    coordinator.historySelection === String(id) &&
    location.hash === routeHashAtStart &&
    parseRoute().type === "personalAssistant" &&
    (!context || personalAssistantSessionMatches(context));
}

function clearPersonalAssistantHistorySelection() {
  const coordinator = state.personalAssistantRequests.coordinator;
  coordinator.historySequence += 1;
  coordinator.historySelection = "";
  state.personalAssistantHistoryEntry = null;
  state.personalAssistantHistoryLoading = false;
  state.personalAssistantHistoryError = "";
}
...
function personalAssistantContinuityPreview(value) {
  if (!value) return "No handoff has been saved yet.";
  if (typeof value === "string") return value.slice(0, 700);
  const items = value.open_items || value.outstanding_items || value.summary || value.content || value;
  return typeof items === "string" ? items.slice(0, 700) : JSON.stringify(items).slice(0, 700);
}

function personalAssistantHandoffItems(value) {
  if (!Array.isArray(value)) return [];
  return value.filter((item) => item !== null && item !== undefined && item !== "").map((item) => {
    if (typeof item === "string") return item;
    if (typeof item === "object") return item.label || item.title || item.name || item.text || JSON.stringify(item);
    return String(item);
  });
}

function personalAssistantHandoffSection(label, value) {
  const items = personalAssistantHandoffItems(value);
  if (!items.length) return "";
  return `<section class="pa-handoff-section"><h4>${escapeHtml(label)}</h4>${items.length === 1 ? `<p>${escapeHtml(items[0])}</p>` : `<ul>${items.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>`}</section>`;
}

function personalAssistantHandoffHtml(handoff, { archived = false } = {}) {
  if (!handoff || typeof handoff !== "object") return `<p>${escapeHtml(personalAssistantContinuityPreview(handoff))}</p>`;
  const summary = handoff.summary || handoff.outcome || handoff.content;
  const summaryHtml = summary ? `<section class="pa-handoff-section"><h4>Summary</h4><p>${escapeHtml(String(summary))}</p></section>` : "";
  const source = archived ? "Saved FRED summary / handoff" : "Current continuity handoff";
  const date = handoff.active_date || handoff.closed_at;
  const metadata = [source, date ? `date ${personalAssistantObservedTime(date)}` : "", handoff.reason ? `source ${handoff.reason}` : ""]
    .filter(Boolean).join(" · ");
  return `<p class="pa-handoff-meta">${escapeHtml(metadata)}</p>${summaryHtml}${personalAssistantHandoffSection("Decisions", handoff.decisions)}${personalAssistantHandoffSection("Open work", handoff.open_items || handoff.open_work || handoff.outstanding_items)}${personalAssistantHandoffSection("References", handoff.references)}${personalAssistantHandoffSection("Lessons", handoff.lessons)}${personalAssistantHandoffSection("Promotion candidates", handoff.promotion_candidates)}`;
}

function personalAssistantProposalExpired(proposal, item = state.personalAssistant) {
  if (!proposal) return false;
  const stateName = String(proposal.state || "").toLowerCase();
  if (stateName === "expired") return true;
  if (stateName !== "awaiting_confirmation") return false;
  return proposal.expired === true ||
    (proposal.active_key && item?.active_key && String(proposal.active_key) !== String(item.active_key)) ||
    (proposal.generation !== undefined && item?.generation !== undefined && Number(proposal.generation) !== Number(item.generation));
}

function personalAssistantHistoryActions() {
  const actions = state.personalAssistantHistoryEntry?.archived_actions;
  return Array.isArray(actions)
    ? actions.filter((proposal) => proposal && String(proposal.id || "").trim())
    : [];
}

function personalAssistantExpiredProposals(item, proposals = []) {
  const activeExpired = (Array.isArray(proposals) ? proposals : [])
    .filter((proposal) => personalAssistantProposalExpired(proposal, item));
  const seen = new Set();
  return [...activeExpired, ...personalAssistantHistoryActions()]
    .filter((proposal) => proposal && String(proposal.id || "").trim())
    .filter((proposal) => {
...
function personalAssistantArchivedActionStatus(proposal) {
  const stateName = String(proposal?.state || proposal?.historical_state || "").toLowerCase();
  if (proposal?.expired === true || stateName === "expired") return "Expired";
  if (proposal?.recovery?.state === "outcome_unknown" || proposal?.code === "outcome_unknown") return "Outcome unknown";
  if (stateName === "executed") return "Completed";
  if (stateName === "failed") return "Failed";
  if (stateName === "rejected") return "Rejected";
  if (stateName === "executing") return "Running";
  if (["queued", "verifying"].includes(stateName)) return stateName === "verifying" ? "Verifying outcome" : "Queued";
  return personalAssistantActionStatus(proposal);
}

function renderPersonalAssistantArchivedAction(proposal, archived) {
  const copy = personalAssistantActionCopy(proposal);
  const status = archived ? personalAssistantArchivedActionStatus(proposal) : "Expired";
  const detail = archived && status !== "Expired"
    ? `${status} — this archived action is read-only.`
    : "This pending proposal belongs to an earlier conversation and cannot be approved.";
  return `<li data-pa-expired-proposal-id="${escapeAttr(proposal.id)}"${archived ? ` data-pa-history-action-id="${escapeAttr(proposal.id)}"` : ""}><span class="pa-history-action-status">${escapeHtml(status)}</span><span>${escapeHtml(copy.label)} · ${escapeHtml(String(copy.target))}</span><span>${escapeHtml(detail)}</span></li>`;
}

function renderPersonalAssistantContinuity(item, proposals) {
  const rollover = item.next_rollover_at ? new Date(item.next_rollover_at) : null;
  const rolloverText = rollover && !Number.isNaN(rollover.getTime()) ? rollover.toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" }) : "Daily handoff time is set by your timezone.";
  const history = Array.isArray(item.history) ? item.history : [];
  const trackedByKey = new Map();
  [...proposals.map((proposal) => proposal.tracked), ...(Array.isArray(item.task_references) ? item.task_references : [])]
    .filter((reference) => reference?.kind === "agent")
    .forEach((reference) => {
      const key = String(reference.agent_key || reference.key || "");
      if (key) trackedByKey.set(key, { ...(trackedByKey.get(key) || {}), ...reference, agent_key: key });
    });
  const tracked = [...trackedByKey.values()].map((reference) => {
    const detail = state.agentDetails[reference.agent_key] || {};
    const list = state.agents.find((agent) => agent.key === reference.agent_key) || {};
    return { ...reference, ...detail, ...list, agent_key: reference.agent_key };
  });
  const recovery = item.recovery;
  const trackedHtml = tracked.length ? `<section class="pa-context-panel pa-tracked-agents" aria-labelledby="pa-context-tracked-title"><div class="pa-context-heading"><h3 id="pa-context-tracked-title">Tracked agents</h3><span>${tracked.length}</span></div><ul>${tracked.map((agent) => `<li><a href="${escapeAttr(routeHash({ type: "agent", key: agent.agent_key }))}">${escapeHtml(agent.name || agent.agent_key)}</a><span class="pa-context-status">${escapeHtml(agent.awaiting_input ? "needs input" : (agent.running ? "running" : (agent.status || agent.last_result || "prepared")))}</span></li>`).join("")}</ul></section>` : "";
  const historyEntry = state.personalAssistantHistoryEntry;
  const selectedHistory = history.find((entry) => String(entry?.id || "") === String(historyEntry?.id || ""));
  const displayedHistory = historyEntry ? { ...(selectedHistory || {}), ...historyEntry } : null;
  const archivedConversationHref = personalAssistantHistoryConversationLink(displayedHistory);
  const archivedConversationLink = archivedConversationHref ? `<a class="ui-button" href="${escapeAttr(archivedConversationHref)}">Open archived conversation (read-only)</a>` : "";
  const historyRows = history.slice(0, 6).map((entry) => {
    const conversationHref = personalAssistantHistoryConversationLink(entry);
    const date = personalAssistantHistoryDate(entry);
    return `<li><article><time datetime="${escapeAttr(entry.active_date || entry.closed_at || "")}">${escapeHtml(date)}</time><p>${escapeHtml(entry.summary || "Saved handoff")}</p><div class="pa-history-actions"><button class="ui-button" type="button" data-open-pa-history="${escapeAttr(entry.id)}">View saved summary</button>${conversationHref ? `<a class="ui-button" href="${escapeAttr(conversationHref)}">Open conversation</a>` : ""}</div></article></li>`;
  }).join("");
  const historyDetail = displayedHistory
    ? `<article class="pa-history-entry" aria-live="polite"><h3>Saved summary / handoff <span>Read-only</span></h3>${personalAssistantHandoffHtml(displayedHistory, { archived: true })}<p>This saved handoff contains continuity fields, not a transcript. Return to the active conversation to continue.</p><div class="pa-history-actions">${archivedConversationLink}<a class="ui-button" href="#personal-assistant" data-pa-back-active>Back to active FRED</a></div></article>`
    : state.personalAssistantHistoryLoading
      ? `<article class="pa-history-entry" aria-live="polite"><p class="pa-history-notice" role="status">Loading the saved summary…</p><a class="ui-button" href="#personal-assistant" data-pa-back-active>Back to active FRED</a></article>`
      : "";
  const historyHtml = history.length ? `<section class="pa-context-panel pa-history" aria-labelledby="pa-context-history-title"><div class="pa-context-heading"><h3 id="pa-context-history-title">Saved summaries and handoffs</h3><span>Latest ${Math.min(history.length, 6)}</span></div><ul class="pa-history-list">${historyRows}</ul>${historyDetail}</section>` : historyDetail;
  const historyNotice = state.personalAssistantHistoryError
      ? `<p class="pa-control-error" role="alert">Could not load the saved summary: ${escapeHtml(state.personalAssistantHistoryError)}</p>`
      : "";
  const recoveryHtml = recovery ? `<div class="pa-recovery"><strong>${recovery.state === "recovered" ? "Handoff recovered" : "Handoff needs attention"}</strong><span>${escapeHtml(recovery.error || recovery.message || "FRED kept the current conversation available. Review the handoff before continuing.")}</span>${recovery.state === "recovered" ? "" : '<button class="ui-button" type="button" data-refresh>Check recovery status</button>'}</div>` : "";
  const activeSettings = item.active_settings || item.config || {};
  const settingsText = [activeSettings.model, activeSettings.reasoning_effort, activeSettings.timezone].filter(Boolean).join(" / ");
  const settingsHtml = `<section class="pa-context-panel pa-context-settings" aria-labelledby="pa-context-settings-title"><h3 id="pa-context-settings-title">Daily boundary and settings</h3><dl><div><dt>Timezone</dt><dd>${escapeHtml(String(activeSettings.timezone || item.active_timezone || item.config?.timezone || "Configured timezone"))}</dd></div><div><dt>Next handoff</dt><dd>${escapeHtml(rolloverText)}</dd></div><div><dt>Session settings</dt><dd>${escapeHtml(settingsText || "Current server configuration")}</dd></div></dl><p>${escapeHtml(item.settings_apply || "Changes apply to a new FRED conversation.")}</p></section>`;
  const expired = personalAssistantExpiredProposals(item, proposals);
  const historyActionIds = new Set(personalAssistantHistoryActions().map((proposal) => String(proposal.id)));
  const expiredHtml = expired.length
    ? `<div class="pa-handoff-section pa-expired-proposals" data-pa-expired-proposals><strong>${historyActionIds.size ? "Actions from this saved conversation" : "Expired proposals"}</strong><p>${historyActionIds.size ? "Archived action records are read-only. Expired proposals cannot be approved here." : "These pending proposals belong to an earlier conversation and cannot be approved here."}</p><ul>${expired.map((proposal) => renderPersonalAssistantArchivedAction(proposal, historyActionIds.has(String(proposal.id)))).join("")}</ul></div>`
    : "";
  return `<details class="pa-continuity"><summary><span class="pa-continuity-summary-copy"><strong>History and context</strong><small>Continuity, tracked work, and previous handoffs</small></span><span class="pa-continuity-next">Next handoff: ${escapeHtml(rolloverText)}</span></summary><div class="pa-continuity-body"><section class="pa-context-panel pa-prior-continuity" aria-labelledby="pa-context-prior-title"><h3 id="pa-context-prior-title">Prior continuity</h3>${personalAssistantHandoffHtml(item.continuity)}</section>${settingsHtml}${trackedHtml}${expiredHtml}${historyHtml}${historyNotice}${recoveryHtml}</div></details>`;
}

function renderPersonalAssistantSetup(item) {
  return `<section class="pa-setup ui-surface" aria-labelledby="pa-setup-title"><h2 id="pa-setup-title">Set up FRED</h2><p id="pa-setup-guidance">FRED—Friendly Robot for Execution Dispatcher—helps you prepare and oversee work in Tycho.</p>${personalAssistantReadinessNotice()}${personalAssistantConfigForm(item)}</section>`;
}
...
function personalAssistantHeaderTitleHtml() {
  const assetVersion = escapeAttr(document.documentElement.dataset.assetVersion || "");
  return `<span class="agent-name-inline personal-assistant-header-title"><img class="fred-avatar" src="/fred-avatar.png?v=${assetVersion}" alt="FRED"><span class="agent-name-label">FRED</span></span>`;
}

function personalAssistantMoreMenuHtml() {
  return moreMenuHtml([
    moreMenuButton({ label: "Settings", icon: "settings", attrs: "data-open-settings" }),
  ]);
}
...
function renderSetup() {
  ...
  const settingsSections = [
    ...
    ["personal-assistant", "FRED", "settings-personal-assistant"],
  ...
  replaceView(`
    ...
    ${settingsPanel("automation", `<section class="detail-card" id="settings-automation">
      <div class="detail-card-body" style="padding-top: 12px;">
        ...
        `).join("")}
        ${Object.entries(setup.schema?.schemas || { ordinary: setup.schema || {} }).map(([key, schema]) => {
          const valid = schema?.valid === true;
          const label = key === "personal_assistant" ? "FRED result schema" : "Agent result schema";
          const fallbackPath = key === "personal_assistant" ? "config/schemas/personal_assistant_result.json" : "config/schemas/agent_result.json";
          return `<div class="detail-row">
            <span class="status-mark ${valid ? "done" : "fail"}" ${statusMarkAttributes(valid ? "done" : "fail")} aria-hidden="true">${statusIcon(valid)}</span>
            <div><strong>${label}</strong><span class="wrap-anywhere">${escapeHtml(schema?.path || fallbackPath)}</span></div>
            ${statusBadge(valid ? "Valid" : "Invalid", valid ? "done" : "fail")}
          </div>`;
        }).join("")}
      </div>
    </section>`)}
    ${settingsPanel("personal-assistant", renderPersonalAssistantSettings(state.personalAssistant))}
...
function renderSkillInstallationRow(harness) {
  ...
  const profileLabel = harness?.adapter && harness.adapter !== harness.harness
    ? `${harness.harness} · ${harness.adapter} profile`
    : harness?.harness;
  ...
  return `<div class="detail-row skill-installation-row" data-skill-installation-harness="${escapeAttr(harness.harness)}">
    ...
    <div><strong>${escapeHtml(profileLabel)}</strong><span class="wrap-anywhere">${escapeHtml(detail)}</span></div>
...
function renderHarnessDetails(item) {
  ...
  const checking = !!state.harnessAuthChecking?.[item.name];
  const auth = harnessAuthSnapshot(item);
  ...
  return `
    <details class="harness-details" data-harness-name="${escapeAttr(item.name)}" ${ready ? "" : "open"}>
      <summary>
        ...
        <div><strong>${escapeHtml(item.name)}</strong><span>${escapeHtml(item.detail)}</span></div>
        <div class="harness-summary-actions">
          ${statusBadge(ready ? "Ready" : "Missing", ready ? "done" : "fail")}
          <details class="harness-action-menu" data-overlay-menu data-overlay-key="harness:${escapeAttr(item.name)}">
            <summary aria-label="${escapeAttr(`${item.name} actions`)}" title="${escapeAttr(`${item.name} actions`)}" aria-haspopup="menu">${iconSvg("ellipsis")}</summary>
            <div class="harness-action-popover ui-overlay-surface" data-overlay-kind="menu">
              ${moreMenuHtml([moreMenuButton({ label: checking ? "Checking authentication" : "Check authentication", icon: checking ? "loaderPinwheel" : "shieldCheck", attrs: `data-check-harness-auth="${escapeAttr(item.name)}" ${checking ? "disabled aria-disabled=\"true\"" : ""}` })])}
            </div>
          </details>
        </div>
      </summary>
      <div class="harness-detail-body">
        <div class="harness-auth-snapshot" data-auth-state="${escapeAttr(auth.state)}">
          <span class="status-mark ${escapeAttr(auth.className)}" ${statusMarkAttributes(auth.className)} aria-hidden="true">${statusIcon(auth.className !== "fail")}</span>
          <div><strong>Authentication: ${escapeHtml(auth.label)}</strong><span>${escapeHtml(auth.description)}</span></div>
          ${statusBadge(auth.label, auth.className)}
        </div>
        <div class="kv-grid">
          ...
          ${kv("Last auth check", auth.checkedAt ? formatHarnessAuthTime(auth.checkedAt) : "Not checked")}
...
function harnessAuthSnapshot(item) {
  const snapshot = item?.auth_snapshot || {};
  const state = String(snapshot.state || "unknown");
  if (state === "authenticated") return { state, className: "done", label: "Authenticated", checkedAt: snapshot.checked_at, description: snapshot.checked_at ? `Last checked ${formatHarnessAuthTime(snapshot.checked_at)}` : "Authentication was confirmed" };
  if (state === "unauthenticated") return { state, className: "fail", label: "Unauthenticated", checkedAt: snapshot.checked_at, description: snapshot.checked_at ? `Last checked ${formatHarnessAuthTime(snapshot.checked_at)}. Run the harness login flow, then check again.` : "Run the harness login flow, then check again." };
  const checkedAt = snapshot.checked_at;
  return { state: "unknown", className: "need", label: checkedAt ? "Unknown" : "Not checked", checkedAt, description: checkedAt ? `Last check ${formatHarnessAuthTime(checkedAt)} could not determine authentication.` : "Authentication has not been checked yet." };
}

function formatHarnessAuthTime(value) {
...
function renderServerActionsMenu(server) {
  ...
  const updateAvailable = !server.local && server.status === "online" && server.update?.available === true;
  const updateInProgress = state.serverUpdateKey === server.key;
  const items = [
    moreMenuButton({
      label: "Refresh",
      icon: "rotateCcw",
    ...
    ...(!server.local ? [
      ...(updateAvailable ? [
        moreMenuButton({
          label: updateInProgress ? "Updating Tycho" : "Update Tycho",
          icon: "rotateCcw",
          attrs: `data-update-server="${key}" ${updateInProgress ? "disabled aria-disabled=\"true\"" : ""}`,
        }),
      ] : []),
      moreMenuButton({
...
function renderAgentWorkspace(agent, blocks, options = {}) {
  ...
  return renderConversationWorkspace({
    classNames,
    ...
    dockHtml: `${renderAgentFloatingActions(agent, floatingOptions)}${composerPlacement.dock}`,
    overlayHtml: composerPlacement.overlay,
  });
}
...
function renderConversationWorkspace({
  ...
  const className = ["agent-workspace", ...classNames].filter(Boolean).join(" ");
  const detail = String(detailHtml || "").trim();
  const dock = String(dockHtml || "").trim();
  return `
    <section class="${escapeAttr(className)}" data-agent-workspace ${attributes}>
...
function renderSummaryAttachmentMenu(agent, attachments = [], summaryId = "") {
  ...
  return `
    <div class="summary-attachment-menu" data-summary-attachment-menu data-summary-attachment-key="${escapeAttr(menuKey)}"${stateKey}>
      <button class="attachment-toggle-button summary-attachment-trigger" type="button" data-toggle-summary-attachment-menu aria-label="Choose summary attachment" title="Attachments" aria-expanded="false">
        ${iconSvg("paperclip")}
        <span class="attachment-count">${escapeHtml(String(normalized.length))}</span>
      </button>
      <div class="summary-attachment-menu-popover ui-overlay-surface hidden" data-summary-attachment-menu-popover data-summary-attachment-key="${escapeAttr(menuKey)}" data-overlay-kind="menu" data-preserve-scroll data-state-key="summary-attachment-menu-list:${escapeAttr(menuKey)}" role="region" tabindex="0" aria-label="Summary attachments">
...
function positionSummaryAttachmentMenu(trigger) {
  ...
  if (popover.parentElement !== els.summaryAttachmentOverlay) {
...
function repositionSummaryAttachmentMenusOnViewportScroll(event) {
  if (event.target instanceof Element && event.target.closest("[data-summary-attachment-menu-popover]")) return;
  repositionOpenSummaryAttachmentMenus();
}
...
function renderAgentConversationView(agent, blocks, options = {}) {
  const allConversationBlocks = conversationBlocksForAgent(agent, blocks);
  const conversationBlocks = options.personalAssistant
    ? personalAssistantVisibleConversationBlocks(allConversationBlocks)
  ...
  return `
    ...
    <div class="message-list">
      ${options.personalAssistant ? "" : renderLoadOlderConversationControl(agent?.key)}
      ${options.leadingHtml || ""}
...
function receiveConversationSnapshot(agentKey, blocks, revision, options = {}) {
  ...
  // `totalBlockCount` describes the full transcript on the server (see
  // conversation_snapshot in remote_server.rb); `blocks` may only be the most
  // recent window. Track them separately so "N new messages" math and the
  // "load older" affordance stay correct even though the rendered array is bounded.
  const totalBlockCount = Number.isFinite(options.totalBlockCount) ? options.totalBlockCount : incoming.length;
  const hasOlder = options.hasOlder === true;
  if (shouldApply) {
    state.conversations[key] = {
      revision, blocks: incoming, digest: options.digest || "", loaded: true, loading: false,
      total: totalBlockCount, hasOlder, loadingOlder: false,
    };
  ...
  state.pendingConversationUpdates[key] = {
    revision, blocks: incoming, unseen, digest: options.digest || "", totalBlockCount, hasOlder,
  };
  return { applied: false, unseen };
}
...
function receiveConversationMetadata(agentKey, metadata) {
  ...
  // Compare against the full transcript total the client last saw, not the
  // (possibly windowed) rendered block count -- otherwise a huge transcript
  // that has been windowed down would look like it has thousands of "new"
  // messages on every poll.
  const previousTotal = Number.isFinite(current.total) ? current.total : current.blocks.length;
  if ((digest && digest === String(current.digest || "")) ||
      (!digest && revision === String(current.revision || "") && blockCount === previousTotal)) {
    current.revision = revision;
  ...
  const unseenCount = Math.max(0, blockCount - previousTotal);
...
async function loadPendingConversationMessages(agentKey) {
  ...
  render({ preserveLiveEditor: true });
  try {
    // If the user had already expanded older history via "Load older
    // messages", request at least that many blocks so catching up on new
    // tail activity does not silently drop what they already scrolled into.
    const previouslyLoaded = state.conversations[key]?.blocks?.length || 0;
    const limit = Math.max(CONVERSATION_DEFAULT_TAIL_LIMIT, previouslyLoaded);
    const data = await apiGet(`/agents/${encodeURIComponent(key)}/conversation?limit=${limit}`);
    state.conversations[key] = {
      ...(state.conversations[key] || {}),
      ...
      total: Number.isFinite(data.conversation_block_count) ? data.conversation_block_count : undefined,
      hasOlder: data.conversation_has_older === true,
      loaded: true,
      loading: false,
    };
...
// Fetches an older page of conversation blocks and prepends them to the
// client's already-loaded window. `before` is a count from the tail (how
// many blocks the client already has), so it stays correct even if new
// blocks were appended to the tail since the last fetch (see
// conversation_snapshot in remote_server.rb).
async function loadOlderConversationMessages(agentKey) {
  const key = String(agentKey || "");
  const current = state.conversations[key];
  if (!key || !current || current.loadingOlder || !current.hasOlder) return;
  ...
  try {
    ...
    const data = await apiGet(
      `/agents/${encodeURIComponent(key)}/conversation?before=${before}&limit=${CONVERSATION_DEFAULT_TAIL_LIMIT}`
    );
    const olderBlocks = Array.isArray(data.conversation) ? data.conversation : [];
    const target = state.conversations[key];
    if (target) {
      // `before` is intentionally relative to the current tail so a request
      // can still find older history after new blocks arrive. That also means
      // a page in flight may overlap the window we already rendered; retain
      // only blocks not already present before prepending so messages never
      // appear twice.
      const uniqueOlderBlocks = REMOTE_HELPERS.unseenConversationBlocks(target.blocks, olderBlocks);
      target.blocks = uniqueOlderBlocks.concat(target.blocks);
      target.hasOlder = data.conversation_has_older === true;
      if (Number.isFinite(data.conversation_block_count)) target.total = data.conversation_block_count;
      target.loadingOlder = false;
      // conversationTailMarker() includes blocks.length, so prepending older
      // history looks like "new tail content" to shouldAutoScrollAgentConversation
      // unless we prime the marker here -- otherwise the render below would
      // incorrectly snap the view back to the bottom instead of preserving
      // the reader's scroll position (restored manually further down).
      state.conversationTailMarkers[key] = conversationTailMarker(key);
    }
  } catch (error) {
    if (current) current.loadingOlder = false;
    showGrowl(error.message || "Could not load older messages", "need");
  } finally {
    // Prepending older blocks changes the scroll container's content height
    // above the fold. Preserve the user's visual scroll position (they were
    // reading near the top) instead of letting the re-render's default
    // "stick to bottom" behavior yank them back down to the newest message.
    const scrollPane = document.querySelector("[data-agent-conversation-scroll]");
    const previousScrollHeight = scrollPane?.scrollHeight || 0;
    const previousScrollTop = scrollPane?.scrollTop || 0;
    state.renderedViewHtml = "";
    render({ preserveLiveEditor: true });
    const restoredPane = document.querySelector("[data-agent-conversation-scroll]");
    if (restoredPane && previousScrollHeight) {
      restoredPane.scrollTop = previousScrollTop + (restoredPane.scrollHeight - previousScrollHeight);
    }
  }
}

function renderLoadOlderConversationControl(agentKey) {
  const current = state.conversations[String(agentKey || "")];
  if (!current?.hasOlder) return "";

  const loading = current.loadingOlder === true;
  const label = loading ? "Loading older messages" : "Load older messages";
  return `
    <div class="conversation-load-older">
      <button class="ui-button load-older-messages" type="button" data-load-older-conversation="${escapeAttr(agentKey)}" ${loading ? 'aria-busy="true" disabled' : ""}>
        ${escapeHtml(label)}
      </button>
    </div>
  `;
...
function renderConversationCatchUpActions(agent) {
  ...
  const recent = load ? "" : `<button class="agent-floating-pill go-recent-fab hidden" type="button" data-go-recent>${iconSvg("chevronDown")}<span>Go to recent</span></button>`;
...
function renderAgentComposerPlacement(agent, skills, options = {}) {
  ...
  if (inquirySubmissionPending(agent.key, agent.latest_inquiry?.id)) {
    return { dock: `${promptQueue}${renderInquiryLoadingSkeleton(agent, "Submitting inquiry")}`, overlay: "" };
  }
...
function renderInquiryLoadingSkeleton(agent, label = "Loading inquiry") {
  return `
    <div class="inquiry-loading-skeleton" data-agent-key="${escapeAttr(agent.key)}" data-inquiry-submission-status="${escapeAttr(label === "Submitting inquiry" ? agent.latest_inquiry?.id || "" : "")}" role="status" aria-live="polite" aria-label="${escapeAttr(label)}" tabindex="-1">
...
function renderInquiryForm(agent, options = {}) {
  ...
  const submissionError = inquirySubmissionError(agent.key, inquiryId);
    ...
    <form id="inquiry-form" class="inquiry-form${fullScreen ? " inquiry-form-full-screen" : ""}" data-ds-form="inquiry" data-agent-key="${escapeAttr(agent.key)}" data-inquiry-id="${escapeAttr(inquiryId)}" aria-describedby="inquiry-validation" novalidate${modalAttributes}>
      ...
      <div class="inquiry-form-content">
        ...
        <div class="inquiry-fields">
          ${submissionError ? `<p class="inquiry-submission-error" role="alert" data-inquiry-submission-error>${escapeHtml(submissionError)}</p>` : ""}
...
function renderPromptQueue(agent) {
  ...
  const first = firstEntry.prompt || "Queued prompt";
  const excerpt = firstEntry.source === "delegation_callback"
    ? "Delegated reply waiting"
...
function renderPromptQueueEntry(agent, entry, index, options = {}) {
  // Queue entries persisted before state was added remain pending work. Match
  // the default shown below so their controls are not disabled as if claimed.
  const state = entry.state || "queued";
  const readOnly = options.readOnly === true;
  const editable = !readOnly && state === "queued" && entry.source !== "delegation_callback";
  const attachments = Array.isArray(entry.attachments) ? entry.attachments : [];
  const stateLabel = readOnly ? "Read" : state === "failed" ? "Dispatch failed" :
    state === "dispatching" ? "Dispatching" :
      state === "in_progress" ? "In progress" :
        state === "open" ? "Open" :
          state === "blocked" ? "Blocked" :
      state === "submitting" ? "Queueing..." : "Queued";
  const attachmentLabel = attachments.length ? ` · ${attachments.length} attachment${attachments.length === 1 ? "" : "s"}` : "";
  const sourceLabel = entry.source === "delegation_callback" ? "Delegated reply" :
    entry.source === "parent" ? "Parent message" :
      entry.source === "internal_continuation" ? "Agent continuation" :
        entry.source === "sleep_circuit_breaker_recovery" ? "Circuit-breaker recovery" :
      entry.source === "user" ? "User message" : "Message";
  const requiredLabel = options.requiredInstruction === true && entry.source !== "delegation_callback"
    ? '<span class="queue-work-required-label" data-queue-work-required>Required</span>'
    : "";
  const authorityLabel = entry.authority?.owner === "parent" ? ` · parent authority v${entry.authority.generation}` :
    entry.authority?.owner === "user" ? ` · user authority v${entry.authority.generation}` : "";
  const dueLabel = entry.not_before ? ` · due ${entry.not_before}` : "";
  ...
  const delegatedReply = parseDelegatedAgentReply(entry);
  return `
    <li class="prompt-queue-entry" data-prompt-queue-entry="${escapeAttr(entry.id)}">
      <div class="prompt-queue-entry-copy">
        ...
        <span>
          ${delegatedReply ? renderDelegatedAgentReply(delegatedReply, entry) : `<strong>${escapeHtml(entry.prompt)}</strong>`}
          <small class="prompt-queue-entry-meta"><span>${escapeHtml(`${sourceLabel} · ${stateLabel}${dueLabel}${authorityLabel}${attachmentLabel}`)}</span>${requiredLabel}</small>
        </span>
      </div>
      ${readOnly || entry.source === "delegation_callback" ? "" : `<div class="prompt-queue-entry-actions">
      ...
      ${readOnly || entry.source === "delegation_callback" ? "" : `${editError ? `<div class="pa-control-error prompt-queue-control-error" role="alert" data-pa-control-error="queue-edit:${escapeAttr(entry.id)}">${escapeHtml(editError.message)}</div>` : ""}
...
function parseDelegatedAgentReply(entry) {
  if (entry?.source !== "delegation_callback") return null;

  const prompt = String(entry?.prompt || "").trim();
  const batchPrefix = "Delegated agent reports:";
  const legacyPrefix = "Delegated agent report:";
  const json = prompt.startsWith(batchPrefix) ? prompt.slice(batchPrefix.length).trim() :
    prompt.startsWith(legacyPrefix) ? prompt.slice(legacyPrefix.length).trim() : prompt;
  let payload;
  try {
  ...
  const reports = payload.type === "delegated_agent_reports" && Array.isArray(payload.reports) ? payload.reports :
    payload.type === "delegated_agent_report" ? [payload.report || payload] : [];
  if (!reports.length || !reports.every(delegatedAgentReportPayload)) return null;

  return reports;
}

function delegatedAgentReportPayload(report) {
  if (!report || Array.isArray(report) || typeof report !== "object") return false;
  const statuses = ["success", "succeeded", "no_action_needed", "partial", "failed", "stopped", "blocked", "input_required"];
  return statuses.includes(String(report.status || "").trim().toLowerCase()) && typeof report.summary === "string";
}

function renderDelegatedAgentReply(reports, entry) {
  const stateKey = `delegated-reply:${entry.id || "unknown"}`;
  return `<div class="delegated-agent-reply" aria-label="Delegated agent report">
    ${reports.map((report, index) => renderDelegatedAgentReport(report, `${stateKey}:${index}`)).join("")}
  </div>`;
}

function renderDelegatedAgentReport(report, stateKey) {
  const agent = report.agent && typeof report.agent === "object" ? report.agent :
    report.child && typeof report.child === "object" ? report.child : {};
  const name = String(agent.name || agent.agent_key || "Delegated agent").trim() || "Delegated agent";
  const status = delegatedAgentReportStatusLabel(report.status);
  const summary = String(report.summary || "Delegated work completed.").trim() || "Delegated work completed.";
  const inquiry = report.inquiry && typeof report.inquiry === "object" ? report.inquiry : null;
  const attachments = delegatedAgentReportAttachments(report.attachments);
  return `<section class="delegated-agent-report">
    <div class="delegated-agent-report-header"><strong>${escapeHtml(name)}</strong>${statusBadge(status.label, status.intent, "chip")}</div>
    <div class="markdown-message-content delegated-agent-report-content">${renderMarkdown(summary, {
      viewerClassName: "markdown-viewer message-markdown-viewer",
      fallbackClassName: "message-markdown-fallback",
      emptyText: "",
      menuScope: stateKey,
    })}</div>
    ${renderDelegatedAgentRecovery(report.recovery)}
    ${inquiry ? renderDelegatedAgentReportInquiry(inquiry) : ""}
    ${attachments.length ? `<ul class="delegated-agent-report-attachments">${attachments.map(renderDelegatedAgentReportAttachment).join("")}</ul>` : ""}
  </section>`;
}

function renderDelegatedAgentRecovery(value) {
  if (!value || Array.isArray(value) || typeof value !== "object" ||
      value.type !== "sleep_circuit_breaker" || value.expected_safety_behavior !== true) return "";

  const state = String(value.state || "unknown").trim().toLowerCase() || "unknown";
  const stateLabel = titleFromKey(state);
  const parentAction = String(value.parent_action || "required").trim().toLowerCase() === "none" ? "None" : "Required";
  const delay = Number(value.delay_seconds);
  const delayLabel = Number.isFinite(delay) && delay >= 0
    ? `${delay} ${delay === 1 ? "second" : "seconds"}`
    : "";
  const notBefore = String(value.not_before || "").trim();
  const reason = String(value.reason || "").trim();
  const incidentId = String(value.incident_id || "").trim();
  const scheduled = state === "scheduled";
  const recoverySummary = scheduled
    ? "Automatic recovery is scheduled."
    : `Automatic recovery is ${escapeHtml(stateLabel.toLowerCase())}.`;
  return `<aside class="delegated-agent-recovery" data-delegated-agent-recovery data-recovery-state="${escapeAttr(state)}">
    <div class="delegated-agent-recovery-header"><strong>Expected safety recovery</strong>${statusBadge(stateLabel, scheduled ? "done" : "need", "chip")}</div>
    <p>Tycho stopped this child through the sleep circuit breaker. This is expected safety behavior. ${recoverySummary}</p>
    <dl>
      ${notBefore ? `<div><dt>Not before</dt><dd><time datetime="${escapeAttr(notBefore)}">${escapeHtml(new Date(notBefore).toLocaleString())}</time></dd></div>` : ""}
      ${delayLabel ? `<div><dt>Relative delay</dt><dd>${escapeHtml(delayLabel)}</dd></div>` : ""}
      <div><dt>Parent action</dt><dd>${escapeHtml(parentAction)}${scheduled && parentAction === "None" ? " while scheduled" : ""}</dd></div>
      ${reason ? `<div><dt>Reason</dt><dd><code>${escapeHtml(reason)}</code></dd></div>` : ""}
      ${incidentId ? `<div><dt>Incident</dt><dd><code>${escapeHtml(incidentId)}</code></dd></div>` : ""}
    </dl>
    <p class="delegated-agent-recovery-cancellation">A manual prompt or ownership change cancels automatic recovery.</p>
  </aside>`;
}

function renderDelegatedAgentReportInquiry(inquiry) {
  const message = String(inquiry.message || "").trim();
  const fields = Array.isArray(inquiry.fields) ? inquiry.fields.filter((field) => field && !Array.isArray(field) && typeof field === "object") : [];
  if (!message && !fields.length) return "";

  return `<aside class="delegated-agent-report-inquiry">
    <strong>Input requested</strong>
    ${message ? `<p>${escapeHtml(message)}</p>` : ""}
    ${fields.length ? `<ul>${fields.map(renderDelegatedAgentReportInquiryField).join("")}</ul>` : ""}
  </aside>`;
}

function renderDelegatedAgentReportInquiryField(field) {
  const key = String(field.key || "").trim();
  const label = String(field.label || key || "Response").trim() || "Response";
  const inputType = titleFromKey(String(field.input_type || "text").trim() || "text");
  const description = String(field.description || "").trim();
  const options = Array.isArray(field.options) ? field.options.map((option) => String(option).trim()).filter(Boolean) : [];
  return `<li><strong>${escapeHtml(label)}</strong> <span>${escapeHtml(inputType)}</span>${description ? `<small>${escapeHtml(description)}</small>` : ""}${options.length ? `<small>Options: ${options.map(escapeHtml).join(", ")}</small>` : ""}</li>`;
}

function delegatedAgentReportStatusLabel(value) {
  const key = String(value || "success").trim().toLowerCase();
  const labels = {
    success: ["Success", "done"],
    succeeded: ["Success", "done"],
    no_action_needed: ["No action needed", "detail"],
    partial: ["Partial", "need"],
    failed: ["Failed", "fail"],
    stopped: ["Stopped", "detail"],
    blocked: ["Blocked", "need"],
    input_required: ["Input required", "need"],
  };
  const [label, intent] = labels[key] || [titleFromKey(key || "unknown"), "detail"];
  return { label, intent };
}

function delegatedAgentReportAttachments(value) {
  if (!Array.isArray(value)) return [];
...
function renderDelegatedAgentReportAttachment(attachment) {
  const href = String(attachment.url);
  const title = String(attachment.title || href).trim() || href;
  const type = attachment.type === "file" ? "File" : "Link";
  const description = String(attachment.description || "").trim();
  const mimeType = String(attachment.mime_type || "").trim();
  const metadata = [type, mimeType].filter(Boolean).join(" · ");
  return `<li><a href="${escapeAttr(href)}" target="_blank" rel="noreferrer">${escapeHtml(title)}</a>${metadata ? `<small>${escapeHtml(metadata)}</small>` : ""}${description ? `<small>${escapeHtml(description)}</small>` : ""}</li>`;
}
...
function renderAgentArchive(key) {
  ...
  const queuedCount = Number(agent.prompt_queue?.pending_count || agent.prompt_queue?.entries?.length || 0);
  const callbackCount = arrayValue(agent.prompt_queue?.entries)
    .filter((entry) => entry.source === "delegation_callback").length;
  if (queuedCount > 0 && callbackCount !== queuedCount) {
    setHeader("Queued work is pending", agentHeaderLabel(agent), "A");
    replaceView(feedbackMessage(
      "Finish queued work before archiving",
      `${queuedCount} queued ${queuedCount === 1 ? "message is" : "messages are"} still durable. Open the agent to review or retry the queue; Tycho will not discard it.`,
      {
        intent: "warning",
        actions: `<button class="primary ui-button" data-variant="brand" type="button" data-open-agent="${escapeAttr(agent.key)}">Open agent</button>`,
      }
    ));
    return;
  }
  ...
  const callbackArchive = callbackCount > 0;
  replaceView(`
    <section class="notice archive-dialog">
      <strong>${escapeHtml(agent.name || agent.key)}</strong>
      <p>${callbackArchive
        ? `${callbackCount} unrun delegation ${callbackCount === 1 ? "callback is" : "callbacks are"} obsolete. Archive without running them and preserve their full messages in read-only history.`
        : "Archive this agent now, or clone it into a fresh editable agent before the source is archived."}</p>
      <div class="kv-grid">
      ...
      <div class="button-row form-actions ui-form-actions archive-actions">
        <button class="danger inline-icon-button ui-button" data-variant="danger" type="button" data-agent-action="archive" data-agent-key="${escapeAttr(agent.key)}"${callbackArchive ? ` data-confirm="Archive with ${callbackCount} unrun delegation ${callbackCount === 1 ? "callback" : "callbacks"}?" data-confirm-label="Archive with history"` : ""}>${iconSvg("folderDown")}<span>${callbackArchive ? "Archive with callback history" : "Archive"}</span></button>
...
function renderProjectWorkspace(route) {
  ...
  const searchTerm = state.projectWorkspaceSearchTerms[route.key] || "";
  const search = searchTerm.trim() ? state.projectWorkspaceSearches[projectWorkspaceSearchKey(route.key, searchTerm.trim())] : null;
    ...
    <section class="workspace-browser" aria-label="Project workspace file browser">
      ...
      <div class="workspace-browser-grid ${route.file ? "has-preview" : ""}">
        <section class="workspace-list-pane" aria-labelledby="workspace-list-title">
          ...
          <div class="workspace-search"><label class="sr-only" for="workspace-search-input">Search workspace files</label><input id="workspace-search-input" type="search" data-workspace-search data-project-key="${escapeAttr(route.key)}" value="${escapeAttr(searchTerm)}" placeholder="Search files" autocomplete="off"><span class="workspace-search-status" aria-live="polite" aria-atomic="true">${workspaceSearchStatus(searchTerm, search)}</span></div>
          ${renderProjectWorkspaceListing(route, searchTerm.trim() ? search : listing, { search: searchTerm.trim() })}
        </section>
...
function workspaceSearchStatus(searchTerm, search) {
  if (!searchTerm.trim()) return "";
  if (!search || search.loading) return "Searching…";
  if (search.error) return "Search failed";
  return `${search.total || 0} matches`;
}
...
function renderProjectWorkspaceListing(route, listing, options = {}) {
  const searching = Boolean(options.search);
  if (!listing || (listing.loading && !(listing.entries || []).length)) {
    return tychoLoadingState(searching ? "Searching workspace" : "Loading workspace", { body: searching ? `Finding files matching “${options.search}”.` : "Reading this project directory." });
  }
  ...
  if (searching && !entries.length) {
    return emptyState("No matching files", `No files match “${options.search}”.`);
  }
  ...
  return `
    ...
    <ul class="workspace-entry-list" aria-label="${escapeAttr(searching ? `Search results for ${options.search}` : `Files in ${route.path || "project root"}`)}">
      ${entries.map((entry) => `
        <li>
          <button type="button" class="workspace-entry ${entry.kind}" ${entry.kind === "directory" ? `data-workspace-directory="${escapeAttr(entry.path)}"` : `data-workspace-file="${escapeAttr(entry.path)}"`} data-project-key="${escapeAttr(route.key)}" aria-label="${escapeAttr(`${entry.kind === "directory" ? "Open directory" : "Preview file"} ${searching ? entry.path : entry.name}`)}">
            <span class="status-mark detail" aria-hidden="true">${iconSvg(entry.kind === "directory" ? "folder" : "fileText")}</span>
            <span class="workspace-entry-name"><strong>${escapeHtml(entry.name)}</strong><small>${escapeHtml(searching ? entry.path : (entry.kind === "directory" ? (entry.symlink ? "Linked directory" : "Directory") : ([formatBytes(entry.size_bytes), entry.symlink ? "linked file" : "file"].filter(Boolean).join(" / "))))}</small></span>
            <span aria-hidden="true">${entry.kind === "directory" ? "›" : ""}</span>
          </button>
        </li>
      `).join("")}
    </ul>
    ${listing.next_offset !== null && listing.next_offset !== undefined ? `<button class="workspace-load-more inline-icon-button ui-button" type="button" data-loading="${listing.loading ? "true" : "false"}" ${searching ? `data-load-more-workspace-search="${escapeAttr(route.key)}" data-workspace-query="${escapeAttr(options.search)}"` : `data-load-more-workspace="${escapeAttr(route.key)}" data-workspace-path="${escapeAttr(route.path || "")}"`} ${listing.loading ? "disabled" : ""}><span class="ui-spinner workspace-load-spinner" aria-hidden="true"></span><span>Load more</span></button>` : ""}
...
function renderProjectWorkspacePreview(route, preview) {
  ...
  const previewKey = projectWorkspaceKey(route.key, route.file);
  const editing = Boolean(state.projectWorkspaceEditing[previewKey]);
  const editAction = preview.editable
    ? `<button class="inline-icon-button ui-button" type="button" data-edit-workspace-file="${escapeAttr(route.file)}" data-project-key="${escapeAttr(route.key)}">${iconSvg("fileText")}<span>Edit file</span></button>`
    : "";
  const body = editing
    ? renderProjectWorkspaceEditor(route, preview)
    : renderProjectWorkspacePreviewContent(route, preview, previewKey);
  ...
  return `
    <div class="workspace-pane-header">
      ...
      <div class="workspace-preview-actions">${editAction}<button class="ui-icon-button ui-button" type="button" data-close-workspace-preview aria-label="Close file preview">${iconSvg("x")}</button></div>
    </div>
    ${body}
  `;
}

function renderProjectWorkspacePreviewContent(route, preview, previewKey) {
  if (preview.format === "image") {
    const imageUrl = state.projectWorkspaceImageUrls[previewKey];
    const error = state.projectWorkspaceImageErrors[previewKey];
    if (error) return workspaceFeedback("image", error, "file");
    if (!imageUrl) return tychoLoadingState("Loading image", { className: "attachment-loading-state" });
    return `<div class="workspace-image-preview"><img class="attachment-image-viewer" src="${escapeAttr(imageUrl)}" alt="${escapeAttr(preview.name || "Workspace image")}"></div>`;
  }
  if (preview.format === "markdown") {
    return renderMarkdown(preview.content || "", { breaks: true, menuScope: `workspace:${route.key}:${route.file}` });
  }
  return `<pre class="workspace-text-preview" tabindex="0" aria-label="Preview of ${escapeAttr(preview.name || workspaceBasename(route.file))}"><code>${escapeHtml(preview.content || "")}</code></pre>`;
}

function renderProjectWorkspaceEditor(route, preview) {
  return `
    <form class="workspace-file-editor" id="workspace-file-editor" data-project-key="${escapeAttr(route.key)}" data-workspace-path="${escapeAttr(route.file)}" data-workspace-version="${escapeAttr(preview.version || "")}">
      <label class="sr-only" for="workspace-file-content">File content</label>
      <textarea id="workspace-file-content" name="content" spellcheck="false" rows="20">${escapeHtml(preview.content || "")}</textarea>
      <div class="button-row"><button type="button" data-cancel-workspace-file-edit>Cancel</button><button class="primary ui-button" data-variant="brand" type="submit">Save file</button></div>
      <span class="field-hint">Saves only valid UTF-8 plain text up to 256 KB. Tycho rejects linked, sensitive, binary, and changed files.</span>
    </form>
  `;
}
...
function workspaceFeedback(code, message, kind, options = {}) {
  const labels = {
    ...
    stale: ["File changed", "Reload this file, review the latest content, and save again."],
    not_editable: ["File cannot be edited", "Tycho edits only safe, valid UTF-8 plain-text files."],
    image: ["Image unavailable", "Tycho could not load this image preview."],
    directory_too_large: ["Directory too large", "This directory exceeds the safe 5,000-entry browse limit."],
    search_too_large: ["Workspace too large", "This workspace exceeds the safe 10,000-entry search limit."],
...
function renderConversationBlocks(blocks, options = {}) {
  ...
  while (index < blocks.length) {
    ...
    const receiptProposalId = String(block?.metadata?.personal_assistant_action_proposal_id || "");
    const actionReceipt = options.personalAssistant && receiptProposalId !== "" && options.personalAssistantProposals?.some((proposal) =>
      String(proposal.id || "") === receiptProposalId);
    if (primaryConversationBlock(block) || actionReceipt) {
      const blockHtml = actionReceipt
        ? ""
        : (circuitBreakerRecoveryConversationBlock(block)
          ? renderCircuitBreakerRecoveryConversationBlock(block, index)
          : queueReadConversationBlock(block)
          ? renderQueueReadConversationBlock(block, index, options)
      ...
      if (options.personalAssistant && options.personalAssistantProposals) {
        rendered.push(personalAssistantProposalHtmlAfterBlock(block, options.personalAssistantProposals, options.renderedPersonalAssistantProposals || new Set()));
      }
      index += 1;
      continue;
    }
...
function queueReadConversationBlock(block) {
  return block?.kind === "message" && block?.metadata?.queue_read === true;
}

function circuitBreakerRecoveryConversationBlock(block) {
  const recovery = block?.metadata?.circuit_breaker_recovery;
  return block?.kind === "message" && block?.role === "user" && recovery && typeof recovery === "object";
}

function renderCircuitBreakerRecoveryConversationBlock(block, index) {
  const recovery = block.metadata.circuit_breaker_recovery;
  const delay = Number(recovery.delay_seconds);
  const delayLabel = Number.isFinite(delay) && delay >= 0 ? `${delay} second recovery` : "Automatic recovery";
  const callCount = Number(recovery.blocking_call_count);
  const threshold = Number(recovery.threshold);
  const callsLabel = Number.isFinite(callCount) && callCount > 0
    ? `${callCount} blocking ${callCount === 1 ? "call" : "calls"}`
    : Number.isFinite(threshold) && threshold > 0
      ? `${threshold} blocking-call threshold`
      : "Sleep-like blocking calls";
  const recoveryKey = recovery.incident_id || recovery.entry_id || block?.id || index;
  const stateKey = `circuit-breaker-recovery:${index}:${recoveryKey}`;
  const timeRow = (label, value) => value
    ? `<div><dt>${escapeHtml(label)}</dt><dd><time datetime="${escapeAttr(value)}">${escapeHtml(new Date(value).toLocaleString())}</time></dd></div>`
    : "";
  return `
    <details class="message-group circuit-breaker-recovery-block" data-circuit-breaker-recovery-block data-state-key="${escapeAttr(stateKey)}">
      <summary aria-label="${escapeAttr(`Recovered from circuit breaker, ${delayLabel}`)}">
        <span class="message-group-label"><span class="circuit-breaker-recovery-icon" aria-hidden="true">${iconSvg("heartPlus")}</span><span>RECOVERED FROM CIRCUITBREAKER</span></span>
        <span>${escapeHtml(delayLabel)}</span>
      </summary>
      <div class="message-group-body circuit-breaker-recovery-body">
        <dl class="circuit-breaker-recovery-details">
          <div><dt>Trigger</dt><dd>${escapeHtml(callsLabel)}</dd></div>
          ${timeRow("Stopped", recovery.stopped_at)}
          ${timeRow("Recovery queued", recovery.accepted_at)}
          ${timeRow("Eligible", recovery.not_before)}
          ${timeRow("Resumed", recovery.resumed_at)}
          ${recovery.incident_id ? `<div><dt>Incident</dt><dd><code>${escapeHtml(recovery.incident_id)}</code></dd></div>` : ""}
        </dl>
        <section class="circuit-breaker-recovery-instruction" aria-label="Recovery instruction">
          <strong>Recovery instruction</strong>
          <p>${escapeHtml(String(recovery.instruction || "Continue without blocking waits."))}</p>
        </section>
      </div>
    </details>
  `;
...
function renderQueueReadConversationBlock(block, index, options = {}) {
  const metadata = block?.metadata || {};
  const entries = Array.isArray(metadata.prompt_queue_entries) ? metadata.prompt_queue_entries : [];
  const projection = metadata.queue_work_projection || {};
  const requiredActions = Array.isArray(projection.required_actions) ? projection.required_actions : [];
  const requiredActionIds = new Set(requiredActions.map((entry) => String(entry?.id || "")));
  const recordedCount = Number(metadata.prompt_queue_entry_count);
  const count = Number.isInteger(recordedCount) && recordedCount >= 0 ? recordedCount : entries.length;
  const label = metadata.read_label || "Read queue";
  const countLabel = `${count} ${count === 1 ? "entry" : "entries"} read`;
  const workState = String(metadata.queue_work_state || projection.state || "read");
  const stateKey = `queue-read:${index}:${blockStateToken(block)}`;
  const detail = entries.length
    ? `<ol class="prompt-queue-list queue-read-list">
        ${entries.map((entry, entryIndex) => renderPromptQueueEntry(options.agent || {}, entry, entryIndex, {
          readOnly: true,
          requiredInstruction: requiredActionIds.has(String(entry?.id || "")),
        })).join("")}
      </ol>`
    : `<div class="message-content queue-read-legacy-content">${escapeHtml(String(block?.content || ""))}</div>`;
  return `
    <details class="message-group queue-read-block" data-queue-read-block data-state-key="${escapeAttr(stateKey)}">
      <summary aria-label="${escapeAttr(`${label}, ${countLabel}`)}">
        <span class="message-group-label"><span class="queue-read-icon" aria-hidden="true">${iconSvg("eye")}</span><span>${escapeHtml(label)}</span></span>
        <span class="queue-read-summary-status"><span class="queue-work-state">${escapeHtml(titleFromKey(workState))}</span><span>${escapeHtml(countLabel)}</span></span>
      </summary>
      <div class="message-group-body queue-read-body">
        ${detail}
      </div>
    </details>
  `;
...
function renderDelegationCallbackBlock(block, index, options = {}) {
  ...
  const reportName = bundled
    ? `${reports.length} delegated agents`
    : delegatedAgentReportName(report, reference);
  const result = bundled
    ? reports.map((item) => `## ${delegatedAgentReportName(item)}\n\n${String(item?.summary || "Delegated work completed.").trim()}`).join("\n\n")
    : String(report.summary || "Delegated work completed.").trim();
  return `
    <article class="message user delegation-callback-event" data-state-key="${escapeAttr(stateKey)}">
      <div class="message-header">
        <div class="message-role delegation-callback-header">
          <span class="delegation-event-brand">DELEGATED AGENT</span>
      ...
      <div class="message-content markdown-message-content delegation-callback-result">
      ...
      ${reports.map((item) => renderDelegatedAgentRecovery(item?.recovery)).join("")}
    </article>
  `;
}

function delegatedAgentReportName(report, reference = null) {
  return String(reference?.name || report?.child?.name || reference?.agent_key || report?.child?.agent_key || "Delegated agent").trim() || "Delegated agent";
}
...
function renderMessage(block, options = {}) {
  ...
  return `
    <article class="${escapeAttr(messageClassName(block))}"${summaryId ? ` data-run-summary-id="${escapeAttr(summaryId)}"` : ""}${requestId ? ` data-client-request-id="${escapeAttr(requestId)}"` : ""}${eventIdentity ? ` data-pa-event-id="${escapeAttr(eventIdentity)}"` : ""}${eventRunId ? ` data-pa-run-id="${escapeAttr(eventRunId)}"` : ""}${eventType ? ` data-pa-event-type="${escapeAttr(eventType)}"` : ""}>
      <div class="message-header">
        <div class="message-role">${identity || `${messageIcon(block)}<span>${escapeHtml(messageRoleLabel(block))}</span>`}${timeHtml}${summaryAttentionHtml}</div>
...
function messageRoleLabel(block) {
  if (block?.metadata?.queue_read === true) return block.metadata.read_label || "Read queue";
...
function summarySuppressionIndicator(metadata) {
  if (metadata?.notification_suppressed !== true || metadata?.unread_suppressed !== true) return "";
  return `<span class="summary-suppression-indicator" role="img" aria-label="Operator notifications and unread count suppressed" title="Operator notifications and unread count suppressed">${iconSvg("bellOff")}</span>`;
}
  ...
  return `
    <section class="pending-pr-contexts" aria-label="Pending pull request context" aria-live="polite">
      <div class="pending-attachment-header">
        ...
        <span>${escapeHtml(`${pending.length} section${pending.length === 1 ? "" : "s"}`)}</span>
      </div>
      <div class="pending-attachment-list">
...
function agentArchiveable(agent) {
  const queued = Number(agent?.prompt_queue_count || agent?.prompt_queue?.pending_count || agent?.prompt_queue?.entries?.length || 0);
  const callbacks = Number(agent?.prompt_queue_delegation_callback_count ||
    arrayValue(agent?.prompt_queue?.entries).filter((entry) => entry.source === "delegation_callback").length);
  return !!agent && !agent.archived && !resourceIsStale(agent) && !agent.running &&
    agent.status !== "running" && (queued === 0 || queued === callbacks);
}
...
function setHeader(title, subtitle, subtitleIcon = "folder", options = {}) {
  if (options.titleHtml) {
    els.title.innerHTML = options.titleHtml;
  } else if (options.agentTitle) {
  ...
  setHeaderSubtitle(subtitle, { hidden: options.hideSubtitle === true });
  syncUnreadAlert();
  syncDetailHeaderLayout();
}
...
function renderHeaderSubtitle() {
  ...
  els.subtitle.classList.toggle("hidden", state.headerSubtitleHidden);
}
...
function syncUnreadAlert() {
  ...
  const mark = agentSwitcherMark();
  syncAppBadge(unreadCount);

  els.mark.innerHTML = brandLogoHtml(unreadCount);
  mark.innerHTML = brandLogoHtml(unreadCount);
  mark.classList.toggle("has-unread", unreadCount > 0);
  mark.classList.toggle("unread-panel-open", state.unreadPanelOpen);
  mark.classList.toggle("shortcut-hints-visible", state.shortcutModifierActive);
  mark.setAttribute("aria-label", quickSwitchLogoLabel(unreadCount));
  mark.setAttribute("title", quickSwitchLogoLabel(unreadCount));
  mark.setAttribute("aria-expanded", state.unreadPanelOpen ? "true" : "false");
...
function toggleUnreadPanel() {
  ...
  rememberOverlayFocus("agent-switcher", agentSwitcherMark());
...
function closeUnreadPanel(options = {}) {
  ...
  state.unreadPanelLinkedAgentKey = "";
  syncUnreadAlert();
  if (options.restoreFocus) restoreOverlayFocus("agent-switcher", agentSwitcherMark());
...
function setShortcutModifierActive(active) {
  ...
  agentSwitcherMark().classList.toggle("shortcut-hints-visible", active);
}
...
function syncHeaderDetailCloseRoute(route) {
  const agentKey = ["agentSummary", "agentAttachment", "agentPullRequests"].includes(route.type)
    ? route.key
    : route.type === "projectDiff" && route.backTo?.type === "agent"
      ? route.backTo.key
      : "";
  const visible = Boolean(agentKey);
  els.headerDetailClose.dataset.closeAgentDetail = agentKey || "";
  els.headerDetailClose.classList.toggle("hidden", !visible);
  els.headerDetailClose.innerHTML = visible ? `${iconSvg("panelRightClose")}<span class="sr-only">Close detail view</span>` : "";
}
...
function scrollSettingsSection(id) {
  const sectionByTarget = {
    ...
    "settings-personal-assistant": "personal-assistant",
...
function agentMoreMenuHtml(agent) {
  ...
  const queued = Number(agent?.prompt_queue_count || agent?.prompt_queue?.pending_count || agent?.prompt_queue?.entries?.length || 0);
  const callbackOnly = queued > 0 && agentArchiveable(agent);
  const lifecycleSublabel = running ? "Stop agent first" : callbackOnly ? `${queued} callbacks; archive with history` :
    queued > 0 ? `${queued} queued; finish first` : agent.name || agent.key;
  ...
  return moreMenuHtml([
    ...
    moreMenuButton({
      ...
      danger: true,
      disabled: running || (queued > 0 && !callbackOnly),
    }),
  ]);
}
...
function setActiveNav(tab) {
  ...
  els.desktopSettings.classList.toggle("active", tab === "settings");
}
...
function agentSummaryDetail(agent, summaryId = "") {
  ...
  return {
    ...
    metadata: agent.structured_result || {},
  };
}
...
async function updateTycho() {
  ...
  const confirmed = await confirmAction({
    ...
    description: `${update.detail || "Homebrew update ready."} Homebrew will update this server, then restart its running Remote server and scheduler daemon automatically.`,
...
async function updateRemoteServer(serverKey) {
  ...
  const server = (state.servers || []).find((item) => item.key === key);
  if (!server || server.local || server.update?.available !== true || state.serverUpdateKey) return;

  const confirmed = await confirmAction({
    title: `Update ${serverDisplayName(server)} with Homebrew?`,
    description: `${server.update.detail || "Homebrew update ready."} The peer will restart its running Remote server and scheduler daemon automatically. This browser stays connected to the UI-serving server while it comes back.`,
    confirmLabel: "Update Tycho",
    intent: "warning",
  });
  if (!confirmed) return;
  ...
  clearTimeout(state.timer);
  state.timer = null;
  state.serverUpdateKey = key;
  setConnection(`Updating ${serverDisplayName(server)}`);
  render();
  try {
    const peerToken = remoteServerToken(key);
    await brokerPostWithHeaders(`/servers/${encodeURIComponent(key)}/update`, {},
      peerToken ? { "X-Tycho-Remote-Server-Token": peerToken } : {});
    await waitForPeerRestart(key);
    await requestServerResourceRefresh(key, { force: true });
    state.failureCount = 0;
    setConnection(`${serverDisplayName(server)} updated and restarted`);
    showGrowl(`${serverDisplayName(server)} updated and restarted`, "done");
  } catch (error) {
    state.failureCount += 1;
    const message = `Update failed for ${serverDisplayName(server)}: ${error.message}`;
...
async function waitForPeerRestart(serverKey, timeoutMs = 15_000) {
  const deadline = Date.now() + timeoutMs;
  const peerToken = remoteServerToken(serverKey);
  const headers = peerToken ? { "X-Tycho-Remote-Server-Token": peerToken } : {};
  await delay(500);
  while (Date.now() < deadline) {
    try {
      await brokerGetWithHeaders(`/servers/${encodeURIComponent(serverKey)}/activity`, headers);
      return;
    } catch (_error) {
      await delay(700);
    }
  }
  throw new Error("Remote peer did not come back online; use Refresh to check it again");
}
...
async function checkHarnessAuth(harness) {
  if (!harness || state.harnessAuthChecking?.[harness]) return;
  state.harnessAuthChecking = { ...state.harnessAuthChecking, [harness]: true };
  setConnection(`Checking ${harness} authentication`);
  render();
  try {
    const data = await apiPost(`/setup/harnesses/${encodeURIComponent(harness)}/auth-check`, {});
    state.setup = data.setup || state.setup;
    await refresh({ force: true });
    const item = (state.setup?.harnesses || []).find((candidate) => candidate.name === harness);
    setConnection(`${harness} authentication: ${harnessAuthSnapshot(item).label.toLowerCase()}`);
  } catch (error) {
    setConnection(error.message);
    showGrowl(error.message, "fail");
  } finally {
    state.harnessAuthChecking = { ...state.harnessAuthChecking, [harness]: false };
    render();
  }
}
...
els.desktopSettings.addEventListener("click", () => {
...
els.back.addEventListener("click", () => {
  ...
  else if (route.type === "agentPullRequests") navigate({ type: "agent", key: route.key });
  else if (route.type === "attachment") {
...
els.headerDetailClose.addEventListener("click", () => {
  const agentKey = els.headerDetailClose.dataset.closeAgentDetail;
...
function personalAssistantActionProposal(id) {
  return state.personalAssistantProposals.find((proposal) => String(proposal?.id || "") === String(id || "")) || null;
}

function personalAssistantActionMutationError(error) {
  const code = String(error?.code || error?.details?.code || error?.data?.details?.code || "").toLowerCase();
  if (["precondition_changed", "proposal_changed", "precondition_required"].includes(code)) {
    return "This exact preview is stale, so Tycho did not run the action. Review the updated server preview before confirming again.";
  }
  if (code === "stale_session") return "This proposal belongs to an earlier FRED session. It was not run; return to the active conversation and ask FRED to prepare it again.";
  if (error?.status === 404) return "Tycho no longer has this proposal receipt, so the outcome remains unknown. Check the current receipt before retrying.";
  return error?.message || "Tycho could not update this action receipt.";
}

function personalAssistantUnknownConfirmationError() {
  return "Confirmation delivery was interrupted; Tycho could not confirm whether this request reached the server. Check the current receipt before confirming again.";
}

function personalAssistantActionConfirmationBody(proposal, preflight) {
  const digest = preflight?.digest || proposal?.digest || "";
  const token = preflight?.precondition_token || preflight?.precondition?.token || "";
  return {
    confirmed: true,
...
function applyPersonalAssistantActionReceipt(response, id) {
  const receipt = response?.proposal || response;
  if (!receipt || typeof receipt !== "object") return null;
  const proposalId = String(receipt.id || id || "");
  if (!proposalId) return null;
  const current = state.personalAssistantProposals.findIndex((proposal) => String(proposal.id) === proposalId);
  if (current >= 0) state.personalAssistantProposals[current] = { ...state.personalAssistantProposals[current], ...receipt };
  else state.personalAssistantProposals = [...state.personalAssistantProposals, receipt];
  if (receipt.preflight && typeof receipt.preflight === "object") state.personalAssistantActionPreflights[proposalId] = receipt.preflight;
  if (receipt.state) delete state.personalAssistantActionStateOverrides[proposalId];
  return state.personalAssistantProposals.find((proposal) => String(proposal.id) === proposalId) || receipt;
}

async function runPersonalAssistantAction(id, operation) {
  const proposal = personalAssistantActionProposal(id);
  const context = personalAssistantSessionContext();
  const routeHashAtStart = location.hash;
  if (!proposal || !context || state.pendingPersonalAssistantProposalIds.has(id)) return;
  const currentRequest = () => location.hash === routeHashAtStart && parseRoute().type === "personalAssistant" && personalAssistantSessionMatches(context);
  if (operation === "confirm") {
    const preflight = personalAssistantActionPreflight(proposal);
    const digest = preflight?.digest || proposal.digest;
    const token = preflight?.precondition_token || preflight?.precondition?.token;
    if (!preflight || !digest || !token || preflight.details?.available === false || preflight.prepared === false) {
      state.personalAssistantActionErrors[id] = "Load the available server-owned preview before confirming this action.";
      void requestPersonalAssistantPreflight(id, { routeHashAtStart, context });
      state.renderedViewHtml = "";
      render({ preserveLiveEditor: true });
      return;
    }
  }
  state.pendingPersonalAssistantProposalIds.add(id);
  state.personalAssistantActionErrors[id] = "";
  state.personalAssistantActionStateOverrides[id] = operation === "confirm" ? "confirming" : "processing";
  state.renderedViewHtml = "";
  setConnection(operation === "confirm" ? "Confirming" : "Updating action", { preserveHeaderSubtitle: true });
  render({ preserveLiveEditor: true });
  let response = null;
  try {
    const preflight = operation === "confirm" ? personalAssistantActionPreflight(proposal) : null;
    const body = operation === "confirm" ? personalAssistantActionConfirmationBody(proposal, preflight) : {};
    response = await apiPost(`/personal-assistant/actions/${encodeURIComponent(id)}/${operation}`, body);
    if (!currentRequest()) return;
    const receipt = applyPersonalAssistantActionReceipt(response, id);
    if (!receipt?.state) delete state.personalAssistantActionStateOverrides[id];
    state.personalAssistantActionErrors[id] = "";
    state.renderedViewHtml = "";
    render({ preserveLiveEditor: true });
    try {
      await requestPersonalAssistantActions(routeHashAtStart, context);
    } catch (error) {
      if (currentRequest()) state.personalAssistantActionErrors.read = `Action receipt refresh failed: ${error.message}`;
    }
    if (!currentRequest()) return;
    const receiptState = String(personalAssistantActionProposal(id)?.state || receipt?.state || "").toLowerCase();
    showGrowl(receiptState === "executed" ? "Action completed" : receiptState === "queued" ? "Action queued" : "Action receipt updated", "done");
  } catch (error) {
    if (!currentRequest()) return;
    const code = String(error?.code || error?.details?.code || error?.data?.details?.code || "").toLowerCase();
    const previewConflict = ["precondition_changed", "proposal_changed", "precondition_required"].includes(code);
    const unknownConfirmation = operation === "confirm" && !previewConflict && code !== "stale_session" && error?.status >= 500;
    state.personalAssistantActionErrors[id] = unknownConfirmation ? personalAssistantUnknownConfirmationError() : personalAssistantActionMutationError(error);
    state.personalAssistantActionStateOverrides[id] = operation === "confirm" && !previewConflict && code !== "stale_session" ? "unknown" : String(proposal.state || "awaiting_confirmation");
    if (operation === "confirm" && previewConflict) {
      state.personalAssistantActionPreflights[id] = null;
      delete state.personalAssistantActionPreflightErrors[id];
      void requestPersonalAssistantPreflight(id, { routeHashAtStart, context }).catch(() => null);
    }
    setConnection(error.message || "FRED action failed", { preserveHeaderSubtitle: true });
  } finally {
    state.pendingPersonalAssistantProposalIds.delete(id);
    if (currentRequest()) {
      state.renderedViewHtml = "";
      render({ preserveLiveEditor: true });
      schedule(personalAssistantPollDelay());
    }
  }
}

async function checkPersonalAssistantActionReceipt(id) {
  const proposal = personalAssistantActionProposal(id);
  const context = personalAssistantSessionContext();
  const routeHashAtStart = location.hash;
  if (!proposal || !context || state.pendingPersonalAssistantProposalIds.has(id)) return;
  const currentRequest = () => location.hash === routeHashAtStart && parseRoute().type === "personalAssistant" && personalAssistantSessionMatches(context);
  state.pendingPersonalAssistantProposalIds.add(id);
  state.personalAssistantActionStateOverrides[id] = "checking";
  state.personalAssistantActionErrors[id] = "";
  state.renderedViewHtml = "";
  render({ preserveLiveEditor: true });
  try {
    await requestPersonalAssistantActions(routeHashAtStart, context);
    if (!currentRequest()) return;
    const latest = personalAssistantActionProposal(id);
    if (latest && ["ready", "awaiting_confirmation"].includes(String(latest.state || "").toLowerCase())) {
      delete state.personalAssistantActionStateOverrides[id];
      state.personalAssistantActionErrors[id] = "Tycho still shows this action awaiting confirmation. Review the exact preview, then confirm again if you still want it.";
    } else if (!latest) {
      state.personalAssistantActionStateOverrides[id] = "unknown";
      state.personalAssistantActionErrors[id] = "Tycho did not return this action receipt, so the outcome remains unknown. Check the current receipt before retrying.";
    }
  } catch (error) {
    if (!currentRequest()) return;
    state.personalAssistantActionStateOverrides[id] = "unknown";
    state.personalAssistantActionErrors[id] = `Could not check the current receipt: ${error.message}`;
  } finally {
    state.pendingPersonalAssistantProposalIds.delete(id);
    if (currentRequest()) {
      state.renderedViewHtml = "";
...
function handleViewClick(event) {
  ...
  const loadOlderConversationButton = event.target.closest("[data-load-older-conversation]");
  if (loadOlderConversationButton) {
    loadOlderConversationMessages(loadOlderConversationButton.dataset.loadOlderConversation);
    return;
  }
  ...
  if (event.target.closest("[data-initiate-personal-assistant]")) {
    navigate({ type: "tab", tab: "personal-assistant" });
    return;
  }

  if (event.target.closest("[data-edit-personal-assistant-settings]")) {
    state.personalAssistantSettingsEditing = !state.personalAssistantSettingsEditing;
    state.renderedViewHtml = "";
    render();
    return;
  }

  if (event.target.closest("[data-cancel-personal-assistant-settings]")) {
    state.personalAssistantSettingsEditing = false;
    state.renderedViewHtml = "";
    render();
    return;
  }

  if (event.target.closest("[data-restart-personal-assistant]")) {
    confirmAction({
      title: "Restart FRED with updated settings?",
      description: "Tycho archives today’s conversation, saves a compact handoff, and starts a new session using the current settings.",
      confirmLabel: "Restart FRED",
    }).then((confirmed) => {
      if (!confirmed) return;
      mutate(async () => {
        const previousKey = state.personalAssistant?.active_key || "";
        const response = await apiPost("/personal-assistant/restart", { confirmed: true });
        state.personalAssistant = response.personal_assistant || response;
        if (state.personalAssistant?.agent) state.agentDetails[state.personalAssistant.agent.key] = state.personalAssistant.agent;
        transferPersonalAssistantDraftArtifacts(previousKey, state.personalAssistant?.active_key || "");
        showGrowl("FRED restarted", "done");
      });
    });
    return;
  }

  if (event.target.closest("[data-reset-personal-assistant]")) {
    confirmAction({
      title: "Reset FRED?",
      description: "This permanently deletes any active FRED session and its logs, then clears FRED setup, daily continuity, and pending actions. This cannot be undone.",
      confirmLabel: "Reset FRED",
      intent: "danger",
    }).then((confirmed) => {
      if (!confirmed) return;
      mutate(async () => {
        const response = await apiPost("/personal-assistant/reset", { confirmed: true });
        state.personalAssistant = response.personal_assistant;
        state.personalAssistantProposals = [];
        showGrowl("FRED reset", "done");
        navigate({ type: "tab", tab: "personal-assistant" });
      });
    });
    return;
  ...
  const backToActivePersonalAssistant = event.target.closest("[data-pa-back-active]");
  if (backToActivePersonalAssistant) {
    event.preventDefault();
    clearPersonalAssistantHistorySelection();
    if (parseRoute().type === "personalAssistant") {
      state.renderedViewHtml = "";
      render({ preserveLiveEditor: true });
    } else {
      navigate({ type: "personalAssistant" });
    }
    return;
  }

  const retryPaPreflight = event.target.closest("[data-retry-pa-preflight]");
  if (retryPaPreflight) {
    const id = retryPaPreflight.dataset.retryPaPreflight;
    const context = personalAssistantSessionContext();
    delete state.personalAssistantActionPreflightErrors[id];
    state.personalAssistantActionPreflights[id] = null;
    state.renderedViewHtml = "";
    render({ preserveLiveEditor: true });
    void requestPersonalAssistantPreflight(id, { routeHashAtStart: location.hash, context }).catch(() => null);
    return;
  }

  const checkPaAction = event.target.closest("[data-check-pa-action]");
  if (checkPaAction) {
    void checkPersonalAssistantActionReceipt(checkPaAction.dataset.checkPaAction);
    return;
  }

  const confirmPa = event.target.closest("[data-confirm-pa-proposal]");
  if (confirmPa) {
    void runPersonalAssistantAction(confirmPa.dataset.confirmPaProposal, "confirm");
    return;
  }
  const rejectPa = event.target.closest("[data-reject-pa-proposal]");
  if (rejectPa) {
    void runPersonalAssistantAction(rejectPa.dataset.rejectPaProposal, "reject");
    return;
  }
  const verifyPa = event.target.closest("[data-verify-pa-proposal]");
  if (verifyPa) {
    void runPersonalAssistantAction(verifyPa.dataset.verifyPaProposal, "verify");
    return;
  }
  const historyButton = event.target.closest("[data-open-pa-history]");
  if (historyButton) {
    const id = String(historyButton.dataset.openPaHistory || "");
    const routeHashAtStart = location.hash;
    const context = personalAssistantSessionContext();
    const coordinator = state.personalAssistantRequests.coordinator;
    const sequence = ++coordinator.historySequence;
    coordinator.historySelection = id;
    state.personalAssistantHistoryEntry = null;
    state.personalAssistantHistoryLoading = true;
    state.personalAssistantHistoryError = "";
    state.renderedViewHtml = "";
    render({ preserveLiveEditor: true });
    apiGet(`/personal-assistant/history/${encodeURIComponent(id)}`).then((response) => {
      if (!personalAssistantHistoryRequestIsCurrent(id, sequence, routeHashAtStart, context)) return;
      const history = response.history || null;
      if (!history || String(history.id || "") !== id) throw new Error("The saved summary response did not match the selected entry.");
      state.personalAssistantHistoryEntry = history;
    }).catch((error) => {
      if (personalAssistantHistoryRequestIsCurrent(id, sequence, routeHashAtStart, context)) state.personalAssistantHistoryError = error.message;
    }).finally(() => {
      if (!personalAssistantHistoryRequestIsCurrent(id, sequence, routeHashAtStart, context)) return;
      state.personalAssistantHistoryLoading = false;
      state.renderedViewHtml = "";
  ...
  const updateServer = event.target.closest("[data-update-server]");
  if (updateServer) {
    closeDetailsOverlay(updateServer.closest("[data-server-action-menu]"));
    updateRemoteServer(updateServer.dataset.updateServer);
    return;
  }
  ...
  if (workspaceDirectoryButton) {
    clearProjectWorkspaceSearch(workspaceDirectoryButton.dataset.projectKey);
    navigate({
      type: "projectWorkspace",
  ...
  const editWorkspaceFileButton = event.target.closest("[data-edit-workspace-file]");
  if (editWorkspaceFileButton) {
    const key = projectWorkspaceKey(editWorkspaceFileButton.dataset.projectKey, editWorkspaceFileButton.dataset.editWorkspaceFile);
    state.projectWorkspaceEditing[key] = true;
    render({ preserveLiveEditor: true });
    document.querySelector("#workspace-file-content")?.focus();
    return;
  }

  if (event.target.closest("[data-cancel-workspace-file-edit]")) {
    const route = parseRoute();
    if (route.type === "projectWorkspace" && route.file) {
      delete state.projectWorkspaceEditing[projectWorkspaceKey(route.key, route.file)];
      render({ preserveLiveEditor: true });
    }
    return;
  }
  ...
  if (refreshWorkspaceButton) {
    const route = parseRoute();
    if (route.type === "projectWorkspace") {
      const searchTerm = state.projectWorkspaceSearchTerms[route.key]?.trim();
      const request = searchTerm
        ? ensureProjectWorkspaceSearch(route.key, searchTerm, { force: true })
        : ensureProjectWorkspace(route.key, route.path, { force: true });
      render();
      request.then(render);
    }
    return;
  ...
  const loadMoreWorkspaceSearchButton = event.target.closest("[data-load-more-workspace-search]");
  if (loadMoreWorkspaceSearchButton) {
    const projectKey = loadMoreWorkspaceSearchButton.dataset.loadMoreWorkspaceSearch;
    const query = loadMoreWorkspaceSearchButton.dataset.workspaceQuery || "";
    const request = ensureProjectWorkspaceSearch(projectKey, query, { append: true });
    render();
    request.then(render);
    return;
  }
  ...
  const archiveScheduleSessionButton = event.target.closest("[data-archive-schedule-session]");
  if (archiveScheduleSessionButton) {
    archiveScheduleSession(archiveScheduleSessionButton.dataset.archiveScheduleSession);
    return;
  }

  const archiveAllScheduleSessionsButton = event.target.closest("[data-archive-all-schedule-sessions]");
  if (archiveAllScheduleSessionsButton) {
    archiveAllScheduleSessions();
    return;
  }
...
window.addEventListener("scroll", repositionSummaryAttachmentMenusOnViewportScroll, true);

document.addEventListener("click", (event) => {
  if (eventPathIncludes(event, els.mark) || eventPathIncludes(event, agentSwitcherMark()) || eventPathIncludes(event, els.unreadPanel)) return;
  closeUnreadPanel();
});
...
els.view.addEventListener("input", (event) => {
  const workspaceSearch = event.target.closest("[data-workspace-search]");
  if (workspaceSearch) {
    state.projectWorkspaceSearchTerms[workspaceSearch.dataset.projectKey] = workspaceSearch.value;
    queueProjectWorkspaceSearch(workspaceSearch.dataset.projectKey, workspaceSearch.value);
    return;
  }
...
els.view.addEventListener("change", (event) => {
  const personalAssistantModel = event.target.closest("[data-pa-model-select]");
  if (personalAssistantModel) {
    const form = personalAssistantModel.closest("form");
    const effort = form?.querySelector("[data-pa-reasoning-effort]");
    const choices = reasoningEffortSuggestionsForModel("codex", personalAssistantModel.value);
    if (effort && choices.length) {
      const selected = choices.includes(effort.value) ? effort.value : (defaultReasoningEffortForModel("codex", personalAssistantModel.value) || choices[0]);
      effort.innerHTML = choices.map((value) => `<option value="${escapeAttr(String(value))}" ${value === selected ? "selected" : ""}>${escapeHtml(String(value))}</option>`).join("");
    }
    return;
  }
...
els.view.addEventListener("submit", (event) => {
  event.preventDefault();
  if (["personal-assistant-setup-form", "personal-assistant-settings-form"].includes(event.target.id)) {
    const form = event.target;
    const values = new FormData(form);
    const settingsUpdate = form.id === "personal-assistant-settings-form";
    mutate(async () => {
      const configured = await apiPost("/personal-assistant/setup", { model: String(values.get("model") || ""), reasoning_effort: String(values.get("reasoning_effort") || ""), timezone: String(values.get("timezone") || ""), personality: String(values.get("personality") || ""), external_events_prompt: String(values.get("external_events_prompt") || ""), confirmed: values.get("confirmed") === "on" });
      const response = settingsUpdate ? configured : await apiPost("/personal-assistant/open", {});
      state.personalAssistant = response.personal_assistant || configured.personal_assistant;
      if (!state.personalAssistant) state.personalAssistant = (await apiGet("/personal-assistant")).personal_assistant || null;
      if (state.personalAssistant?.agent) state.agentDetails[state.personalAssistant.agent.key] = state.personalAssistant.agent;
      state.personalAssistantSettingsEditing = false;
      showGrowl(settingsUpdate ? "FRED settings saved" : "FRED initiated", "done");
    }, { form });
    return;
  }
  ...
  const checkHarnessAuthButton = event.target.closest("[data-check-harness-auth]");
  if (checkHarnessAuthButton) {
    closeDetailsOverlay(checkHarnessAuthButton.closest("[data-overlay-menu]"));
    checkHarnessAuth(checkHarnessAuthButton.dataset.checkHarnessAuth);
    return;
  }
  ...
  if (event.target.id === "inquiry-form") {
    ...
    const submissionKey = inquirySubmissionKey(key, inquiryId);
    if (!submissionKey || state.pendingInquirySubmissionKeys.has(submissionKey)) return;
    ...
    const routeHashAtStart = location.hash;
    const currentRequest = () => personalAssistantRequestIsCurrent(adapter, routeHashAtStart);
    saveInquirySubmissionDraft(form);
    saveFormDraft(form);
    state.inquirySubmissionErrors.delete(submissionKey);
    state.pendingInquirySubmissionKeys.add(submissionKey);
    state.fullScreenInquiryKeys.delete(key);
    state.expandedInquiryKeys.delete(`${key}:${inquiryId}`);
    state.renderedViewHtml = "";
    schedule();
    render();
    focusInquirySubmissionStatus(inquiryId);
    closeAttachmentFlyout();
    mutate(async () => {
      ...
      state.pendingInquirySubmissionKeys.delete(submissionKey);
      state.pendingInquirySubmissionDrafts.delete(submissionKey);
    }, {
      isCurrent: currentRequest,
      onError: (error) => {
        state.pendingInquirySubmissionKeys.delete(submissionKey);
        state.inquirySubmissionErrors.set(submissionKey, `Could not submit inquiry: ${error.message}`);
        restoreInquirySubmissionFailure(key, inquiryId);
      },
    });
  }
  ...
  if (event.target.id === "workspace-file-editor") {
    const form = event.target;
    const projectKey = form.dataset.projectKey;
    const path = form.dataset.workspacePath;
    const version = form.dataset.workspaceVersion;
    if (!projectKey || !path || !version) return;

    const content = String(new FormData(form).get("content") || "");
    mutate(async () => {
      const data = await apiPut(`/projects/${encodeURIComponent(projectKey)}/workspace/file`, { path, content, version });
      const key = projectWorkspaceKey(projectKey, path);
      state.projectWorkspacePreviews[key] = { ...(data.preview || { path }), loading: false };
      delete state.projectWorkspaceEditing[key];
      render({ preserveLiveEditor: true });
      showGrowl("File saved", "done");
    }, { form });
  }
...
async function runAgentAction(agentAction) {
  ...
  if (confirmation) {
    ...
    const confirmed = await confirmAction({
      ...
      confirmLabel: agentAction.dataset.confirmLabel || titleFromKey(action),
...
async function archiveSelectedAgents() {
  ...
  const callbackCount = selectedAgents.reduce((count, agent) => count +
    Number(agent?.prompt_queue_delegation_callback_count ||
      arrayValue(agent?.prompt_queue?.entries).filter((entry) => entry.source === "delegation_callback").length), 0);
  const serverNames = [...new Set(selectedAgents.map((agent) =>
    serverDisplayName(resourceServer(agent.server_key))))];
  const confirmed = await confirmAction({
    title: `Archive ${keys.length} selected ${noun}?`,
    description: `Archived agents leave the active workspace on ${serverNames.join(", ")}. Running agents are skipped and remain active.${callbackCount > 0 ? ` ${callbackCount} unrun delegation ${callbackCount === 1 ? "callback will" : "callbacks will"} move to read-only history without running.` : ""}`,
...
async function archiveScheduleSession(key) {
  if (!key) return;
  const schedule = findSchedule(key);
  const label = schedule?.name || key;
  const confirmed = await confirmAction({
    title: `Archive ${label} session?`,
    description: "The current idle session will move to the archive. The schedule stays configured and its next run starts a fresh session. Running sessions and sessions with queued prompts are left in place.",
  ...
  mutate(async () => {
    const data = await apiPost(`/schedules/${encodeURIComponent(key)}/archive-session`);
    const message = data.archived ? "Schedule session archived; schedule kept" : "No current session to archive; schedule kept";
    setConnection(message);
    showGrowl(message, "done");
  }, {
    onError: (error) => showGrowl(error.message, "need"),
  });
}

async function archiveAllScheduleSessions() {
  const confirmed = await confirmAction({
    title: "Archive all schedule sessions?",
    description: "Idle sessions will move to the archive. Every schedule stays configured. Running sessions and sessions with queued prompts are skipped and reported.",
  ...
  mutate(async () => {
    const data = await apiPost("/schedules/archive-sessions");
    const archived = data.archived?.length || 0;
    const skipped = data.skipped?.length || 0;
    const failed = data.failed?.length || 0;
    const details = [skipped ? `${skipped} skipped` : null, failed ? `${failed} failed` : null].filter(Boolean).join(" / ");
    const message = details ? `Archived ${archived} schedule sessions / ${details}` : `Archived ${archived} schedule sessions`;
    setConnection(message);
    showGrowl(message, failed ? "fail" : (skipped ? "need" : "done"));
  }, {
    onError: (error) => showGrowl(error.message, "fail"),
  });
}
