...
function renderAgentReference(reference, options = {}) {
  ...
  const content = `<span class="status-mark ${escapeAttr(statusClass({ status: reference.status || reference.state }))}" aria-hidden="true">${iconSvg("botMessageSquare")}</span><span class="agent-reference-copy"><strong>${escapeHtml(label)}</strong><span>${escapeHtml(status)} · ${escapeHtml(serverDisplayName(server))} · ${escapeHtml(serverHealth)}${escapeHtml(reportStatus)}</span></span>`;
...
function renderSwitcherAgentRow(agent, index) {
  ...
  const linkedSymbol = linkedCount
    ? `<span class="switcher-linked-symbol" data-show-linked-agents="${escapeAttr(agent.key)}" aria-label="Show ${linkedCount} linked ${linkedCount === 1 ? "agent" : "agents"}" title="Show linked agents">${iconSvg("link")}<span>${escapeHtml(String(linkedCount))}</span></span>`
    : "";
  ...
  return `
    <button class="agent-row switcher-agent-row ${selected ? "selected" : ""}" type="button" data-open-agent="${escapeAttr(agent.key)}" data-switcher-index="${index}" ${selected ? "aria-selected=\"true\"" : ""}>
      ...
      <span class="switcher-agent-tools">${statusText}${linkedSymbol}</span>
    </button>
  `;
}
