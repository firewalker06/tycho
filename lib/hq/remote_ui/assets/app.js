...
function setActiveServerKey(key) {
  ...
  if (next === state.activeServerKey) return;
...
function renderServerSwitcher() {
  ...
  return `
    <section class="detail-card">
      <div class="detail-card-body" style="padding-top: 12px;">
        <div class="section-label"><strong>Servers</strong><span>${escapeHtml(activeServerLabel())} active</span></div>
...
function renderServerRow(server) {
  ...
  return `
    <div class="detail-row">
      ...
      <button class="inline-icon-button" type="button" data-select-server="${escapeAttr(server.key)}" ${active ? "disabled" : ""}>
        ${iconSvg(active ? "check" : "activity")}
