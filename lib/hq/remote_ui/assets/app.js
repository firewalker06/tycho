...
const state = {
  agents: [],
  archivedAgents: [],
  archivedAgentsLoaded: false,
  archivedAgentsLoading: false,
  archivedAgentsError: "",
  archivedAgentsByServer: {},
  archivedPaginationByServer: {},
  archivedLoadGeneration: 0,
  archivedSearchTimer: null,
  agentDetails: {},
  projects: [],
  schedules: [],
  scheduleDaemon: null,
...
function forgetArchivedServer(serverKey) {
  delete state.archivedAgentsByServer[serverKey];
  delete state.archivedPaginationByServer[serverKey];
  ...
  syncArchivedAgentsFromServerCache();
  state.archivedAgentsLoaded = false;
}
...
function applyResourceCatalog(data) {
  const previousServerKeys = new Set((state.servers || []).map((server) => server.key));
  const servers = Array.isArray(data?.servers) ? data.servers : [];
  state.servers = servers;
  const serverKeys = new Set(servers.map((server) => server.key));
  Object.keys(state.archivedAgentsByServer).forEach((key) => {
    if (!serverKeys.has(key)) delete state.archivedAgentsByServer[key];
  });
  Object.keys(state.archivedPaginationByServer).forEach((key) => {
    if (!serverKeys.has(key)) delete state.archivedPaginationByServer[key];
  });
  Object.keys(state.agentDetails).forEach((key) => {
    const detail = state.agentDetails[key];
  ...
  state.projects = servers.flatMap((server) =>
    (Array.isArray(server.projects) ? server.projects : [])
      ...
      .filter(Boolean));
  const addedArchiveServer = servers.some((server) => !previousServerKeys.has(server.key));
  if (addedArchiveServer && state.filters.agentState !== "active") {
    window.setTimeout(() => loadArchivedAgents({ force: true }), 0);
  }
...
async function archivedAgentPage(server, page) {
  const params = new URLSearchParams({ page: String(page), per_page: "100" });
  const search = state.filters.agents.trim();
  if (search) params.set("q", search);
  const query = params.toString();
  const path = resourceApiPath(server.key, `/agents/archived?${query}`);
  const response = await fetch(path, { headers: resourceHeaders(server.key) });
  return scopeApiResponse(await readJson(response), server);
}

async function loadArchivedAgents(options = {}) {
  if (state.archivedAgentsLoaded && !options.force) return;

  const generation = ++state.archivedLoadGeneration;
  state.archivedAgentsLoading = true;
  state.archivedAgentsError = "";
  if (parseRoute().type === "tab" && parseRoute().tab === "agents") {
    renderAgents({ focusFilter: options.focusFilter === true });
  }
  const failures = [];
  const servers = state.filters.server
    ? (state.servers || []).filter((server) => server.key === state.filters.server)
    : (state.servers || []);
  const results = await Promise.all(servers.map(async (server) => {
    try {
      const response = await archivedAgentPage(server, 1);
      return { server, response };
    } catch (error) {
      failures.push(`${serverDisplayName(server)}: ${error.message}`);
      return { server, response: null };
    }
  }));
  if (generation !== state.archivedLoadGeneration) return;

  results.forEach(({ server, response }) => {
    if (!response) return;
    state.archivedAgentsByServer[server.key] = response.agents || [];
    state.archivedPaginationByServer[server.key] = response.pagination || {};
  });
  syncArchivedAgentsFromServerCache();
  state.archivedAgentsLoaded = true;
  state.archivedAgentsLoading = false;
  state.archivedAgentsError = failures.join("; ");
  if (parseRoute().type === "tab" && parseRoute().tab === "agents") {
    renderAgents({ focusFilter: options.focusFilter === true });
  }
}

function syncArchivedAgentsFromServerCache() {
  const knownServers = new Set((state.servers || []).map((server) => server.key));
  state.archivedAgents = Object.entries(state.archivedAgentsByServer)
    .filter(([serverKey]) => knownServers.has(serverKey))
    .flatMap(([, agents]) => agents || []);
}

function hasMoreArchivedAgents() {
  const keys = state.filters.server ? [state.filters.server] : (state.servers || []).map((server) => server.key);
  return keys.some((key) => state.archivedPaginationByServer[key]?.next_page);
}

async function loadMoreArchivedAgents() {
  const restoreLoadMoreFocus = document.activeElement?.matches?.("[data-load-more-archived]") === true;
  const priorKeys = new Set(state.archivedAgents.map((agent) => agent.key));
  const generation = ++state.archivedLoadGeneration;
  state.archivedAgentsLoading = true;
  renderAgents({ focusFilter: false });
  const failures = [];
  const servers = (state.servers || []).filter((server) =>
    (!state.filters.server || server.key === state.filters.server) &&
    state.archivedPaginationByServer[server.key]?.next_page);
  const results = await Promise.all(servers.map(async (server) => {
    try {
      const page = state.archivedPaginationByServer[server.key].next_page;
      return { server, response: await archivedAgentPage(server, page) };
    } catch (error) {
      failures.push(`${serverDisplayName(server)}: ${error.message}`);
      return { server, response: null };
    }
  ...
  results.forEach(({ server, response }) => {
    if (!response) return;
    const existing = state.archivedAgentsByServer[server.key] || [];
    const byKey = new Map(existing.map((agent) => [agent.key, agent]));
    (response.agents || []).forEach((agent) => byKey.set(agent.key, agent));
    state.archivedAgentsByServer[server.key] = [...byKey.values()];
    state.archivedPaginationByServer[server.key] = response.pagination || {};
  });
  syncArchivedAgentsFromServerCache();
  state.archivedAgentsLoading = false;
  ...
  if (restoreLoadMoreFocus) {
    const loadMore = document.querySelector("[data-load-more-archived]");
    const firstAdded = state.archivedAgents.find((agent) => !priorKeys.has(agent.key));
    (loadMore || (firstAdded && document.querySelector(`[data-open-agent="${CSS.escape(firstAdded.key)}"]`)))
      ?.focus({ preventScroll: true });
  }
}

function scheduleArchivedAgentSearch() {
  window.clearTimeout(state.archivedSearchTimer);
  state.archivedSearchTimer = window.setTimeout(() => {
    state.archivedSearchTimer = null;
    if (state.filters.agentState !== "active") void loadArchivedAgents({ force: true, focusFilter: true });
  }, 250);
}

async function setAgentStateFilter(value) {
  const previous = state.filters.agentState;
  const next = ["active", "archived", "all"].includes(value) ? value : "active";
  state.filters.agentState = next;
  if (next !== "active") cancelBulkArchiveMode();
  if (next !== "active") await loadArchivedAgents({ force: previous === "active" || Boolean(state.archivedAgentsError) });
  renderAgents({ focusFilter: false });
}
...
function focusedWorkspacePollingSubtitle(route = parseRoute()) {
  const detail = {
    ...
    agentPullRequests: "PR details",
    agentSummary: "summary",
    attachment: "attachment",
    projectDiff: "project diff",
  }[route.type];
  return detail ? `Conversation poll paused / ${detail}` : "";
}
...
function renderAgents(options = {}) {
  ...
  const stateLabel = state.filters.agentState === "archived" ? "archived" : state.filters.agentState === "all" ? "active and archived" : "active";
  const resultSummary = query
    ? `${resultCount} ${resultCount === 1 ? "agent" : "agents"} match “${state.filters.agents.trim()}”`
    : `${resultCount} ${stateLabel} ${resultCount === 1 ? "agent" : "agents"} shown`;
  const scopedActive = state.filters.server
    ? currentAgents().filter((agent) => agent.server_key === state.filters.server)
    : currentAgents();
  const unread = scopedActive.filter((agent) => agent.unread).length;
  const paginationKeys = state.filters.server ? [state.filters.server] : (state.servers || []).map((server) => server.key);
  const archiveCount = state.archivedAgentsLoaded
    ? paginationKeys.reduce((count, key) => count + Number(state.archivedPaginationByServer[key]?.total || 0), 0)
    : "—";
  setHeader("Agents", `${scopedActive.length} active / ${archiveCount} archived / ${unread} unread`, "A");
    ...
    <section class="ui-searchable-collection" aria-label="Managed agents">
      <div class="top-actions ui-collection-toolbar">
        ...
        <div class="agent-toolbar-actions">
          ${agentFiltersHtml()}
          ${agentSortMenuHtml()}
          ${state.filters.agentState === "active" ? agentBulkControlsHtml() : ""}
        </div>
        ${agentMobileToolbarMenuHtml()}
      </div>
      <p class="ui-collection-summary" id="agent-results-summary" role="status">${escapeHtml(resultSummary)}</p>
      ${state.archivedAgentsLoading ? feedbackMessage("Loading archived agents", "Reading each available server archive.") : ""}
      ${state.archivedAgentsError ? feedbackMessage("Some archives are unavailable", state.archivedAgentsError, { intent: "warning", announce: "polite" }) : ""}
      ${state.filters.agentState === "active" ? renderBulkArchiveBar() : ""}
      <div class="ui-collection-results">
        ${results}
      </div>
      ${state.filters.agentState !== "active" && hasMoreArchivedAgents() ? `<div class="button-row"><button type="button" data-load-more-archived ${state.archivedAgentsLoading ? "disabled" : ""}>Load more archived agents</button></div>` : ""}
    </section>
  `);
...
function agentFiltersHtml() {
  return `<div class="agent-filter-controls">${agentStateFilterHtml()}${serverFilterHtml()}</div>`;
}

function agentStateFilterHtml() {
  return `
    <label class="server-filter-control" for="agent-state-filter">
      <span class="sr-only">Filter agents by archive state</span>
      <select id="agent-state-filter" data-filter-agent-state aria-label="Filter agents by archive state">
        <option value="active" ${state.filters.agentState === "active" ? "selected" : ""}>Active</option>
        <option value="archived" ${state.filters.agentState === "archived" ? "selected" : ""}>Archived</option>
        <option value="all" ${state.filters.agentState === "all" ? "selected" : ""}>Active and archived</option>
      </select>
    </label>
  `;
}
...
function agentMobileToolbarMenuHtml() {
  ...
  ].map(([value, label]) => moreMenuButton({
    label,
    sublabel: state.filters.agentState === value ? "Current agent filter" : "Filter agents",
    icon: value === "archived" ? "archive" : "robot",
    attrs: `data-agent-state-choice="${value}"`,
  }));
  ...
  const items = [
    ...
    ...sortItems,
    moreMenuSeparator(),
    ...(state.filters.agentState === "active" ? bulkItems : []),
  ];
...
function renderAgentsEmpty(query) {
  ...
  if (state.archivedAgentsLoading) return emptyState("Loading archives", "Reading archived agents from available servers.");
  if (state.filters.agentState === "archived") return emptyState("No archived agents", "Archived agents will appear here as read-only history.");
  if (state.filters.agentState === "all" && state.agents.length === 0) return emptyState("No agents found", "Create an agent or switch servers.");
...
function renderAgentRelationshipContext(agent) {
  ...
  return `
    <section class="agent-relationships" aria-label="Delegation relationships">
      ${parent ? `<div class="agent-relationship-section"><span class="agent-relationship-label">Started by</span>${renderAgentReference(parent)}</div>` : ""}
      ${children.length ? `<div class="agent-relationship-section"><span class="agent-relationship-label">Delegated agents</span><div class="agent-relationship-list">${children.map((reference) => renderAgentReference(reference)).join("")}</div></div>` : ""}
    </section>
  `;
}

function renderAgentReference(reference) {
  if (!reference) return "";
  ...
  const content = `<span class="status-mark ${escapeAttr(statusClass({ status: reference.status || reference.state }))}" aria-hidden="true">${iconSvg("botMessageSquare")}</span><span class="agent-reference-copy"><strong>${escapeHtml(label)}</strong><span>${escapeHtml(status)} · ${escapeHtml(serverDisplayName(server))} · ${escapeHtml(serverHealth)}</span></span>`;
  const unavailable = ["missing", "hidden"].includes(reference.state) || serverDisconnected(server);
  if (unavailable) return `<span class="agent-reference unavailable" aria-label="${escapeAttr(`${label}, ${status}, ${serverHealth}`)}">${content}</span>`;
  const href = routeHash({ type: "agent", key: reference.key });
  return `<a class="agent-reference" href="${escapeAttr(href)}" data-open-agent="${escapeAttr(reference.key)}" aria-label="Open ${escapeAttr(label)}, ${escapeAttr(status)}, on ${escapeAttr(serverDisplayName(server))}, ${escapeAttr(serverHealth)}">${content}</a>`;
}
...
function renderAgentComposer(agent, skills, options = {}) {
  ...
  const composer = `
    <form id="composer" class="composer${fullScreen ? " composer-full-screen" : ""}" data-agent-key="${escapeAttr(agent.key)}" data-agent-running="${agentIsRunning(agent) ? "true" : "false"}" data-speech-state="${escapeAttr(speechState)}"${modalAttributes}>
...
function renderSpeechModeButton(agent) {
  ...
  const speechState = speechModeStateForAgent(agent);
  const disabled = agentIsRunning(agent);
...
function renderDiffLine(line, lineIndex = 0, options = {}) {
  ...
  return `
    <${container} class="diff-line ${escapeAttr(kind)}${selected ? " selected" : ""}${selectable ? " selectable" : ""}">
      ${selectable ? `<input class="diff-line-select" type="checkbox" data-select-pr-diff-line data-agent-key="${escapeAttr(context.agentKey)}" data-pull-request-id="${escapeAttr(context.pullRequestId)}" data-snapshot-id="${escapeAttr(context.snapshotId)}" data-review-line-path="${escapeAttr(options.path)}" data-review-line-hunk="${Number(options.hunkIndex)}" data-review-line-index="${Number(lineIndex)}" aria-label="Select ${escapeAttr(kind)} line ${escapeAttr(lineNumber)} in ${escapeAttr(options.path)}" ${selected ? "checked" : ""} ${context.selectionLocked ? "disabled" : ""}><span class="diff-line-check${selected ? " selected" : ""}" aria-hidden="true">${selected ? "✓" : ""}</span>` : "<span></span>"}
...
function selectPullRequestDiffLine(target, extendRange = false) {
  ...
  const line = pullRequestDiffLine(
    diff,
    target.dataset.reviewLinePath,
    Number(target.dataset.reviewLineHunk),
    Number(target.dataset.reviewLineIndex)
  );
  if (!line) {
...
function pullRequestLineDescriptor(control) {
  return {
    path: control.dataset.reviewLinePath,
    hunk_index: Number(control.dataset.reviewLineHunk),
    line_index: Number(control.dataset.reviewLineIndex),
  };
}
...
function systemEventBlock(block) {
  return ["validation_retry", "delegation_event"].includes(block?.kind);
}
...
function listedAgents() {
  if (state.filters.agentState === "archived") return state.archivedAgents;
  if (state.filters.agentState === "all") return [...currentAgents(), ...state.archivedAgents];
  return currentAgents();
}
...
function renderHeaderSubtitle() {
  ...
  const className = [
    ...
    value.startsWith("Conversation poll paused") ? "subtitle-polling-paused" : "",
  ].filter(Boolean).join(" ");
...
function findAgent(key) {
  const listAgent = state.agents.find((agent) => agent.key === key) ||
    state.archivedAgents.find((agent) => agent.key === key) || null;
...
function upsertAgent(agent) {
  ...
  if (agent.archived) {
    const archivedIndex = state.archivedAgents.findIndex((item) => item.key === agent.key);
    if (archivedIndex >= 0) state.archivedAgents[archivedIndex] = { ...state.archivedAgents[archivedIndex], ...agent };
    else state.archivedAgents.unshift(agent);
...
els.view.addEventListener("click", (event) => {
  ...
  if (agentServerButton) {
    ...
    const refresh = state.filters.agentState === "active"
      ? Promise.resolve(renderAgents({ focusFilter: false }))
      : loadArchivedAgents({ force: true });
    void refresh.then(() => visibleAgentToolbarTrigger()?.focus({ preventScroll: true }));
    return;
  }

  const agentStateButton = event.target.closest("[data-agent-state-choice]");
  if (agentStateButton) {
    void setAgentStateFilter(agentStateButton.dataset.agentStateChoice)
      .then(() => visibleAgentToolbarTrigger()?.focus({ preventScroll: true }));
    return;
  }

  if (event.target.closest("[data-load-more-archived]")) {
    void loadMoreArchivedAgents();
    return;
  }
...
els.view.addEventListener("input", (event) => {
  ...
  if (filter) {
    ...
    if (filter.dataset.filter === "agents") scheduleArchivedAgentSearch();
    return;
  }
...
els.view.addEventListener("change", (event) => {
  if (event.target.matches("[data-filter-agent-state]")) {
    void setAgentStateFilter(event.target.value).then(() => {
      document.querySelector("[data-filter-agent-state]")?.focus();
    });
    return;
  }
  if (event.target.matches("[data-filter-server]")) {
    ...
    const refresh = state.filters.agentState === "active"
      ? Promise.resolve(renderAgents({ focusFilter: false }))
      : loadArchivedAgents({ force: true });
    void refresh.then(() => document.querySelector("[data-filter-server]")?.focus());
    return;
  }
...
els.view.addEventListener("submit", (event) => {
  ...
  if (event.target.id === "composer") {
    ...
    const submittedRouteHash = parseRoute().type === "agent" ? "" : location.hash;
    ...
    mutate(async () => {
      ...
      await apiPost(`/agents/${encodeURIComponent(key)}/messages`, {
        prompt,
        start: true,
        attachments,
      ...
      const submittedIds = new Set(submittedPullRequestContexts.map((context) => context.id));
      state.pendingPullRequestContexts[key] = pendingPullRequestContextsFor(key)
        .filter((context) => !submittedIds.has(context.id));
      if (submittedRouteHash && location.hash === submittedRouteHash) {
