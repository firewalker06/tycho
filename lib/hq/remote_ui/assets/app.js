...
function renderPersonalAssistantSettings(assistant) {
  ...
  return `<section class="detail-card" id="settings-personal-assistant" data-personal-assistant-state="${escapeAttr(item.state)}">
    <div class="detail-card-body" style="padding-top: 12px;">
      ...
      <p class="field-hint">${escapeHtml(personalAssistantReadiness(item))}</p>
...
function renderPersonalAssistantWelcome(item = {}) {
  const projects = state.projects || [];
  const projectOptions = projects.map((project) => `<option value="${escapeAttr(project.key)}">${escapeHtml(project.name || project.key)}</option>`).join("");
  const types = Array.isArray(item.capabilities?.types) ? item.capabilities.types : [];
  const capabilityText = types.length
    ? "Inspect Tycho, then prepare approved changes to projects, agents, or schedules."
    : "Inspect docs, agents, projects, and schedules. It can create or update projects, agents, and schedules after your approval.";
  return `<article class="pa-welcome"><strong>What would you like to do?</strong><p>FRED works on this Tycho server and asks before making changes.</p><div class="pa-suggestions"><button type="button" data-pa-suggestion="Show running agents and their status">Show running agents</button><button type="button" data-pa-suggestion="Explain a Tycho feature and how to use it">Explain a feature</button>${projects.length ? `<label class="pa-project-starter"><span>Prepare an agent for</span><select data-pa-project-picker>${projectOptions}</select><button type="button" data-pa-project-starter>Prepare</button></label>` : '<button type="button" data-pa-suggestion="Set up my first project on this Tycho server">Set up my first project</button>'}</div><details class="pa-capabilities"><summary>What FRED can do</summary><p>${escapeHtml(capabilityText)}</p></details></article>`;
}
...
function renderPersonalAssistantSetup(item) {
  const readiness = personalAssistantReadiness(item).replace("Codex is available on this server. Account access is checked when it runs.", "Codex found; account access is checked when it runs.");
  return `<section class="pa-setup ui-surface" aria-labelledby="pa-setup-title"><h2 id="pa-setup-title">Set up FRED</h2><p id="pa-setup-guidance">Review these defaults. FRED sends nothing until your first message.</p><p class="field-hint">${escapeHtml(readiness)}</p>${personalAssistantConfigForm(item)}</section>`;
}
...
function handleViewClick(event) {
  ...
  if (event.target.closest("[data-pa-project-starter]")) {
    const picker = event.target.closest(".pa-project-starter")?.querySelector("[data-pa-project-picker]");
    const project = state.projects.find((item) => item.key === picker?.value);
    const input = els.view.querySelector("#composer #prompt-input");
    if (input && project) {
