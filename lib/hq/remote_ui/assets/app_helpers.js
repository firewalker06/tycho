(function initTychoRemoteHelpers(global) {
  ...
  function parseRoute(hash, config = {}) {
    ...
    if (parts[0] === "agent" && parts[1] && parts[2] === "loop") {
      return { type: "agentLoop", key: parts[1] };
    }
  ...
  function routeHash(route) {
    ...
    if (route.type === "agentLoop") return `#agent/${encodeURIComponent(route.key)}/loop`;
  ...
  function routeStateKey(route) {
    ...
    if (route.type === "agentLoop") return `agent-loop:${route.key}`;
