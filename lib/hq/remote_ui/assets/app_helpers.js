(function initTychoRemoteHelpers(global) {
  ...
  function agentNeedsResponse(agent) {
    return Boolean(agent?.awaiting_input || agent?.latest_inquiry);
  }
  ...
  function compareQuickSwitchAgents(a, b) {
    if (agentNeedsResponse(a) !== agentNeedsResponse(b)) return agentNeedsResponse(a) ? -1 : 1;
  ...
  function parseRoute(hash, config = {}) {
    ...
    if (parts[0] === "personal-assistant") return { type: "personalAssistant" };
